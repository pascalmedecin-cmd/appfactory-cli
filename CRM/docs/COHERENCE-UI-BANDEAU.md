...
**Reste (c + d + reliquats esthétiques + CTAs prospection) :**
...
- **c** compteurs (pastilles `.length`→global signaux/pipeline/contacts/veille [relève de `ff_page_bandeau`], ProspectionTabs 999px→token, filter-badge) et **d** grille 8px (page-gutter tokenisé, largeur ancrée à gauche, Veille→gauche, rythme vertical) = lots suivants.
