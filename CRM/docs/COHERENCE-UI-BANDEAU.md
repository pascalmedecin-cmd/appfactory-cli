...
- **CTAs desktop Prospection = LIVRÉ 2026-07-21** (`ff_ui_coherence` OFF byte-identical). Mécanique retenue : hook `coh-prosp-cta` appendé (inconditionnel) aux 5 boutons + override co-localisé `:global(.coherence-ui) .coh-prosp-cta { padding: 8px 16px; font-weight: 600; transition: 220ms var(--ease-out-expo) }` + garde `@media(prefers-reduced-motion){ transition:none }` (contrat `.ws-btn` complet, `workspace.css:222`). **JAMAIS de class-swap vers `.ws-btn`** (les 5 CTAs portent `hidden md:inline-flex`, `.ws-btn` non-layered écraserait le `hidden` → visible mobile, cf. `feedback_ws_btn_unlayered_vs_tailwind_layered`). L'override ne touche NI display NI couleur/bordure → responsive intact + **teinte violette « Enrichir » préservée** (increment c « la teinte porte le sens »). Focus ring NON redéclaré (déjà global `app.css:311-325`). Élargissement absorbé par le wrap content-driven (fix collision `05ee3e6`). QA `tests/_qa-coherence-cta-prospection.mjs` (off/on/**mixed 2 flags** × 767-1920 × reduced-motion × dormant enrich injecté non-vacue) + workflow adversarial 4 lentilles + bug-hunter = SHIP. Latent 5-onglets (débordement tabs-bar) = pré-existant, watch simap/regbl. → `memory/project_coherence_ui_cta_prospection_bloque_collision_2026-07-21.md`.
