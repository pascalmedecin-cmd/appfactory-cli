...
- **d** grille 8px (page-gutter tokenisé, largeur ancrée à gauche, Veille→gauche, rythme vertical) = lot suivant.
  - **d1 (gouttière unique + largeur bornée/ancrée à gauche) CARTOGRAPHIÉ + planifié (2026-07-20)**, prêt à coder. Diagnostic : **double-gouttière** aujourd'hui (socle `main>div.p-4.md:p-6` 16/24px **+** gouttière propre de chaque brique 32/24/16 = 48/56/64 effectif). Mécanique retenue : **socle centralisateur** - le socle porte la gouttière UNIQUE (`--page-gutter` 32/24/16) + la borne (`--content-max`, ancrée à gauche `margin-inline:0`), **chaque brique met sa gouttière horizontale à 0** (`padding-inline` seul → vertical/FAB préservés) ; Veille+Aide dé-centrées. **Tout ou rien** sur les 10 pages (changer le socle sans neutraliser une brique = double-retrait 64px). ~13 fichiers. Plan file:line complet : `.atelier-209/coherence-ui/inventory-d1-implementation-plan.md` (scratch). **Décision design en attente Pascal** : `--content-max` (reco **1440px** data ; ne mord que sur grand écran ; ajustable = 1 token). Blast-radius vérifié (`/decoupe` = layout séparé, non touché).
