...
import { filmproLogoSvg } from '$lib/pdf/filmpro-logo';
...
// --- Logo FilmPro (source unique : src/lib/pdf/filmpro-logo.ts, tracés verbatim) ---------------
function logo(x: number, y: number, h: number): string {
	return filmproLogoSvg(x, y, h, C.logo);
}
