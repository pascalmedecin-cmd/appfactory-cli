...
<nav
	class="sidebar-root fixed top-0 left-0 h-full bg-primary-dark text-white flex flex-col transition-all duration-200 z-30"
			...
			>
				<span class="shrink-0 relative">
					...
					{#if collapsed && badge > 0}
						<span class="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 rounded-full bg-warning text-primary-dark text-[10px] font-bold flex items-center justify-center">{badge}</span>
					{/if}
				</span>
				{#if !collapsed}
					<span class="flex-1">{item.label}</span>
					{#if badge > 0}
						<span class="min-w-[20px] h-5 px-1.5 rounded-full bg-warning text-primary-dark text-xs font-bold flex items-center justify-center">{badge}</span>
					{/if}
				{/if}
			</a>
		{/each}
