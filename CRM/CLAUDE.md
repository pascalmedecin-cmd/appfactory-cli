...
**Prochaine attaque** : **Atelier 209 - Run 3, suite du code**. **Gate design VALIDÉ (2026-07-15)** : flux 3 étapes (déposer → colonnes → aperçu), upload + mapping assisté, tout compte connecté, ignorer doublons **avec dédup robuste multi-axes stress-testée**. **Fait + prouvé** : module dédup `import-dedup.ts` (31 tests / 15 familles verts) + D4 source `manuel` (130 tests, svelte-check 0/0). **Reste** (spec `docs/ATELIER-209-RUN3-SPEC.md`, ordre) : endpoint `import-liste` → UI modale 3 étapes → D3 source unique secteurs marque-aware → QA Colima + revue. Suivi → `docs/ATELIER-209-SUIVI.md` § Avancement du code.
...
- [x] ~~**Atelier 209 - Run 3 : gate design validé + fondations code (dédup multi-axes + D4)**~~ - 2026-07-15/16 (ultracode). Maquette import 4 vues validée par Pascal dans Chrome (flux 3 étapes, upload + mapping assisté, tout compte connecté). **Module dédup multi-axes** `import-dedup.ts` (4 axes nom+localité/tél/e-mail/domaine, marque-scopé, cross-source, `source_id` synthétique idempotent) : **31 tests / 15 familles de stress verts** (exigence Pascal « robuste multi-axes »). **D4 source `manuel`** : migration + Zod + libellés + test (130 tests verts, svelte-check 0/0). Reste : endpoint → UI → D3 → QA. → `docs/ATELIER-209-RUN3-SPEC.md` + SUIVI § Avancement.
