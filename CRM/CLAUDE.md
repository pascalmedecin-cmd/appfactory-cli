...
**Prochaine attaque** : Bloc 1 - **Bascule adresse portail** (SUPERVISÉ) - la QA 360 portail est verte (tous les AC bloquants phase 3-4 ✓, 0 H/C/M sécu), ce qui débloque la bascule. **NB OTP Supabase** : quota limité, utiliser un alias stable (pas d'URL jetable) pour tout test → `feedback_vercel_branch_alias_pour_smoke`. Plan : `.product-architect/portail/delivery-plan.md` (Session 3). **Préalable** : 12 fichiers de fix QA (repréfixage `CRM_BASE` + e2e + garde-fou) sont en working tree **non commités** sur `portail-session-1` — à commiter/déployer avant ou pendant la bascule (les liens racine marchent en prod via le filet 308, mais le fix supprime la fragilité).

### 1. Bascule adresse portail [SUPERVISÉ • high • ~0,5 session]

- **Pourquoi** : bascule URL = seul vrai risque (accès fondateurs), faite avec Pascal. Le code est déjà en prod (QA verte).
- [ ] **[EXÉCUTABLE - SUPERVISÉ]** Bascule adresse : alias `filmpro.vercel.app` + redirection ancienne→nouvelle + template OTP Supabase (dashboard) + metrics-baseline. Trap : vérifier `vercel inspect` après tout rollback. → voir `.product-architect/portail/delivery-plan.md` (Session 3) + `feature-flag-plan.md`.
  - **NB communication fondateurs** : avertir les 2 autres fondateurs du re-login N'EST PAS un livrable tracé (décision Pascal 2026-06-01). Pascal la déclenchera à la demande quand utile — ne pas l'ajouter comme tâche.

### Livré cette session

- [x] ~~QA 360 portail (Session 2) : 18/18 AC bloquants phase 3-4 verts + 11 liens CRM racine fixés~~ - Livré 2026-06-01 (xhigh, ~2,5h). **Méthode** : session test mintée **OTP-free** via service_role (`tests/mint-session.mjs`, `admin.generateLink` n'envoie aucun email → quota OTP préservé) → e2e authentifiés sur localhost. **Couverture** : `tests/portail.test.ts` 34/34 (AC-001/002/003/004/005/015/018/019/021 + dashboard no-308) ; **axe-core** home 0 violation serious/critical (AC-009) ; **responsive** 375/720/1024 (AC-010) ; **snapshot** baseline home (AC-021) ; **Lighthouse** prod Vercel mobile LCP 1985ms<2500 + CLS 0 + perf 97 (AC-011, le preview local mono-process fausse à 2,58s) ; **security-auditor** 0 H/C/M + OWASP 11/11 (AC-016, artefact `audit_secu_2026-06-01_portail`) ; svelte-check 0 (AC-013) ; Vitest 1488 verts hors baseline rouge 17 Sentry (AC-012) ; AC-006/007/008/014/020/022/024 vérifiés. **Findings fixés in-session (no-debt)** : bug-hunter+contracts ont sorti **11 liens internes CRM restés à la racine** (`/pipeline`, `/prospection`, `/log?/...`) dans 6 composants (TriageQueue, LeadExpress, PipelineQuickAdvance, FeedbackForm, FeedbackTable, QuickActionsFooter) — marchaient via le filet 308 mais fragiles → repréfixés via `CRM_BASE` + **garde-fou Vitest** `no-root-crm-links.test.ts` + assertion e2e no-308 ; `as never` inutiles retirés (RPC `entreprises_lookup_by_name` typée). **Dette chantier 2 tracée** : `resolve/+server.ts` doublon_detecte null/false, google-places dédup, DEFAULT_APP_URL ×4. **Fix en working tree NON commité**. → `audit_secu_2026-06-01_portail`, `.product-architect/portail/delivery-plan.md` (Session 2).
