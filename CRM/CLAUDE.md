...
**Prochaine attaque** : Bloc 1 - Découpe Films **Refonte UI premium (étape 3bis)**, AVANT l'étape 4 PDF. L'UI du résultat a été **rejetée 2× par Pascal** le 2026-06-05 (dense v1 5/10 ; minimaliste v3 pire). Cible = premium/moderne/**riche en artefacts visuels** (badges, icônes, pills), simple d'usage. Méthode imposée : **scraper 4-6 outils pro best-in-class** (Linear/Vercel/Stripe/Retool) → golden (`scripts/_decoupe_golden.ts`, vrai algo) **validé Pascal AVANT** portage Svelte. Acquis (data layer 24 tests, structure 4 écrans, algo, commits `bab1c38`→`142dbb9`) à ne pas refaire ; diagramme SVG abandonné (4/4 council). → voir `memory/feedback_decoupe_ui_premium_moderne_riche.md`.
...
- [ ] **[EXÉCUTABLE]** Étape 3bis - **Refonte UI premium Découpe** (benchmark scraping outils pro best-in-class → golden validé Pascal → portage Svelte). Cible premium/moderne/riche en artefacts visuels, pas dense (v1) ni épuré (v3). Montrer aussi la saisie (chantiers/produits/fiche), pas jugée par Pascal. Retirer `PlanDecoupeSvg`/`ChuteGauge` si non retenus. → voir `memory/feedback_decoupe_ui_premium_moderne_riche.md`.
- [ ] **[BLOQUÉ - 3bis validée]** Étape 4 - **checkpoint template PDF avec Pascal** (ADR-0005, AC-025) puis export PDF vectoriel client (jsPDF + svg2pdf), miroir de l'écran de résultat refondu.
