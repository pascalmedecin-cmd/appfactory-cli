...
**Dernière mise à jour :** 2026-07-20 (Cohérence UI **increment d1 LIVRÉ** = gouttière unique 32px + largeur bornée 1440 ancrée à gauche, socle centralisateur, 24 fichiers, `ff_ui_coherence` OFF ; QA DOM prouvée 1440/2000/1000 + revue adversariale 5 lentilles = 1 medium Tabs ≤1024 corrigé ; détail → « Livré cette session » + `docs/COHERENCE-UI-BANDEAU.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : **increment d2/d3 (suite grille)** - d2 = 4 zones + trame de colonnes commune (12-col façon Gouvernance), d3 = rythme vertical échelle de 8 (16/16/24/32/48), derrière `ff_ui_coherence` OFF. d1 (gouttière+borne) est **LIVRÉ** (voir « Livré cette session »). Alternative autonome sans gate : **CTAs desktop Prospection** (5, `hidden md:inline-flex`, passe dédiée, QA 767px). Les autres restes Phase 1 sont `[GATE mockup]` (reliquats esthétiques, accueil, import mobile). Contexte : Atelier 209 à **3/7 runs en prod**, 5 phases (figer base, timeline, LED Run 7, pipeline contact, coiffer) ; Phase 1 = lots 2A/2B/2C **+ d1** livrés (`ff_ui_coherence` OFF). → [[project_coherence_ui_d1_grille_2026-07-20]], [[feedback_qa_micro_mon_ressort_gate_pas_excuse]].
...
- [ ] **[EXÉCUTABLE]** Cohérence UI **d2/d3** (suite grille) : d2 = 4 zones + trame de colonnes commune (12-col façon Gouvernance) ; d3 = rythme vertical échelle de 8 (16/16/24/32/48). Derrière `ff_ui_coherence` OFF, QA avant/après par page. → `docs/COHERENCE-UI-BANDEAU.md` § d.
...
- [x] ~~**Cohérence UI lot 2A - modales/surfaces/eyebrows/titres**~~ - 2026-07-20. `4e45bb8` OFF : modales radius-2xl, shadow-card, 21 eyebrows, titres 700. → [[project_coherence_ui_lot2a_2026-07-20]].
