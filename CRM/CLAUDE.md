...
**Prochaine attaque** : Bloc 1 Page Log livré S185 (entry `af586294` → delivered). Reste côté Pascal : (1) appliquer la migration `supabase/migrations/20260513_001_feedback_entries.sql` au Supabase Dashboard prod (SQL Editor → coller → exécuter ; sans ça, `/log` retourne 0 entry et le form action `create` échoue), (2) smoke prod : login pascal@filmpro.ch puis antoine@filmpro.ch, click bouton flottant, saisir bug + suggestion, vérifier toast vert + apparition dans `/log`, click `À actionner` (admin) → badge change, click `Exporter en JSON` → fichier `feedback-YYYY-MM-DD.json` se télécharge, (3) Chrome DevTools manuel breakpoint < 1024px : confirmer FAB et lien sidebar « Log » absents + page `/log` affiche l'encart d'indisponibilité. Reste également en attente smoke prod du S183 : `/aide` (diagrammes refondus) + nav mobile (menu latéral qui se ferme). Tâches dormantes possibles (hors-scope nommé S182, à transmettre via UI cockpit) : tour guidé interactif, bouton « ? » contextuel sur les 9 pages, push première-connexion vers /aide.
