...
**Dernière mise à jour :** 2026-07-20 (Cohérence UI **lot 2B** livré derrière `ff_ui_coherence` OFF - INC-10/7/8/9/5 + fix a11y clear-focus prouvé e2e ; QA DOM OFF/ON + adversarial 4 lentilles clean. Reste increment b : reliquats esthétiques [mockup] + CTAs prospection + c/d. Vitest **2863**. → `docs/COHERENCE-UI-BANDEAU.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : chantier Atelier 209 à **3/7 runs en prod** ; reste 4 runs + les améliorations CRM, livrés en **5 phases logiques** (pas de priorisation, on a le temps → tout est à livrer) : (1) figer la base visuelle, (2) poser la timeline, (3) rendre LED autonome (Run 7 veille), (4) compléter le pipeline de contact (Runs 4/6/5), (5) coiffer (outil Coûts + aide bi-marque). Runs 4/5 bloqués chez Pascal (comptes Dropcontact/Pingen). Phase 1 avance : **lot 2B livré** (`ff_ui_coherence` OFF) ; reste dans la Phase 1 = reliquats esthétiques b (mockup), CTAs prospection (passe dédiée), puis c/d, puis retouches accueil + bandeau. Résumé exécutif validé Pascal 2026-07-20. Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[EXÉCUTABLE]** Cohérence UI **c** (compteurs : pastilles `.length`→global, ProspectionTabs token) + **d** (grille 8px : page-gutter tokenisé, largeur ancrée à gauche, Veille→gauche, rythme vertical) = derniers axes de l'increment. Page par page derrière `ff_ui_coherence`, QA OFF/ON DOM. → `docs/COHERENCE-UI-BANDEAU.md`.
...
- [x] ~~**Cohérence UI lot 2B (finir increment b) - INC-10/7/8/9/5 + fix a11y clear-focus**~~ - 2026-07-20 (ultracode). 18 fichiers derrière `ff_ui_coherence` OFF : champs 40px, 3 vides partagés→EmptyState, 6 kickers scopés, titre log, recherche Aide→SearchInput (primitive étendue `trailing`/`focus()`), + fix a11y clear-focus NON-gaté re-livré AVEC preuve e2e (souris+clavier). Workflow inventaire 6 agents + critique (1 BLOCKER SearchInput double-plomberie fusionné) ; QA DOM OFF/ON sur vraies vues (oubli câblage call-site FeedbackTable attrapé) ; adversarial 4 lentilles REFUTED-clean ; svelte-check 0/0, Vitest 2863. Déférés parqués : reliquats esthétiques [mockup], CTAs prospection [faux-vert], c/d. → [[project_coherence_ui_lot2b_2026-07-20]].
