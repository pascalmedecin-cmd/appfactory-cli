...
**Derniere mise a jour :** 2026-05-13 (S185 : page Log CRM LIVRÉE - 15 critères binaires verts, 58 nouveaux tests Vitest, 0 C/H/M audits Opus, migration SQL `20260513_001_feedback_entries.sql` APPLIQUÉE en prod via skill `supabase` + `pg` lib).
...
- **Pourquoi** : la page Log a été livrée S185 (5 commits, push, audits 0 C/H/M) et la migration Supabase prod a été appliquée par Claude in-session via le skill `supabase` + `pg` lib (smoke DB vérifiée : `table=1, policies=3, index=4, RLS=true, trigger=1`). Reste les smokes UI prod pour valider l'expérience bout en bout (admin + user + mobile breakpoint), plus le smoke résidu S183 (`/aide` diagrammes refondus + nav mobile fermeture menu) toujours en attente.
...
- [ ] **[EXÉCUTABLE]** Smoke prod user (`antoine@filmpro.ch`) : FAB visible, modale s'ouvre, peut créer un retour, voit la liste lecture seule (pas de boutons statut, pas d'export, pas de note interne).
- [ ] **[EXÉCUTABLE]** Chrome DevTools manuel breakpoint < 1024 px (iPhone 14 Pro Max preset) : FAB absent (display:none via media query), entrée sidebar « Log » absente (`.desktop-only-nav` masquée), page `/log` rend uniquement l'encart « Disponible uniquement depuis ordinateur » avec icône `desktop_windows`.
- [ ] **[EXÉCUTABLE]** Smoke prod résidu S183 : `/aide` (5 diagrammes SVG corrigés + schéma écosystème refondu + chiffres bulles centrés + tirets longs retirés + respiration accrue) + nav mobile (menu latéral se ferme à la sélection d'une page).
