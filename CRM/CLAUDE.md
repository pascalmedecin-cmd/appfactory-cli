...
**Dernière mise à jour :** 2026-07-03 (**workflow campagne e2e clos** : câblage page dédiée `640cda2` + message partage `6f385c0`, smoke prod Pascal OK → § Livré). Trunk = `main` ; Flag `ffCrmListesV2` **ON fondateurs**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : variable Daily Email (§ Chez Pascal). **Prochain bug :** #001.
