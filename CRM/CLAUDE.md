...
- [x] ~~Nettoyer Redirect URLs Supabase (retrait legacy `template-rho-three/auth/callback`)~~ - Livré 2026-06-05 (low, SUPERVISÉ). Pascal a retiré l'URL legacy du dashboard auth ; `filmpro-crm.vercel.app` **conservé** (le 308 du hook en a besoin) ; **OTP réel revalidé OK**. Clôt le Bloc 0. → voir `feedback_filmpro_vercel_deploy_cli.md`.
- [x] ~~Couvrir le redirect host (F-1) + finitions bascule (F-3)~~ - Livré 2026-06-05 (xhigh, ~0,8h, commit `b24c232`). Helper pur `legacyHostRedirect` extrait de `hooks.server.ts` + 4 Vitest (behavior-preserving, redeploy inutile). **Bonus no-debt** : (a) F-3, 4 fallbacks `PUBLIC_APP_URL` alignés sur le portail ; (b) **restauré 17 tests sécu H-18 morts** faussement étiquetés « baseline Sentry » (kit 2.59 `sequence()` request-store + `sentryHandle()` throw → fix = tester `baseHandle` exporté en isolation). **1509/1509 verts** (baseline 17 rouges → 0), svelte-check 0, audit 0 H/C/M. → voir `audit_secu_2026-06-05_finitions_bascule.md` + `feedback_svelte_hooks_test_baseHandle_isolation.md`.
...
→ Livré antérieur (Shell mobile V3 terrain + Portail Session 1 coding, 2026-06-01 ; cadrage Phase 1 + specs Phase 2 portail 2026-06-01, gates signés `.product-architect/portail/` ; V3 mobile terrain backend + specs 2026-05-31, S171→S192bis) → `archive/2026-06-01-sessions.md` + `archive/` (`2026-05-25/13/12/10/09/08-sessions.md`).
