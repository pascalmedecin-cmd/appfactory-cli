...
- [ ] **[EXÉCUTABLE - pas urgent, GATE mockup d'abord]** Retouches accueil (`AtelierShell.svelte`) : hero plus grand HD sans l'étirer (image encore floue → source 2x, cf. SUIVI), proportion 2/3 image / 1/3 bandeau (aujourd'hui l'inverse), blocs bandeau aérés + alignés grille, boutons OTP même largeur ET hauteur, fondu premium image↔bandeau. `redesign-skill`, maquette Chrome d'abord. → `docs/ATELIER-209-SUIVI.md` § Retouches accueil.- [ ] **[EXÉCUTABLE]** **Cohérence UI globale du CRM** - benchmark = app **Gouvernance** (→ [[feedback_gouvernance_benchmark_layout_lisibilite]]). **Principe** : on prend la *discipline de structure* de Gouvernance (lisibilité, layout, aération, alignement grille), **pas sa peau** - identité Atelier 209 (couleurs, ton, Inter) conservée. **(a) Bandeau de page : mockup VALIDÉ + increment 1 (Entreprises) LIVRÉ derrière `ff_page_bandeau` (16/07)** → dérouler **increments 2-4** (autres pages) selon `docs/COHERENCE-UI-BANDEAU.md` (composant `PageBand.svelte` + `isBandeauActive` + QA avant/après par page ; décisions : description **1 ligne** toujours, pastille compteur partout, textes = base à affiner). Mockup réf : `.atelier-209/coherence-ui/bandeau-mockup.html`. **Restent à mocker séparément** : (b) composants transverses (compteurs/onglets/boutons standardisés) ; (c) compteurs agrandis/visuels/consistants ; (d) grille à 2 niveaux. `redesign-skill` + `refactoring-ui`.
...
- [x] ~~**Cohérence UI - bandeau de page façon Gouvernance : mockup validé Pascal + increment 1 (Entreprises) derrière `ff_page_bandeau` (gate OFF)**~~ - 2026-07-16 (ultracode). Workflow benchmark (Gouvernance `PageHeader`) → mockup validé (bandeau in-page + Header allégé, bascule FilmPro/LED prouvée) → spec `docs/COHERENCE-UI-BANDEAU.md` → composant `PageBand.svelte` + flag `ff_page_bandeau` + page Entreprises. Source unique `isBandeauActive` (ferme le piège double/zéro-titre, revue bug-hunter). **OFF = zéro régression prouvée** (avant/après vraie vue) ; svelte-check 0/0 ; Vitest 2824. Décisions Pascal : description **1 ligne** toujours, pastille compteur partout, textes = base à affiner. Restent increments 2-4 (autres pages) + parties (b/c/d). → `docs/COHERENCE-UI-BANDEAU.md` + [[feedback_gouvernance_benchmark_layout_lisibilite]].
- [x] ~~**Page Campagnes : doublon titre/description du bandeau retiré + bouton « Nouvelle campagne » réaligné (PROD)**~~ - 2026-07-16 (ultracode). Le titre + le compteur live restent dans le `<Header>` du haut ; bouton primaire seul en haut à droite (convention refonte, cohérent Entreprises/Contacts). QA visuelle avant/après sur la vraie vue (session premium locale), Vitest 2817 + svelte-check 0/0. → [[feedback_gouvernance_benchmark_layout_lisibilite]].
