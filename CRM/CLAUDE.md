...
**Derniere mise a jour :** 2026-06-19 (**Vague 2 refonte CRM - cascade 4/5** : Entreprises+Contacts [`60a583c`] + Signaux [`0800b03`] + Pipeline [`35d7df7`] premium livrés [flag `ffCrmListesV2` OFF, **non déployé**, 1832 Vitest, revues 0 C/H/M/L] ; reste Prospection bloquée. Avant 18/06 : **Mini-projet Prospection CLOS** P1/P2/P3 [code pas déployé], Vague 1 `676a9d4`, cadrage 2 specs, 10 bugs `6b9f6e1`). Prod = alias `filmpro-portail.vercel.app` sur `810f2e6`. Détail → « Livré cette session » + archives.
...
**Prochaine attaque** : Bloc 1 - Vague 2 cascade, page **Prospection** (dernière des 5). ⚠ **BLOQUÉE** : la page Prospection est entremêlée avec le travail P1/P2/P3 prospection **non commité** (working tree sale, ~14 fichiers modifiés + non-suivis, jamais commités malgré « livré le 18/06 »). Trancher d'abord (commiter le P1/P2/P3 en commit séparé, PUIS cascader ; ou différer). **Signaux** (`0800b03`) + **Pipeline** (`35d7df7`) livrés (flag `ffCrmListesV2` OFF, **non déployés**). NB : Signaux recoupe la Vague 3 « Signaux condensés » - rester sur la ligne/fiche premium.
...
- **Fait** : golden validé Chrome ; Vague 2 **Entreprises + Contacts** (`60a583c`) + **Signaux** (`0800b03`) + **Pipeline** (`35d7df7`) livrés (non déployés, flag OFF) ; design-system partagé (KpiStrip, StagePill, SourcePill, `crm-*` dans app.css) + helpers purs fuzzés. Revues adversariales 0 C/H/M/L sur chaque cascade. **4/5 pages faites** ; reste **Prospection** (bloquée, voir Prochaine attaque). Golden Pipeline : `CRM/.product-architect/refonte-vague2/golden-pipeline-v1.html`.
- [ ] **[BLOQUÉ - working tree sale]** Cascade Vague 2 premium (flag `ffCrmListesV2`) sur la **dernière page : Prospection** (réutiliser StagePill/SourcePill/KpiStrip/`crm-*`). Prérequis : trancher le sort du travail P1/P2/P3 prospection non commité (commiter en commit séparé puis cascader, ou différer). Puis Vague 3 (Signaux condensés + actions, Prospection colonne Campagne + CSV, Dashboard Capsule) et Vague 4 (emailing individuel → nLPD → groupé). Garde-fous : golden avant toute nouvelle direction visuelle, recherche visible pas Cmd+K, sous-domaine d'envoi + base légale nLPD du stock avant 1er email, réunion 3 fondateurs pour les retraits.
- [ ] **[EXÉCUTABLE - optionnel]** Polir les fiches Entreprises + Contacts au niveau golden complet (grille de faits bordée + timeline historique) si souhaité.
...
- [x] ~~**Vague 2 cascade - Pipeline premium (kanban)**~~ - Livré 2026-06-19 (ultracode/xhigh, commit `35d7df7`, **non déployé**, flag `ffCrmListesV2` OFF). Cascade 5/5. Golden kanban validé Chrome (`.product-architect/refonte-vague2/golden-pipeline-v1.html`, **demande Pascal : cartes à hauteur égale**). KPI strip compact + cartes premium (accent d'étape palette workflow + logo initiales + hauteur égale + SourcePill source réelle) + hero fiche (StagePill/Badge + source + activité). Server : +`source_officielle` embed FK nommée (pas de PGRST201). 2 helpers purs fuzzés (`etapeAccent`/`entrepriseInitials`, 5k runs). Drag&drop/onglets/totaux préservés. svelte-check 0, **1832 Vitest**, build vert, carte premium vérifiée live (hauteurs égales). Revues adversariales 3 lentilles **0 C/H/M/L**, OFF byte-identique. → [[audit-secu-2026-06-19-vague2-cascade-signaux-pipeline]].
- [x] ~~**Vague 2 cascade - Signaux premium**~~ - Livré 2026-06-19 (ultracode/xhigh, commit `0800b03`, **non déployé**, flag `ffCrmListesV2` OFF). Cascade 3/5. KPI strip compact (bande géante → chips, « mal #1 empilement » du golden) + hero fiche (SourcePill SIMAP + activité) ; cartes déjà premium depuis V4. **Fix racine bug latent** : `fiber_new`/`track_changes` absents d'`icon-map.ts` → la bande prod affichait 2 « ? » ; ajout `Sparkles`/`Target` (seuls 2 call sites). svelte-check 0, **1827 Vitest**, build vert. Revues adversariales 3 lentilles **0 C/H/M/L**, OFF byte-identique (sauf le fix icône = strict gain). → [[audit-secu-2026-06-19-vague2-cascade-signaux-pipeline]].
