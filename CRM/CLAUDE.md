...
**Dernière mise à jour :** 2026-07-18 (**parité bi-marque : les 2 divergences gate-free LIVRÉES + DÉPLOYÉES prod** `96dc026` - dropdown campagne vide → « Aucune campagne » (bug 2) + logo PDF liste marque-aware (LED magenta verbatim / FilmPro byte-identique). Vitest 2847, svelte-check 0/0, revue adversariale 0 finding, QA réelle 2 marques (svg2pdf + dropdown). **Reste 4 divergences** (Pascal-gated) : #4/#5/#6 copies métier LED (maquette + mots-clés secteur) + bug 1 (repro env LED). → `docs/ATELIER-209-SUIVI.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : **Parité bi-marque - 2 divergences gate-free LIVRÉES + DÉPLOYÉES 18/07** (`96dc026` : bug 2 dropdown vide + logo PDF marque-aware ; QA réelle 2 marques). **Focus choisi par Pascal (18/07) : maquette copies LED (#4/#5/#6)** - proposer dans Chrome les libellés/placeholders LED marque-aware (secteurs événementiel/enseigne/stand : `ImportModal`, `SourceSearchFields`, `activity-types` Google Places) + **valider les mots-clés secteur LED** de `src/lib/prospection/secteurs.ts` [À VALIDER PASCAL], PUIS coder (miroir exact + QA avant/après par marque). Bug 1 (bouton import absent LED) = repro sur env LED réel d'abord. Autres tracks disponibles : Cohérence UI PART 2 lot 2+c+d (flag `ff_ui_coherence` OFF), flag bandeau (a), **Run 7** veille LED (cadrage, absorbe #8). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[EXÉCUTABLE - reste 4 divergences ; GATE maquette + validation mots-clés secteur LED]** Parité bi-marque LED ↔ FilmPro : 2 HIGH (17/07) + 2 gate-free (#3 logo PDF + bug 2 dropdown, 18/07 `96dc026`) LIVRÉS. **Reste 4** : **#4/#5/#6** copies métier LED (`ImportModal` défauts/placeholders vitrage, `SourceSearchFields` placeholders, `activity-types` Google Places FilmPro) = **maquette Chrome des libellés LED** (événementiel/enseigne/stand) + **valider mots-clés secteur LED** de `secteurs.ts` [À VALIDER PASCAL] AVANT de coder ; **bug 1** (bouton import absent LED) = **NON reproduit en code**, reproduire sur env LED réel d'abord ; **#8** hero Signaux « vitrage » = Run 7. Copies/logos visibles = « miroir exact + QA avant/après » + checklist parité par marque sur env LED peuplé. Détail file:line + fixes → `docs/ATELIER-209-SUIVI.md` § Parité bi-marque. → [[feedback_bi_marque_parity_qa_en_sortie]].
...
- **[WATCH] Parité bi-marque LED ↔ FilmPro (18/07)** : **2 HIGH + 2 gate-free (#3 logo PDF + bug 2 dropdown) corrigés + déployés**. **Reste 4** : #4/#5/#6 copies métier LED (gate maquette + mots-clés secteur LED) + bug 1 (repro env LED). Tant qu'elles traînent, LED n'est pas un miroir 100 % de FilmPro. → backlog dev « Parité bi-marque » + `docs/ATELIER-209-SUIVI.md` § Parité bi-marque + [[feedback_bi_marque_parity_qa_en_sortie]].
