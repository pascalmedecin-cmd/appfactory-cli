...
**Session courante :** 2026-07-01 - **Étiquettes migrées en page dédiée `/crm/campagnes/[id]/etiquettes` + destinataire de mailing, LIVRÉ PROD** (`e263240`). Destinataire inline (non persisté = 0 migration) sous la raison sociale, édition en lot, aperçu planche Avery réelle. QA verte (2379 vitest, e2e 4/4 Outfit) + revue 18 agents (0 C/H). **À FAIRE Pascal** : smoke OTP. Détail → § Livré + [[project_etiquettes_destinataire_page_dediee_2026-07-01]].
...
- [x] ~~**Étiquettes : page dédiée `/crm/campagnes/[id]/etiquettes` + destinataire de mailing**~~ - Fait 2026-07-01 (**LIVRÉ PROD** `e263240`, ultracode/xhigh). Migration volet V1 -> page dédiée (golden v2) : destinataire inline (générique, **non persisté = 0 migration**) sous la raison sociale (1 ligne ellipsée, cellule préservée), édition en lot, aperçu planche Avery réelle (SVG moteur + Outfit FontFace = PDF au pixel près), adresse incomplète ni sélectionnable ni imprimable, volet retiré + menu -> navigation + marges resserrées. TDD 14 tests, 2379 vitest, e2e 4/4 (Outfit réel), preuve visuelle. Revue **18 agents** -> 5 correctifs (race inter-campagnes, fuite aperçu, maxlength, défense sélection⊆complète, e2e Outfit), **sécu 0 C/H**. → [[project_etiquettes_destinataire_page_dediee_2026-07-01]] + [[audit_secu_2026-07-01_etiquettes_destinataire_page]]. *(Volet V1 `187f2ae` du 30/06 superseded par cette page.)*
- [x] ~~**Refonte Centre d'aide (`/crm/aide`) : contenu à jour + UI premium**~~ - 2026-06-30 (**LIVRÉ PROD** `d402fe7`). Contenu remis à jour (portail, Campagnes, Entreprises serveur, éditeur veille, Daily Email, 6 crons, V5, Log, OTP), golden v9, gate a11y permanent 4/4, fact-checker 0 finding, sécu 0 C/H/M. → [[project_refonte_aide_crm_2026-06-30]] + [[feedback_svg_flow_arrow_paint_order]] + [[feedback_e2e_tests_robustes_au_feature_flag]].
- [x] ~~**Refonte UI Découpe Films**~~ - 2026-06-30 (**LIVRÉ PROD** `0af198d`). Diagrammes échelle partagée + cote/règle, lignes-cartes, header/KPI unifiés 5 écrans. Golden 1er coup, QA tolérance zéro (e2e 13/13 axe 0). → [[project_decoupe_refonte_ui_2026-06-30]] + [[feedback_global_invalid_in_plain_css_file]].
- [x] ~~**Fix expiration session 7j + re-vérif prod (0 faille)**~~ - 2026-06-29 (`c7545ed`, **LIVRÉ PROD**). Plafond 7j = code mort (cookie s'auto-supprimait au seuil) → fix fail-secure + 2 tests, prouvé prod. Audits 360 (23+12 agents) 0 exploitable. → [[audit_secu_2026-06-29_session_delog_7j]] + [[feedback_cookie_maxage_equals_threshold_dead_check]].
→ Antérieurs (Bloc B socle vite 8 `e75919d` 28/06 ; Bloc A Entreprises `4b3974e`, Backlog `6c477cf` 28/06 ; Bloc D `cd5426e` → [[project_bloc_d_audit_360_PROGRESS]] ; icônes `a6ae0e1`, veille pont/PDF `b767056` 26/06 ; Bloc 0/2, Daily Email `d1db821`, Vague 3.3, Campagnes) → `archive/claude-md-crm-livre-2026-06-26.md` + mémoires veille 06-22/23/24.

### Watch list active après pivot

- **[WATCH] Étiquettes = page dédiée + destinataire en prod (`e263240`, 2026-07-01)** : `/crm/campagnes/[id]/etiquettes` (volet V1 retiré). Reste **smoke OTP** Pascal (sélection -> destinataire inline/lot -> Compléter -> aperçu -> PDF). Surveiller : (a) ligne destinataire sous le nom sur le PDF réel, (b) le moteur ellipse le destinataire à 1 ligne par prudence (0.62 em) - si trop agressif sur destinataires longs, calibrer un facteur dédié (jamais au prix d'un débordement), (c) quota search.ch « Compléter » (1/clic), (d) flag `ffCrmListesV2`. Rollback = flag OFF ou `vercel rollback`. → [[project_etiquettes_destinataire_page_dediee_2026-07-01]].
