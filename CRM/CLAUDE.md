...
**Dernière mise à jour :** 2026-07-01 (**Lot 1 - Refonte Signaux LIVRÉ PROD** `db6182e` : statut simplifié + bouton Statut + lien SIMAP ; détail → § Livré). Trunk unique = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`**. Prod = `filmpro-portail.vercel.app` : push `main` auto-déploie (mais **a raté cette fois** -> déploiement manuel `vercel --prod` ; surveiller). **Prochain bug :** #001.
**Session courante :** 2026-07-01 - **Refonte CRM 3 lots cadrée + validée** (workflow global : recherche -> prospection -> pipeline -> entreprise ; signaux détachés). **Lot 1 (Signaux) LIVRÉ PROD** (`db6182e`, migration prod OK, audit 19 agents 0 C/H/M). Lots 2 (Prospects+Pipeline) et 3 (Campagnes) spécifiés. **À FAIRE Pascal** : smoke OTP page Signaux. Détail → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
...
**Prochaine attaque** : **Lot 3 - Campagnes** (statut En cours/Active, recherche prospects embarquée par campagne, refonte UI page campagnes, badge statut en prospection). Specs → [[project_refonte_signaux_prospects_campagnes_2026-07-01]]. **Lot 1 (Signaux) + Lot 2 (Prospects+Pipeline) LIVRÉS CODE** (`db6182e` prod, `aff32cd` code). **Geste Pascal** : déployer la migration Lot 2 en prod (voir mémoire § GESTE DÉPLOIEMENT - vérifier volume `interesse` avant le backfill).
...
- [ ] **[EXÉCUTABLE]** Lot 3 - Campagnes (SUPERVISÉ • xhigh • ~1 session) : statut campagne En cours/Active (migration) ; recherche de prospects embarquée par campagne (EntrepriseSearchModal pré-cochée) ; refonte UI page campagnes ; statut visible sur badges campagne en prospection. **Payload** → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
...
- [ ] **[BLOQUÉ - Pascal : go déploiement prod Lot 2]** Déployer le Lot 2 (Prospects+Pipeline, code `aff32cd`). Outward-facing sensible : la migration inclut un **backfill qui crée de vraies opportunités** pour les prospects `interesse` existants. Mode op → mémoire § GESTE DÉPLOIEMENT : (1) `SELECT count(*) ... WHERE statut='interesse'` pour dimensionner, (2) appliquer `20260701000002_lot2_prospects_pipeline.sql` via pg/DATABASE_URL_ADMIN, (3) push `main`. Sur go de Pascal, Claude peut exécuter (1)+(2)+(3). → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
...
- [x] ~~**Lot 2 - Prospects + Pipeline : refonte statut + entrée pipeline**~~ - 2026-07-01 (**LIVRÉ CODE** `aff32cd`, ultracode/xhigh ; reste déploiement prod = geste Pascal). Statut prospect vide/a_contacter/ecarte/transfere ; migration (remap + colonne `opportunites.prospect_lead_id` + RPC `mark_lead_for_contact` atomique + backfill opps + `transfer_lead_to_crm` FOR UPDATE), prouvée postgres réel ; Zod durci ; query (file de tri vide, vue Écartés, tri date_import, retrait température) ; triage->RPC ; `convertToClient` au pipeline (récupérable) ; UI sidepane 2 boutons + Réactiver, batch contextuel, retrait colonne priorité/filtre température, édition statut inline, bouton Convertir client, fix lien SIMAP, liens dashboard. **Audit 14 agents : 9 findings TOUS corrigés, 0 H/C.** 2362 tests verts, svelte-check 0/0. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]] + [[audit_secu_2026-07-01_lot2_prospects_pipeline]].
- [x] ~~**Réparer le stack Supabase local (tests jetables)**~~ - 2026-07-01 (`7e21a18`). Cause racine : format `YYYYMMDD_NNN` cassait le tracking de version du CLI (collision `schema_migrations_pkey` dès 2 migrations/jour) -> from-scratch jamais fonctionnel. 42 migrations renommées au format canonique 14 chiffres, `supabase init` + analytics off (Colima), scripts alignés. From-scratch validé, ports 127.0.0.1 only. A permis de prouver toute la DB du Lot 2 sur postgres réel.
- [x] ~~**Lot 1 - Refonte Signaux : statut simplifié + bouton Statut + lien SIMAP, retrait conversion**~~ - Fait 2026-07-01 (**LIVRÉ PROD** `db6182e`, déploiement vercel `43q5zlw7i`, ultracode/xhigh). Bouton « Créer opportunité » (plantage FK 23503) + « Modifier » supprimés ; bouton « Statut » (À suivre/Archivé) + Supprimer séparé ; statuts `nouveau/a_suivre/archive` (migration data+CHECK appliquée prod via pg) ; lien « Voir sur SIMAP » (`project-detail/{source_id}`, format vérifié navigateur). 2357 tests, migration validée postgres jetable (bug d'ordre attrapé), audit 19 agents 0 C/H/M. Auto-deploy git n'a pas fiché -> déploiement manuel `vercel --prod`. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]] + [[audit_secu_2026-07-01_signaux_statut_lot1]].
- [x] ~~**Étiquettes : page dédiée + destinataire de mailing**~~ - 2026-07-01 (**LIVRÉ PROD** `e263240`). Page `/crm/campagnes/[id]/etiquettes`, destinataire inline (0 migration), aperçu Avery réel. 2379 vitest, revue 18 agents 0 C/H. → [[project_etiquettes_destinataire_page_dediee_2026-07-01]].
- [x] ~~**Refonte Centre d'aide (`/crm/aide`)**~~ - 2026-06-30 (**LIVRÉ PROD** `d402fe7`). Contenu à jour + UI premium, gate a11y 4/4, sécu 0 C/H/M. → [[project_refonte_aide_crm_2026-06-30]].
- [x] ~~**Refonte UI Découpe Films**~~ - 2026-06-30 (**LIVRÉ PROD** `0af198d`). Diagrammes échelle partagée, 5 écrans, e2e 13/13 axe 0. → [[project_decoupe_refonte_ui_2026-06-30]].
→ Antérieurs (Fix session 7j `c7545ed` 29/06 → [[audit_secu_2026-06-29_session_delog_7j]] ; Bloc B vite 8 `e75919d`, Bloc A `4b3974e` 28/06 ; Bloc D `cd5426e` ; veille pont/PDF `b767056` 26/06 ; Daily Email `d1db821`) → `archive/claude-md-crm-livre-2026-06-26.md`.

### Watch list active après pivot

- **[WATCH] Lot 1 Signaux en prod (`db6182e`, 2026-07-01)** : `/crm/signaux` refondu (bouton Statut À suivre/Archivé, lien SIMAP `project-detail/{source_id}`, migration statut `nouveau/a_suivre/archive` appliquée prod). Surveiller : (a) **smoke OTP** Pascal (bouton Statut + lien SIMAP), (b) le cron signaux stocke `source_id` = UUID SIMAP (le lien en dépend), (c) colonne dormante `opportunite_associee_id` + FK `fk_signaux_opportunite` laissées (nettoyage ultérieur), (d) l'auto-deploy git Vercel n'a pas fiché ce push (déploiement manuel requis) - vérifier si ça se reproduit. Rollback = `vercel rollback` (migration se remappe). → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
- **[WATCH] Smoke OTP Pascal en attente (livrables prod 06-30/07-01)** : Signaux (`db6182e`, bouton Statut + lien SIMAP), Étiquettes (`e263240`, PDF Avery + destinataire ; surveiller ellipse destinataire longue 0.62 em + quota search.ch « Compléter »), Centre d'aide (`d402fe7`, 3 niveaux, `prerender=false`), Découpe (`0af198d`, flag `ff_decoupe`, échelle partagée assumée). Rollback = flag OFF ou `vercel rollback`. → mémoires respectives [[project_refonte_signaux_prospects_campagnes_2026-07-01]] / [[project_etiquettes_destinataire_page_dediee_2026-07-01]] / [[project_refonte_aide_crm_2026-06-30]] / [[project_decoupe_refonte_ui_2026-06-30]].
...
- **[WATCH] Vercel auto-deploy git INCERTAIN** : censé auto-promouvoir `main` (vérifié 26/06) mais **n'a pas fiché le push `db6182e` du 01/07** (aucun build déclenché) → déploiement manuel `vercel --prod`. Surveiller : si ça se reproduit, toujours vérifier `vercel ls`/`vercel inspect filmpro-portail.vercel.app` après un push et déployer manuellement au besoin. Un `vercel rollback` passé peut aussi épingler l'alias.
