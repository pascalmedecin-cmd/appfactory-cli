...
**Prochain bug :** #001
**Session courante :** 2026-06-07 - **Bloc 1 re-audit seedé LIVRÉ** (`8c74885`, branche, non déployé prod) : seed prod éphémère 15/22/30 purgé, stories peuplées vertes, gate `vague2-a11y` 8/8 + bascule `aria-required-children`. **A débusqué un bug prod HIGH** : Pipeline vide quelles que soient les données (embed PostgREST `signaux_affaires` ambigu, PGRST201) → fixé + test de régression + remontée d'erreur. 1711 Vitest, svelte-check 0. Détail « Livré cette session ».
...
**Prochaine attaque** : décision déploiement prod de la branche `portail-session-1` (Vagues 4a→4d + **fix pipeline HIGH** `8c74885` + retrait Sentry `e561747`) - checkpoint Pascal. Le fix pipeline mérite la prod (bug prod réel, sans impact tant que 0 opportunité en base, mais cassant dès la 1re opp créée). Reliquat : 4d-copy bloqué (re-cadrage prospection V5).

### 1. Re-audit live avec données seedées [LIVRÉ 2026-06-07 `8c74885`]
- [x] ~~seed prod éphémère (15 entreprises / 22 contacts / 30 opportunités, marqueur `seedaudit-*`, purgé en finally) + stories peuplées re-déroulées~~ → counts onglets == badges (Pipeline 20/10/30, Contacts 22/6/6/3, Entreprises filtres OK), tri (`aria-sort`), slide-out rempli, **empty recherche M1 contextuel confirmé**. Pas de pagination serveur (ANO-01) → tri/recherche client. **A débusqué un bug prod HIGH** : Pipeline vide quelles que soient les données (2 FK `opportunites↔signaux_affaires` → embed nu PGRST201 → `data=null`) - fixé (FK nommée), `oppsRes.error` n'est plus avalé, test de régression. → outil `scripts/_audit_seed.mjs` (seed/purge réutilisable), `memory/feedback_postgrest_embed_ambigu_2fk.md`.
- [x] ~~gate `vague2-a11y` AVEC données + bascule `aria-required-children`~~ → 8/8 vert, Kanban peuplé (6 `role=list` / 30 `role=listitem`, vert non vacant vérifié), slide-out/focus LIVE-H2/H3 exercés sur vraies lignes. `aria-required-children` retiré de `OUT_OF_SCOPE_RULES` (enforced). → `tests/vague2-a11y.test.ts`.
...
- [x] ~~**Bloc 1 - Re-audit live seedé + bug prod HIGH pipeline**~~ - Livré 2026-06-07 (xhigh, `8c74885`, branche, NON déployé prod). Seed prod éphémère (15/22/30, `seedaudit-*` + `[SEED]`, **purgé en finally, DB restaurée à l'identique 0 résidu**) via service_role. **Bug prod HIGH débusqué** (invisible jusqu'ici car pipeline prod toujours vide) : le `load` pipeline embarque `signaux_affaires(...)` mais 2 FK existent entre `opportunites` et `signaux_affaires` → PostgREST PGRST201 → `oppsRes.data=null` → **Pipeline vide quelles que soient les données**. Fix cause racine : embed via FK nommée `signaux_affaires!opportunites_signal_affaires_id_fkey` + remontée d'erreur (le `load` avalait `oppsRes.error`) + test de non-régression (garde l'embed désambiguïsé). Re-audit a11y : gate `vague2-a11y` 8/8 AVEC données, `aria-required-children` basculé en enforcement (Kanban peuplé 6 `role=list`/30 `role=listitem`, vert non vacant). Stories peuplées vertes (counts onglets = badges, tri, slide-out rempli, empty recherche M1 contextuel). 1711 Vitest, svelte-check 0. → `scripts/_audit_seed.mjs` + `memory/feedback_postgrest_embed_ambigu_2fk.md`.
