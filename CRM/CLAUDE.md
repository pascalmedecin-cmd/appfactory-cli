...
**Dernière mise à jour :** 2026-07-17 (**parité bi-marque LED ↔ FilmPro : les 2 HIGH corrigés + prouvés** - page validation externe pilotée par marque + scoring marque-aware ; Vitest 2838, QA réelle 2 marques, audit sécu 0 H/C ; reste 6 divergences → `docs/ATELIER-209-SUIVI.md`. Règle QA parité gravée [[feedback_bi_marque_parity_qa_en_sortie]]. Bug 1 « bouton import LED » non reproduit en code). **Prochain bug :** #001.
...
**Prochaine attaque** : **Parité bi-marque - les 2 HIGH sont LIVRÉS 17/07** (validation externe + scoring, testés + QA réelle). Suite au choix : **reste parité (6 divergences, 5 medium dont bug 2 Pascal + 1 low)** OU **Cohérence UI increment b - PART 2 lot 2 + c + d** (lot 1 livré `7518cc1`, flag OFF ; détail → backlog + `docs/COHERENCE-UI-BANDEAU.md`), page par page derrière `ff_ui_coherence`, QA OFF/ON au DOM (⚠ toggle test : clé `null` pas `delete`), zéro régression, **attention aux espaces** (8px). Ou flag bandeau (a), ou **Run 7** veille LED (cadrage). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[EXÉCUTABLE - reste 6 divergences (5 medium + 1 low) ; GATE maquette pour les copies visibles]** Parité bi-marque LED ↔ FilmPro (prospection) : audit 17/07 = 8 divergences. **2 HIGH LIVRÉS 17/07** (page validation externe pilotée par marque + scoring marque-aware, testés + QA réelle 2 marques). **Reste 6** : (3) PDF liste = logo FilmPro (`pdf-liste-prospects.ts:421`) ; (4) copies vitrage `ImportModal` ; (5) `activity-types` Google Places FilmPro-only ; (6) copies vitrage `SourceSearchFields` ; (7) **dropdown campagne vide = bug 2 Pascal** (`MultiSelectDropdown.svelte:88`, fix `{:else}` + prop `emptyLabel`) ; (8, low) hero Signaux « vitrage » = déjà Run 7. Copies/logos visibles = « miroir exact + QA avant/après » + checklist parité par marque sur env LED peuplé. **Bug 1 Pascal (bouton import absent LED) = NON reproduit en code** → reproduire sur env LED réel d'abord. Détail file:line + preuves → `docs/ATELIER-209-SUIVI.md` § Parité bi-marque. → [[feedback_bi_marque_parity_qa_en_sortie]].
...
- [x] ~~**Parité bi-marque LED ↔ FilmPro - les 2 HIGH corrigés**~~ - 2026-07-17 (ultracode, xhigh, go Pascal « reco ok »). (1) **Page de validation externe publique** pilotée par marque (logo LED magenta / FilmPro + « l'équipe {marque} » + footer + teinte `[data-marque]` locale) ; (2) **scoring marque-aware** (`LeadScoring.marque`, `secteursCiblesFor` : FilmPro verbatim, LED = `LED_SECTEURS_CIBLES`), câblé aux 10 sites de scoring LED (veille + regbl/simap laissés FilmPro). Vitest **2838** (+11), svelte-check 0/0, build OK, **QA réelle Colima 2 marques** (pages `/validation` rendues : LED magenta / FilmPro bleu inchangé), audit sécu **0 H/C**. Reste 6 divergences (5 medium dont bug 2 Pascal + 1 low Run 7). → [[feedback_bi_marque_parity_qa_en_sortie]] + `docs/ATELIER-209-SUIVI.md` § Parité bi-marque.
...
- **[WATCH] Parité bi-marque LED ↔ FilmPro (17/07)** : **2 HIGH corrigés** (validation externe + scoring). **Reste 6** divergences présentation (5 medium dont bug 2 Pascal + 1 low Run 7) : tant qu'elles traînent, LED n'est pas un miroir 100 % de FilmPro. → backlog dev « Parité bi-marque » + `docs/ATELIER-209-SUIVI.md` § Parité bi-marque + [[feedback_bi_marque_parity_qa_en_sortie]].
