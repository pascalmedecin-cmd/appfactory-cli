...
- [ ] **[EXÉCUTABLE - pas urgent, GATE mockup d'abord]** Retouches page d'accueil (`AtelierShell.svelte`, Run 1) : hero plus grand HD **sans l'étirer** (image **encore floue** - source native 1184×864 trop petite, régénérer une source 2x, cf. SUIVI), proportion **2/3 image / 1/3 bandeau** (aujourd'hui l'inverse), blocs bandeau **aérés + 100 % alignés grille**, boutons OTP « recevoir le code » / « Se connecter » **même largeur ET hauteur** (grille), fondu premium image↔bandeau. `redesign-skill`, maquette Chrome d'abord. → `docs/ATELIER-209-SUIVI.md` § Retouches accueil.
...
- [ ] **[EXÉCUTABLE - GATE mockup à valider Pascal]** **Cohérence UI globale du CRM** - benchmark = app **Gouvernance** (référence layout/lisibilité, → [[feedback_gouvernance_benchmark_layout_lisibilite]]) : **(a)** bandeau de page répliqué sur **toutes les pages** façon Gouvernance = icône premium + nom de page + courte description 1 ligne (note d'humour subtile) ; check visuel alignement icône↔titre ; **(b)** standardiser les composants **transverses** (compteurs, onglets, boutons…) → app hyper cohérente et navigable (« comme chez soi »), rationnel pas inventé ; **(c)** compteurs **agrandis**, plus qualitatifs/visuels, consistants partout ; **(d)** grille **à 2 niveaux** (niv. 1 = grille partagée des éléments principaux pour la lisibilité globale ; niv. 2 = grille de page pour lisibilité/navigation locale). `redesign-skill` + `refactoring-ui`. **Mockup Chrome validé par Pascal AVANT tout code.** (Pascal 16/07.)
- [ ] **[EXÉCUTABLE - pas urgent, maquette validée]** Outil « Coûts des outils » Atelier 209 : sortir la page Coûts du CRM (`/crm/dashboard/couts`) → 3e outil sur l'accueil (à côté de Découpe), refonte premium « Heure bleue ». Agrège 3 sources live via API : veille Claude API (`cost_audit_runs`), **fal.ai** ([billing](https://fal.ai/docs/api-reference/platform-apis/for-accounts), tous coûts, **sans mention Enseignement**), **Firecrawl** ([credit-usage](https://docs.firecrawl.dev/api-reference/endpoint/credit-usage), abo intermittent). Exclut l'abo Claude (perso). Maquette `.atelier-209/retours-maquettes/atelier209-retours.html`. (Pascal 16/07 : pas urgent.)
...
- [x] ~~**Retours macro Pascal (16/07) : robustesse import + logo LED + bouton import + Campagnes pleine largeur (DÉPLOYÉ PROD)**~~ - 2026-07-16 (ultracode). 8 retours : logo LED blanc (halo = frange chromatique, pas un filtre), bouton import persistant/proéminent, Campagnes pleine largeur ; **12 bugs robustesse import corrigés+testés** (stress adversarial). Vitest 2817, svelte-check 0/0, build OK, QA visuelle + revue diff 0 défaut ; merge `a987f6d` smoke vert. Backlog logé (Coûts, aide, WATCH veille LED). → [[project_atelier_209_run3_import_liste_2026-07-16]].
- [x] ~~**Atelier 209 Run 3 : QA visuelle import + e2e + resize colonnes (DÉPLOYÉ PROD)**~~ - 2026-07-16. e2e Playwright import + audit 20 agents → 7 défauts micro corrigés + resize Entreprises/Contacts. Vitest 2801, merge `c106a67` smoke vert. → [[project_atelier_209_run3_import_liste_2026-07-16]].
- [x] ~~**Atelier 209 Run 3 : import de liste DÉPLOYÉ EN PROD**~~ - 2026-07-16. Écran import CSV/TSV marque-scopé (dédup multi-axes, NPA→canton, secteur) + modale + onglet Ma liste + D3/D4. Vitest 2801 + 15 intégration, merge `4e3f149` smoke vert. → [[project_atelier_209_run3_import_liste_2026-07-16]] + [[audit_secu_2026-07-16_atelier209_run3_import_liste]].
