...
**Prochaine attaque** : la **Phase 1 « figer la base visuelle » est quasi close** (lots 2A/2B/2C + d1 + **d2/d3** livrés, tous `ff_ui_coherence` OFF). **Les CTAs desktop Prospection sont BLOQUÉS** (tentés + revertés 2026-07-21 : la revue adversariale a découvert une **collision responsive prod pré-existante** dans la tabs-shell — onglets↔CTA overlap ≤~1280 sidebar déployée — que tout gain de largeur des CTA aggrave ; nouveau bug prod `[GATE mockup]` en Phase 1, le CTA re-passe après le fix). Option sans gate restante : **allumer `ff_ui_coherence` + `ff_page_bandeau` sur les fondateurs** pour voir toute la cohérence UI en vrai (rendu prouvé, réversible). Le reste Phase 1 est `[GATE mockup]` (fix collision tabs-shell, reliquats esthétiques, accueil, import mobile) + le **redessin KpiStrip→grille/12-col parqué**. Ensuite **Phase 2 = timeline d'interactions** (`[GATE mockup]`). Contexte : Atelier 209 à **3/7 runs en prod**, 5 phases (figer base, timeline, LED Run 7, pipeline contact, coiffer). → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]], [[project_coherence_ui_d3_grille_2026-07-21]], [[feedback_qa_micro_mon_ressort_gate_pas_excuse]].
...
- [ ] **[BLOQUÉ - déploiement du fix collision tabs-shell ci-dessous]** Cohérence UI - **CTAs desktop Prospection** (5, `hidden md:inline-flex`) : **tenté 2026-07-21, REVERTÉ** (aggravait la collision pré-existante ; le fix collision a été livré depuis). Mécanique validée à re-appliquer : override co-localisé `.coh-prosp-cta` (jamais class-swap `.ws-btn` cf. gotcha display ; padding-16 + poids 600 + focus ; garder teinte enrich violette). **Débloqué une fois le fix collision déployé** (le wrap absorbe l'élargissement des CTA → plus de collision). → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
- [ ] **[BLOQUÉ - toi : `git push origin main` (déploie le fix)]** **Fix collision onglets↔CTA tabs-shell Prospection - CODE PRÊT + QA'd + mockup validé Pascal (2026-07-21)**. Bug prod d'origine : onglets par-dessus CTA ≤~1280 sidebar déployée (wrap @768-1023 viewport mais sidebar 240px ≥1024). Fix = wrap **CONTENT-DRIVEN** (`flex-wrap` + `.tabs-bar min-width:max-content` + `.tabs-actions max-width:100%/flex-wrap`), **unconditionnel** (répare la prod pour tous, robuste au nombre d'onglets/boutons). Revue adversariale bug-hunter (1 finding corrigé = débordement horizontal du cluster boutons ; 1 latent documenté = 5 onglets → tab-scroll si simap/regbl réactivés). QA : `tests/_qa-tabs-wrap-collision.mjs`, svelte-check 0/0, Vitest 2863, visuel. Commit local non poussé (règle review avant push). → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
...
- [ ] **[EXÉCUTABLE]** Pousser les commits cohérence UI **d1 `6a19adb` + d2/d3 `8bc4b6e`** (+ docs) locaux sur `main`, **non poussés** — sessions autonomes, règle « review humaine avant push main » respectée. Auto-déploie la prod mais **zéro effet visible** (`ff_ui_coherence` OFF pour tous, OFF byte-identique prouvé). `git push origin main` quand tu valides.
