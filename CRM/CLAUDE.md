...
**Dernière mise à jour :** 2026-07-21 (soir : **CTAs desktop Prospection TENTÉS + REVERTÉS** — la revue adversariale a découvert une **collision responsive prod pré-existante** dans la tabs-shell (onglets↔CTA overlap ≤~1280 sidebar déployée) que tout gain de largeur des CTA aggrave ; increment reverté à la baseline (git propre), bug prod + fix racine parqués `[GATE mockup]`. Leçon QA : ma sonde overflow visait `.tabs-actions` (flex-shrink:0 = vacue) au lieu de `.tabs-bar`. → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]]. Plus tôt : Cohérence UI **d2/d3 LIVRÉ** `8bc4b6e`, `ff_ui_coherence` OFF). **BUG #001 login OTP RÉSOLU (21/07, ultracode)** SANS toucher au code login (git propre - « remets ce qui marchait », directive Pascal) : la vraie cause était « le compte n'existait pas » (pas le code, qui marchait pour les `@filmpro.ch`). Les 2 comptes `@lamaisoncreativedirection.ch` sont désormais **provisionnés** (existants + confirmés + identité email + flags miroir de leur homologue `@filmpro.ch`) → 1er login = code 6 chiffres, plus jamais « Confirm signup ». Reste : le smoke test réel (recevoir le code) = geste Pascal. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
...
**Prochaine attaque** : **BUG #001 login OTP RÉSOLU** (21/07, provisioning pur - zéro changement de code login, git propre, directive Pascal « remets ce qui marchait »). Les 2 comptes `@lamaisoncreativedirection.ch` existent + confirmés + identité email + flags miroir → 1er login = code 6 chiffres (comme les `@filmpro.ch`), plus de « Confirm signup ». **Smoke test réel débloqué** (geste Pascal). Livrable Phase 1 suivant = **CTAs desktop Prospection** (re-appliquer la mécanique validée, débloquée par le fix collision déployé). **Ensuite** : la Phase 1 « figer la base visuelle » avance bien - **d1/d2/d3 + le fix collision tabs-shell sont EN PROD** (poussés 21/07), `ff_ui_coherence`+`ff_page_bandeau` **allumés sur pascal@** (preview de cohérence en vrai). Les **CTAs desktop Prospection** sont **débloqués** (le fix collision absorbe l'élargissement) - mécanique validée à re-appliquer. Reste Phase 1 `[GATE mockup]` (reliquats esthétiques, accueil, import mobile) + redessin KpiStrip/12-col parqué. Puis **Phase 2 = timeline d'interactions**. Contexte : Atelier 209 à **3/7 runs en prod**. → [[project_login_otp_new_user_confirm_signup_2026-07-21]], [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]], [[feedback_qa_micro_mon_ressort_gate_pas_excuse]].
...
- [ ] **[EXÉCUTABLE]** Cohérence UI - **CTAs desktop Prospection** (5, `hidden md:inline-flex`) : re-appliquer la mécanique validée (tentée+revertée 21/07, débloquée depuis que le fix collision est déployé - le wrap absorbe l'élargissement) : override co-localisé `.coh-prosp-cta` (jamais class-swap `.ws-btn` cf. gotcha display ; padding-16 + poids 600 + focus ; garder teinte enrich violette). QA : re-tester le débordement horizontal + collision après élargissement. → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
