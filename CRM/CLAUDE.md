...
**Derniere mise a jour :** 2026-06-23. Prod = alias `filmpro-portail.vercel.app` sur le deploy de `47841d9` (veille trust-by-source round-2 + titre « Filtrer l'essentiel » + W25 régénérée à 3 items, déployés ; refonte CRM listes flag `ffCrmListesV2` toujours OFF). **Derniere revue /optimize :** 2026-04-05. **Prochain bug :** #001.
**Session courante :** 2026-06-23 (session 3) - **Veille TRUST-BY-SOURCE round-2 LIVRÉ + DÉPLOYÉ** (`47841d9`, prod `lut2v4cy1`) : 4 correctifs bloquants fermés via 3 rondes de re-vérif adversariale, W25 régénérée 1→3 items propres, 2054 tests, CI verte. Détail → [[project_veille_trust_by_source_2026-06-23]]. WIP Campagnes 3.2 toujours non commité (intact) → [[project_module_campagnes_vague32_2026-06-22]].
...
**Prochaine attaque** : **Campagnes Vague 3.2** (multi-étiquetage N-N, 3/6 in-flight, WIP non commité dans le working tree, migration N-N à appliquer) → [[project_module_campagnes_vague32_2026-06-22]]. La veille trust-by-source round-2 est **LIVRÉE + DÉPLOYÉE** (`47841d9` main + prod, W25 1→3 items, bloc 0). **W26 (vendredi 26/06) = 1er run cron sur le code round-2** (main à jour) + 1er brief brandé réel à antoine@ : surveiller densité items, mix géo (`mix_drift`), rendu brief. → [[project_veille_trust_by_source_2026-06-23]].
...
### 0. Veille - TRUST-BY-SOURCE + titre (2026-06-23 session 2) : ROUND-2 + W25 + deploy [LIVRÉ]

- [x] ~~**Veille trust-by-source : ROUND-2 → régén W25 → déployer**~~ - **LIVRÉ + DÉPLOYÉ 2026-06-23 (session 3)** (`47841d9` main, prod `lut2v4cy1`). Recalibrage anti-hallu « confiance par fiabilité de source » + titre « Filtrer l'essentiel ». **ROUND-2 = 4 correctifs bloquants fermés via 3 rondes de re-vérif adversariale** (qui ont attrapé 3 bloqueurs invisibles aux tests verts : brèche paywall `<p><a>`, faux strict corps `<div>`, DoS event-loop ReDoS `<[^<>]+>`). Garde paywall = mesure du corps hors ancres + max-sur-scopes ; backstop cabinet = chiffre de marché + proximité (frontières conscientes des accents) ; extraction HTML 100% linéaire (indexOf). Gate déterministe INCHANGÉ ; invariants zéro-hallu re-prouvés. **W25 régénérée 1 → 3 items propres** (RTS romand récupéré + Vitro/glassonweb + Fraunhofer ISE), objectif cadrage ≥3 atteint. 2054 tests, svelte-check 0, build OK, CI main verte, 0 tiret long. Détail → [[project_veille_trust_by_source_2026-06-23]] + [[audit_secu_2026-06-23_veille_trust_by_source]]. Cadrage : `.product-architect/veille/cadrage-sources-fiables-v1.md`.
- **Observations W25 (non-bloquantes, → Lot 3)** : `mix_drift` 33% local (seul RTS romand cette semaine) + `low_volume` 3/8-10 (largeur de couverture, pas la garde anti-hallu). Résiduels LOW assumés tracés (cookies/newsletter `<p>` non-ancrés hors `<aside>`, `<script>` littéral non échappé en prose) → [[project_veille_trust_by_source_2026-06-23]] § résiduels.
- Session 1 (W25/emails/chips/dedash) livrée + déployée prod (`8aa9547`) → [[project_veille_w25_emails_chips_dedash_2026-06-23]].
...
- [x] ~~**Veille trust-by-source round-2 + titre + W25 + deploy (session 3, 23/06)**~~ - LIVRÉ + DÉPLOYÉ (`47841d9` main, prod `lut2v4cy1`) : 4 correctifs bloquants fermés (3 rondes re-vérif adversariale), W25 1→3 items propres, 2054 tests, CI verte → bloc 0 + [[project_veille_trust_by_source_2026-06-23]].
