...
**Prochaine attaque** : **Cohérence UI - bandeau de page increments 1 + 2 + 3 LIVRÉS prod** (9 pages : Entreprises, Contacts, Pipeline, Signaux, Campagnes, Reporting, Aide, Dashboard, Prospection, derrière `ff_page_bandeau` OFF ; `5e0eea3` puis `b4fe644`). **2 décisions Pascal en attente** : **(1) Veille** = mockup produit (`.atelier-209/coherence-ui/veille-bandeau-mockup.html`, bandeau standard + peau magazine conservée) → **à valider dans Chrome avant code** (traitement particulier, identité magazine) ; **(2) Dashboard** = bandeau au-dessus du greeting (hiérarchie OK en vue premium) → option d'**alléger le greeting** si Pascal le veut. Ensuite : increments (b) composants transverses / (c) compteurs / (d) grille 2 niveaux du bandeau, ou **Run 7** veille LED (cadrage en session). Run 4/5 bloqués (comptes Hunter/Pingen chez Pascal). **[à valider Pascal] mots-clés secteur LED** `secteurs.ts` (token « led » trop court, cf. WATCH). Micro sous ma responsabilité validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Détail → `docs/COHERENCE-UI-BANDEAU.md` + [[project_atelier_209_run3_import_liste_2026-07-16]].
...
- [ ] **[EXÉCUTABLE - Veille = valider mockup ; Dashboard = option greeting]** **Cohérence UI globale du CRM** - benchmark **Gouvernance** ([[feedback_gouvernance_benchmark_layout_lisibilite]]) : discipline de structure, **pas la peau** (identité Atelier 209 conservée). **(a) Bandeau : increments 1 + 2 + 3 LIVRÉS prod (9 pages) derrière `ff_page_bandeau` (OFF)** ; **restent (a1) Veille** = valider le mockup `.atelier-209/coherence-ui/veille-bandeau-mockup.html` dans Chrome PUIS coder, **(a2) Dashboard** = alléger le greeting si Pascal le veut. Spec `docs/COHERENCE-UI-BANDEAU.md`. **Restent à mocker** : (b) composants transverses, (c) compteurs agrandis, (d) grille 2 niveaux. `redesign-skill` + `refactoring-ui`.
