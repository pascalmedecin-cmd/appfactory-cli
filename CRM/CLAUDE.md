...
**Dernière mise à jour :** 2026-06-30 (Étiquettes de publipostage **LIVRÉES PROD** `187f2ae`, QA verte ; détail → § Livré). Trunk unique = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`** (Vagues 1+2+3 live). Prod = `filmpro-portail.vercel.app` : **push `main` auto-déploie en production** (vérifié 26/06). **Prochain bug :** #001.
**Session courante :** 2026-06-30 - **Étiquettes de publipostage LIVRÉES PROD** (`187f2ae`, déploiement Vercel Ready, alias prod basculé). Page Campagnes -> volet de sélection des prospects -> PDF planche Avery 6122 (police **Outfit**, jsPDF+svg2pdf doctrine Découpe) + bouton **« Compléter »** par ligne (lookup unitaire search.ch, tout sur la page, aucune navigation). Adresse lue du stock, **zéro API à la génération**. UI premium. QA verte (2365 vitest, build OK, e2e 3/3) + revue 16 agents (sécu 0 finding, 8 med/low/info corrigés). **À FAIRE Pascal** : smoke OTP de la feature. Détail → [[project_etiquettes_publipostage_campagnes_2026-06-30]] + § Livré.
...
- **[WATCH] Étiquettes de publipostage en prod (`187f2ae`, 2026-06-30)** : déploiement Vercel Ready, alias prod basculé, HTTP 303 (auth gate) OK. Reste **smoke OTP** Pascal du flux complet (sélection -> Compléter -> PDF). Surveiller : (a) rendu PDF Avery réel (police Outfit embarquée), (b) bouton « Compléter » = quota search.ch (1000/mois ; le lookup unitaire `search-ch` consomme 1 requête/clic) - pas de batch par design V5, (c) la feature vit sous le flag `ffCrmListesV2` (fondateurs). Rollback = `vercel rollback`. → [[project_etiquettes_publipostage_campagnes_2026-06-30]].
