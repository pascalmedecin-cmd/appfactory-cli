...
**Dernière mise à jour :** 2026-07-17 (cohérence UI - bandeau de page increment 2 : Contacts/Pipeline/Signaux/Campagnes derrière `ff_page_bandeau` OFF, `5e0eea3` déployé prod. Vitest 2824). **Prochain bug :** #001.
...
**Prochaine attaque** : **Cohérence UI - bandeau de page (a) COMPLET : les 10 pages LIVRÉES prod** derrière `ff_page_bandeau` OFF (Entreprises, Contacts, Pipeline, Signaux, Campagnes, Reporting, Aide, Dashboard, Prospection, **Veille**) + **convention kicker/sur-titre de section = 12px** (live, `d51fd31`). Veille = bandeau `flush` + peau magazine conservée (`e60f1b6`). **Il ne reste plus qu'à allumer `ff_page_bandeau`** quand tu veux (tout le CRM prend le bandeau d'un coup) - option d'alléger le greeting Dashboard si tu le veux (sinon laissé tel quel). **Restent à mocker** : bandeau (b) composants transverses, (c) compteurs agrandis, (d) grille 2 niveaux. Sinon **Run 7** veille LED (cadrage en session). Run 4/5 bloqués (comptes Hunter/Pingen chez Pascal). **[à valider Pascal] mots-clés secteur LED** `secteurs.ts` (token « led » trop court, cf. WATCH). Micro sous ma responsabilité validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Détail → `docs/COHERENCE-UI-BANDEAU.md` + [[project_atelier_209_run3_import_liste_2026-07-16]].
...
- [ ] **[EXÉCUTABLE - reste (b)(c)(d) + allumage flag]** **Cohérence UI globale du CRM** - benchmark **Gouvernance** ([[feedback_gouvernance_benchmark_layout_lisibilite]]) : discipline de structure, **pas la peau** (identité Atelier 209 conservée). **(a) Bandeau : COMPLET - les 10 pages LIVRÉES prod derrière `ff_page_bandeau` (OFF)** + convention kicker 12px (live) ; reste à **allumer le flag** quand Pascal veut (option : alléger greeting Dashboard). **Restent à mocker** : (b) composants transverses, (c) compteurs agrandis, (d) grille 2 niveaux. Spec `docs/COHERENCE-UI-BANDEAU.md`. `redesign-skill` + `refactoring-ui`.
...
- [x] ~~**Cohérence UI - bandeau : migration Veille (10e page) + convention kicker 12px**~~ - 2026-07-17 (ultracode). **Veille** = mockup validé Pascal → codé (`e60f1b6`) : masthead magazine remplacé par le bandeau standard (radar, « L'hebdo/Veille », pastille éditions, bouton Éditeur), **peau magazine conservée** ; nouveau prop `PageBand flush` (gouttière 0, Veille porte déjà `px-10`) additif, desktop+mobile. **Convention kicker/sur-titre de section = 12px** (`d51fd31`, live) sur `pband__eyebrow` + `mag-kicker` + label `SourceSelector` (badges exclus). QA : svelte-check 0/0, Vitest 2824, build OK ; Veille live ON = 1 h1 + flush (padding-left 0px) vérifié + eyebrow 12px, OFF = masthead d'origine. **Les 10 pages ont désormais le bandeau** (flag OFF). → `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - bandeau de page increment 3 (Reporting/Aide/Dashboard/Prospection) derrière `ff_page_bandeau` (OFF)**~~ - 2026-07-17 (ultracode). 4 pages à en-tête **bespoke** alignées sur le bandeau standard (décision Pascal « uniformité ») : Reporting (hero→PageBand), Aide (`.aide-head` titre-niveau → « Aide » statique, niveau dans les onglets), Prospection (bandeau en tête du `100dvh`, `shrink-0`, layout préservé), Dashboard (bandeau au-dessus du greeting ; hiérarchie OK en vue premium). QA : svelte-check 0/0, Vitest 2824, build OK, revue adversariale 5 dims (**1 low subjectif** = stacking Dashboard, non bloquant), avant/après vraie vue (4 ON = 1 seul `h1` ; OFF = en-têtes d'origine intacts). `b4fe644` déployé prod. **Veille = mockup produit à valider** (`.atelier-209/coherence-ui/veille-bandeau-mockup.html`). → `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - bandeau de page increment 2 (Contacts/Pipeline/Signaux/Campagnes) derrière `ff_page_bandeau` (OFF)**~~ - 2026-07-17 (ultracode). 4 pages liste calquées sur Entreprises ; `BANDEAU_ROUTES` étendu → le Header gate son titre par la MÊME source unique `isBandeauActive` (jamais de titre double/absent). OFF = rendu actuel strict (branche `{:else}` identique, FAB préservé). QA : svelte-check 0/0, Vitest 2824, build OK, revue adversariale 5 dims **0 finding**, avant/après vraie vue (4 pages ON = 1 seul `h1` + copy/icône/pastille corrects, mobile desc→variante OK ; OFF = bandeau absent + ws-page-actions/FAB présents). `5e0eea3` déployé prod (smoke vert). **Prospection + pages éditoriales NON migrées** (identité forte → décision design). → `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - bandeau de page façon Gouvernance : mockup validé + increment 1 (Entreprises) derrière `ff_page_bandeau` (OFF)**~~ - 2026-07-16 (ultracode). Composant `PageBand.svelte` + source unique `isBandeauActive` (ferme le piège double/zéro-titre, revue bug-hunter) ; OFF = zéro régression prouvée (avant/après vraie vue), Vitest 2824, svelte-check 0/0. Décisions : desc 1 ligne, pastille partout, textes à affiner ; restent increments 2-4. → `docs/COHERENCE-UI-BANDEAU.md` + [[feedback_gouvernance_benchmark_layout_lisibilite]].
- [x] ~~**Page Campagnes : doublon titre/description retiré + bouton réaligné (PROD)**~~ - 2026-07-16 (ultracode). Titre + compteur vivent dans le `<Header>` ; bouton primaire seul à droite (convention refonte). QA avant/après vraie vue, Vitest 2817, svelte-check 0/0. → [[feedback_gouvernance_benchmark_layout_lisibilite]].
...
→ Antérieurs (Run 3 import + Run 2 déployé + Run 1, login OTP 14/07, veille/étiquettes/refonte Lots 1-3/Aide/Découpe/Daily Email) → `archive/claude-md-crm-livre-2026-07-16.md` + `-07-15.md` + `-07-14.md` + `-07-03.md` + `-07-02.md` + `-06-26.md` ; mémoires [[project_atelier_209_run3_import_liste_2026-07-16]], [[project_atelier_209_run2_cablage_2026-07-15]], [[project_login_otp_smtp_supabase_2026-07-14]].
