...
**Derniere mise a jour :** 2026-06-20. Prod = alias `filmpro-portail.vercel.app` sur `810f2e6`. **Derniere revue /optimize :** 2026-04-05. **Prochain bug :** #001.
**Session courante :** 2026-06-20 - **intendance + veille**. Hygiène working tree (124 → 1, commits `407d7b8`+`0eedeab` poussés `main`) + **W25 rattrapée** (re-run `27862437173` success : 2 fix veille prouvés prod, mais édition vide = famine confirmée live → renforce Lot 1/2). → [[project_veille_refonte_qualite_cadrage_2026-06-19]].
...
- **Tests** : Vitest 1869 (dernier run vérifié 2026-06-19) + Playwright e2e (suite + P1/P2/P3 Prospection). Accessibilité : focus trap + ConfirmModal partout, axe-core 0 violation modale P3. Sécurité : Zod sur 20 form actions/endpoints, rate limiting 10/min, headers CSP/XFO/referrer, timing-safe secrets
...
**Prochaine attaque** : Bloc 0 - **Refonte veille éditoriale (Lot 1)** - audit 360 + baseline livrés cette session, décisions tranchées (radar éditorial, gratuit d'abord) ; Lot 1 = persona analyste sénior + so-what exigé + email transformé en vrai brief + bouton prospection réparé, zéro IA en plus, gros gain ressenti. Puis Bloc 1 - **Vague 3.2 Prospection Campagne** (fusionner les 3 filtres dupliqués `prospection-query.ts` puis migration `campagne TEXT`). W25 rattrapée 2026-06-20 (2 fix prouvés prod mais édition vide = famine confirmée live → renforce Lot 1) ; surveiller W26 = 1er run cron naturel.
...
### 0. Refonte veille éditoriale - Lot 1 puis Lot 2 [SUPERVISÉ • xhigh • cascade par lots]

- **Pourquoi** : audit 360 + baseline empirique livrés cette session. Vrai défaut MESURÉ (pas l'hypothèse de l'audit code) : veille famélique ~3 items/édition (cible 8-10), **57 % des candidats rejetés par le filtre**, mix géo inversé (77 % monde vs 2/3 local visé), mono-source glassonweb 55 %. Le « so what » rédigé est correct ; le problème est la tuyauterie aval + l'absence de cadrage éditorial. Décisions Pascal : radar éditorial (pas les AO comme items), gratuit d'abord (pas d'IA en plus).
- **Payload** → [[project_veille_refonte_qualite_cadrage_2026-06-19]] (3 lots détaillés + baseline chiffrée + ventilation des rejets) + métier [[project_filmpro_metier]].
- [ ] **[EXÉCUTABLE]** **Lot 1 - qualité éditoriale (gratuit, prompt + rendu)** : persona analyste sénior ; définir + exiger le « so what » (filmpro_relevance structuré + filet anti-générique déterministe) ; rubrique de décision actionability ; direction éditoriale + few-shot ; email = vrai brief (édito en tête + signaux action_directe + impacts_filmpro, replier le tableau de coûts) ; bouton prospection réparé (chip défaut `simap` cassé 403 → `zefix`) ; page index (remonter le so-what, dé-tronquer l'executive_summary).
- [ ] **[EXÉCUTABLE]** **Lot 2 - pertinence & volume (garde-fous serveur déterministes, gratuit)** : récupérer une partie des 57 % rejetés sans casser le zéro-hallu (retirer patsnap brevets payants, cross-check qui *corrige* le chiffre divergent au lieu de *jeter* l'item, sauver les sources locales espazium/ge.ch) ; garantir le mix romand côté serveur ; alerte de dérive (Low-1 reporté du fix robustesse) ; whitelist 7 tiers effective (optionnel).
- [ ] **[BLOQUÉ - après mesure Lots 1-2]** **Lot 3 - structurel (optionnel)** : mémoire longue + tableau de bord qualité ; boucle de feedback ; PDF de marque partageable (`filmpro-pdf`) ; pont veille→action fin (tâche/opportunité) ; élargir sources (OFS, associations, salons - après mesure densité) ; décomposer le mono-appel LLM (si plafond persiste) ; nettoyage enum RegBL désynchronisé.
...
- [x] ~~**Veille W25 vérifiée + rattrapée**~~ - 2026-06-20. Re-run manuel `27862437173` success = 1er run prod des 2 fix : 0 max_tokens (64K tient), publie au lieu de planter. Édition VIDE (4 candidats rejetés cross-check) = famine baseline → renforce Lot 1/2. → [[project_veille_fix_racine_max_tokens_2026-06-19]].
- [x] ~~**Fix robustesse veille (anti-plantage semaine creuse)**~~ - 2026-06-19 soir (`9c06d54`, poussé `main`). Chips optionnels min(0) + garde ratio non bloquante (échec si 0 valide) + anti-coquille ; zéro-hallu intact (2 tests intégration) ; 1869 Vitest, sécu 0 C/H/M/L + bug-hunter 0 C/H/M. → [[project_veille_fix_robustesse_2026-06-19]] + [[audit_secu_2026-06-19_veille_fix_robustesse_ratio_chips]].
- [x] ~~**Audit 360 + baseline + cadrage refonte veille**~~ - 2026-06-19 soir (read-only). Baseline réelle (~3 items/sem, 57 % rejet filtre, mix inversé 77 % monde, mono-source glassonweb 55 %) ; 3 décisions Pascal + 3 lots ; la baseline a renversé l'hypothèse de l'audit code. → [[project_veille_refonte_qualite_cadrage_2026-06-19]] + [[feedback_baseline_reelle_avant_refonte_pipeline]].
- [x] ~~**Fix racine veille max_tokens**~~ - 2026-06-19 (`5d0d8db`). 64K + relance 128K + cross-check résilient par-item + retry du soir ; xhigh + anti-hallu intacts. → [[project_veille_fix_racine_max_tokens_2026-06-19]].
→ Livrés antérieurs : Vague 3.1 Signaux (`5e0c369`), export CSV Prospection (`6626818`), Vague 2 Prospection premium (`8c5d84d`), Fiches golden (`87a1842`), P1/P2/P3 (`6c4230a`), Pipeline (`35d7df7`) + Signaux premium (`0800b03`), listes/fiches (`60a583c`), Vague 1 (`676a9d4`) → `archive/claude-md-crm-livre-2026-06-19.md`. Plus anciens (Cadrage refonte, Audit 360 + 10 bugs `6b9f6e1`, cron veille, copy V5, Vague 4) → `archive/claude-md-crm-livre-2026-06-18.md` + `archive/2026-06-07-sessions.md`.

### Watch list active après pivot

- **[WATCH] W26 (vendredi 26/06) = 1er run cron naturel des 2 fix** ; W25 déjà rattrapée manuellement 2026-06-20 (run `27862437173` success, reportId `3ef350af`) : 2 fix **prouvés prod** (0 `stop_reason=max_tokens` / 64K tient ; publie au lieu de planter) MAIS édition **vide** (4 candidats rejetés cross-check anti-hallu = famine baseline, pas régression). Surveiller W26 : densité contenu + 1er warn `[report-validate] dérive possible` (canari) + `cost_audit_runs`. `thinking_tokens` loggé seulement sur débordement (non lu, normal). → [[project_veille_fix_racine_max_tokens_2026-06-19]] + [[project_veille_fix_robustesse_2026-06-19]].
