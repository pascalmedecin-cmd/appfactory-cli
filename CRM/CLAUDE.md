...
**Derniere mise a jour :** 2026-06-05 (Découpe Films **LIVRÉ EN PROD** : Phase 5 déployée sur `filmpro-portail.vercel.app`, flag `ff_decoupe` ON pour les 2 fondateurs `@filmpro.ch`, smoke prod export PDF vert. Chantier 2 du portail terminé.)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** 2026-06-05 - Découpe Films **Phase 5 livraison prod CLÔTURÉE** (`vercel deploy --prod` → alias canonique vérifié + flag `ff_decoupe` ON 2 fondateurs + smoke prod PDF 33,2 Ko donut OK). Chantier 2 du portail FilmPro terminé. Détail dans « Livré cette session » ci-dessous.
...
**Prochaine attaque** : **à définir avec Pascal.** Le chantier 2 du portail (Découpe Films) est **livré en prod** ce 2026-06-05 - le portail FilmPro héberge désormais 2 outils (CRM `/crm` + Découpe `/decoupe`). Aucune tâche CRM en cours. Pistes au backlog cockpit (slug `filmpro`/sub `crm`, via `/start`) : mobile V3 terrain (chantier dimensionné, cf. [[project_refonte_mobile_v3_terrain]]) ; durcissement RLS si 4e user non-fondateur ; chantier 3 portail (non cadré).

### 1. Chantier 2 Découpe Films - LIVRÉ EN PROD ✅

- Phase 5 livraison clôturée 2026-06-05 (voir « Livré cette session »). Plus aucune tâche Découpe ouverte. Pack specs archivé : `.product-architect/decoupe/`.

### Livré cette session

- [x] ~~Découpe Films **Phase 5 livraison prod** : deploy + flag ON + smoke prod export PDF~~ - Livré 2026-06-05 (xhigh, SUPERVISÉ, go Pascal). **Deploy** `vercel deploy --prod` depuis la racine repo (build SvelteKit OK, `dpl_uV9Si…` READY target production) ; **alias canonique vérifié** : `filmpro-portail.vercel.app` pointe bien sur le nouveau deploy (trap watch list `rollback`/alias verrouillé évité par `vercel inspect`). **Flag** `ff_decoupe` ON pour les **2 comptes** `@filmpro.ch` prod (`antoine` absent→true, `pascal` false→true ; script `_set-decoupe-flag.mjs` étendu : cible domaine `@filmpro.ch` + mode `--dry-run` lecture seule). Flag lu **en temps réel** côté serveur (getUser) → pas de re-login fondateur requis. **Smoke prod non-invasif** : `GET /decoupe` HTTP 200 sert `Chantiers · Découpe Films` avec session fondateur mintée (≠ redirect home → flag pris en compte). **Smoke prod export PDF** (`scripts/_decoupe_smoke_prod.mjs`, Playwright headless contre la vraie prod, chantier éphémère `[SMOKE-P5]` seedé puis **purgé**) : écran optimisation servi (4 KPI + strip SVG), clic « Exporter en PDF » → **vrai PDF 33,2 Ko `%PDF-` téléchargé** (donut rendu sans rejet svg2pdf). Aucune donnée prod résiduelle. → `memory/feedback_decoupe_tolerance_zero_qa.md` + [[project_portail_filmpro_multi_outils]].
- [x] ~~Découpe Films Phase 4 QA 360 résiduel : RLS réelle (Postgres) + perf authentifiée~~ - Livré 2026-06-05 (xhigh). **RLS** (`scripts/_decoupe_rls_check.mjs`, contre la VRAIE base, pas les mocks Vitest) : RLS activée sur les 3 tables `decoupe_*` ; policies `FOR ALL TO authenticated USING (auth.uid() IS NOT NULL)` (design plat mono-tenant assumé) ; **anon bloqué en lecture ET écriture** (SELECT masqué + INSERT refusé), authentifié autorisé. **Perf** (`scripts/_decoupe_perf_check.mjs`, écran résultat authentifié) : **LCP 932 ms** (cap 2500, en dev → prod plus rapide), **CLS 0**, a11y 100 (axe). Phase 4 QA 360 complète → reste Phase 5 (deploy prod, go Pascal). → `feedback_decoupe_tolerance_zero_qa.md`.
