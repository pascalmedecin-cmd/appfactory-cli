...
**Prochaine attaque** : Bloc 1 - Découpe Films **Phase 3 étape 4 = checkpoint template PDF avec Pascal** (ADR-0005, l'écran d'optimisation est fonctionnel) puis export PDF vectoriel client. Étapes 2 (data layer) + 3 (4 écrans premium + form actions + flag `ffDecoupe` + guard + card portail) livrées 2026-06-05. ⚠️ **Smoke visuel/e2e impossible tant que la migration `decoupe_*` n'est pas appliquée** (tables absentes en prod : un dev server pointe la prod → les load `decoupe_*` lèveraient « relation does not exist »). Validation visuelle = soit une branche Supabase de test (migration appliquée hors prod), soit un golden HTML statique de l'écran d'optimisation, soit la Phase 5 supervisée. À trancher avec Pascal avant l'étape 4.
...
- [x] ~~Étape 3 - 4 écrans `/decoupe` premium + form actions + flag + guard + card portail~~ - Livré 2026-06-05 (voir Livré cette session).
- [ ] **[EXÉCUTABLE]** Étape 4 - **checkpoint template PDF avec Pascal** (ADR-0005, AC-025 ; l'écran d'optimisation est fonctionnel) puis export PDF vectoriel côté client (jsPDF + svg2pdf) à partir du SVG déjà calculé.
...
- [x] ~~Découpe Films Phase 3 étape 3 : 4 écrans premium homogènes CRM + form actions (UI complète)~~ - Livré 2026-06-05 (xhigh, redesign-skill, commits `e496d95` + `ac77969`). **Fondation** : flag `ffDecoupe` (`feature-flags.ts`), card portail activable, route `/decoupe` (guard par flag → redirect `/` si OFF, shell dédié `PortailHeader` + nav outil + Toast), `src/lib/styles/decoupe.css` (primitives partagées). **Écrans** : (1) liste chantiers (DataTable + nb vitres + création), (2) base produit CRUD + archive/restore soft-delete (modal riche : familles, laizes CSV, marge/recouvrement, attributs nesting), (3) fiche chantier + saisie vitres rapide en ligne (focus conservé) + édition/suppression, (4) **optimisation** = cœur visuel : `optimiserDecoupe` côté serveur → `PlanDecoupeSvg` (bande laize×longueur, pièces colorées par vitre, repères 1 m) + `ChuteGauge` (taux de chute, seuils) + légende + **table de repli accessible** (AC-017) + liste commande fournisseur + alertes + consolidation suggérée (AC-014) + « Lancer » (statut→lancee). Form actions branchées sur les builders étape 2. **svelte-check 0, Vitest 1560/1560, build prod OK.** Reste : PDF (étape 4, checkpoint Pascal) + smoke visuel/QA (Phase 4, requiert migration). → voir `.product-architect/decoupe/DESIGN.md`.
- [x] ~~Découpe Films Phase 3 étape 2 : data layer (migration + types + Zod + builders, TDD)~~ - Livré 2026-06-05 (xhigh). Migration locale `supabase/migrations/20260605_001_decoupe_films.sql` (3 tables `decoupe_produits`/`decoupe_chantiers`/`decoupe_vitres` + RLS mono-tenant plat `decoupe_*_all` via `auth.uid() IS NOT NULL`, idempotente, **NON appliquée en prod** = Phase 5 supervisée) ; `database.types.ts` étendu manuellement (3 tables) ; schemas Zod dans `$lib/schemas` (`DecoupeProduit/Chantier/Vitre` Create/Update + archive/delete + FIELDS, **Zod v4** `error:` pas `invalid_type_error`, couvre AC-019) ; couche d'écriture pure `src/lib/server/decoupe/builders.ts` (insert/update/archive/lancement ; `created_by`+`updated_at` sur produits/chantiers, vitres sans ; vides → null). **24 tests** (`builders.test.ts` : validation + builders). Vitest 1560/1560, svelte-check 0. Form actions glissées en étape 3 (indissociables de leur page SvelteKit, accord Pascal). → voir `.product-architect/decoupe/data-model.sql` + `rls-policies.sql`.
