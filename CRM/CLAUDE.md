...
**Prochaine attaque** : **#1 = Refonte page Aide CRM** (cadrage validé S180, spec écrite — reprise immédiate). La tâche Google Places (anciennement #2) est **LIVRÉE S181** (2026-05-12, voir § Livré + spec `notes/google-places-2026-05-12/spec.md`) — reste 1 action ops hors-code : ajouter `GOOGLE_PLACES_API_KEY` dans les variables Vercel prod (`filmpro-crm`) — cf. tâche #2 ci-dessous.

### 1. Refonte page Aide CRM from scratch [SUPERVISÉ • xhigh • session dédiée]

**Pourquoi** : Tâche #4 livraison client. 3 niveaux : *Prise en main* (cœur onboarding : parcours 5 min illustré + checklist de démarrage interactive + carte mentale SVG des écrans + callouts) / *Fonctions détaillées* (9 fiches écran + diagrammes de flux SVG : cycle pipeline, déroulé veille, scoring) / *Doc technique + runbook* (archi/stack/BDD-RLS/auth OTP/APIs/crons/déploiement/sécurité + runbook ops 4 procédures). Pascal veut une section aide **riche, pédagogique, visuellement attrayante** (onboarding outil), conforme GOLDEN v9 (registre sobre FilmPro, tokens, SVG sur-mesure, zéro emoji/gradient/dashed). H-28 (/aide hors charte 1443 lignes) et M-29 (getElementById /aide) résorbés par cette refonte.
**État** : cadrage validé avec Pascal le 2026-05-12 (S180). **Spec écrite** : `notes/refonte-aide-2026-05-12/spec.md` (objectif, 6 arbitrages tranchés, contenu détaillé 3 niveaux, archi technique `src/lib/aide/{content.ts,search.ts}` + composants `AideBlock/AideDiagram/AideChecklist`, 10 critères d'acceptation binaires, hors-scope nommé). **Arbitrages clés** : data-driven (pas de HTML monolithe) ; recherche full-text client ; layout 3 onglets (primitive `Tabs.svelte`) + docs (TOC sticky / contenu / « sur cette page ») ; **diagrammes SVG sur-mesure, ZÉRO capture d'écran V1** ; runbook ops inclus niveau 3. **Hors-scope (tâches suivantes)** : tour guidé interactif (tooltips séquentiels), bouton « ? » contextuel sur les 9 pages, push première-connexion vers /aide. **Aucune ligne de code écrite** (seuls la spec + dossiers vides `src/lib/aide/` et `src/lib/components/aide/` créés — non trackés Git car vides).
**Prérequis** : ✓ aucun — cascade audit 360 entièrement fermée. Fichier `/aide/+page.svelte` indépendant.
**Skills** : refactoring-ui + golden-standard + frontend-design + ux-guide + taste-skill (globaux) ; GOLDEN_STANDARD.md comme contrainte dure. **Subagents** : security-auditor + test-coverage-reviewer (DoD).

- [ ] **[EXÉCUTABLE]** Reprendre la refonte page Aide CRM 3 niveaux → voir `notes/refonte-aide-2026-05-12/spec.md` (spec figée) + GOLDEN_STANDARD.md + `CRM/CLAUDE.md § DECISIONS STRUCTURELLES > Doctrine styling`

### 2. Google Places prod : ajouter la clé Vercel [OPS • trivial • 5 min]

**Pourquoi** : la source Google Places est livrée et fonctionnelle (code + migrations prod appliquées S181), mais l'import réel en prod renverra « clé non configurée » tant que la variable d'environnement n'est pas posée côté Vercel. La clé existe (`.env.local` local, projet GCP Pascal).

- [ ] **[EXÉCUTABLE]** Ajouter `GOOGLE_PLACES_API_KEY` dans les variables d'environnement Vercel du projet `filmpro-crm` (Production + Preview), même valeur que `.env.local` → puis redéployer (ou attendre le prochain deploy auto).
