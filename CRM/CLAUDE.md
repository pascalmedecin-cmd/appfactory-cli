...
**Derniere mise a jour :** 2026-06-25. Prod web = `filmpro-portail.vercel.app` = branche d'intégration **éditeur veille + Campagnes V3.2** (déployée par-dessus l'éditeur veille `69cd968`, flag `ffCrmListesV2` OFF). Veille cron sur `main` (`9bef41b`, W26) **intact, non touché** (trunk préservé pour le run de vendredi). **Derniere revue /optimize :** 2026-04-05. **Prochain bug :** #001.
...
**Prochaine attaque** : après le déploiement Campagnes (Option A, en cours) - (a) **reconcilier `main` post-W26** (vendredi 26/06 : merger éditeur veille + Campagnes → trunk, le cron prend alors le sourcing 238) ; (b) **Vague 3.3 Dashboard temporel** (golden validé). Campagnes V3.2 déployée web prod (flag OFF), éditeur veille live. **W26 (vendredi 26/06) = 1er run cron sur le sourcing `9bef41b` + 1er brief réel antoine@**, à surveiller (densité, part locale, 0 `<cite>`/underscore). → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]].
...
- [x] ~~**Vague 3.2 Module Campagnes (multi-étiquetage N-N) - UI COMPLÈTE 6/6 + DÉPLOYÉE**~~ - LIVRÉ 2026-06-24 (`c905952`), **DÉPLOYÉ web prod 2026-06-25** (Option A, par-dessus l'éditeur veille, flag OFF). Liste (colonne multi-pastilles + filtre relationnel) + `CampagneCombo` réutilisable accessible (combobox ARIA + clavier ; fiche + import) + écran dédié `/crm/campagnes` CRUD + sidebar gatée + export colonne + **gate premium serveur** (defense-in-depth). Revue adversariale 5 lentilles : 11 findings **corrigés + testés**. RESTE = reconciliation `main` post-W26 (vendredi). → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]].
