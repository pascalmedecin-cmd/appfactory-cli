...
- [x] ~~**[SIGNAL]** `<Icon name>` littéraux/feeders non mappés dans ICON_MAP (rendaient un « ? » via `CircleHelp` en prod)~~ - **RÉSOLU 2026-06-26** : **10 noms** mappés (sweep indépendant = **+3 vs les 8 signalés** : `domain`, `monitor_heart`, `place`, via feeders `icon:`) + **test lint anti-régression** `src/lib/components/icon-map.test.ts` (3 voies : `name="x"`, branches de ternaire, props `icon:`), red-green prouvé, aurait attrapé `picture_as_pdf`. → § Livré.

### Livré cette session

- [x] ~~**Fix icônes « ? » en prod + garde anti-régression ICON_MAP**~~ - 2026-06-26 (ultracode/xhigh) : **10 noms Material Symbols** utilisés dans l'UI mais absents d'ICON_MAP (rendaient un « ? » via `FALLBACK_ICON`) mappés dans `icon-map.ts` (`bug_report`→Bug, `desktop_windows`→Monitor, `domain`→Building2, `event`→CalendarDays, `inbox`→Inbox, `inventory_2`→Archive, `monitor_heart`→Activity, `open_in_full`→Maximize2, `phone`→Phone, `place`→MapPin). Sweep indépendant (workflow) = **+3 vs le SIGNAL** (8→10, feeders `icon:`) ; vérif adversariale par-icône (1 réfutation retenue : `inventory_2` Package→**Archive** sur usage réel « Archivées »). **Test lint** `icon-map.test.ts` (littéraux + ternaires + feeders, red-green prouvé). 2324 tests verts, svelte-check 0/0, build OK. **Pas encore poussé** (gate review/deploy Pascal). → [[project_veille_bloc_c_pont_pdf_2026-06-26]].
