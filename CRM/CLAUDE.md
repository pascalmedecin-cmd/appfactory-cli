...
- **État : Run 1 ET Run 2 DÉPLOYÉS EN PROD (2026-07-15).** Run 2 = cloisonnement bi-marque (sélecteur, chrome teinté LED, étanchéité DB) : gate visuel Pascal franchi (logo LED sidemenu « flou » régénéré HD au passage), migration `marque` appliquée + vérifiée en prod (non-régression prouvée : 100 % des lignes existantes en `filmpro`), mergé `main` (`48d0e66`), smoke prod vert. LED reste **vide** jusqu'au Run 3. **Prochain : Run 3** (import de listes LED, sources par marque, source unique secteurs). V6 Hunter / V7 Pingen en attente comptes Pascal.
...
- [x] ~~**Atelier 209 - Run 2 DÉPLOYÉ EN PROD + logo LED corrigé HD**~~ - 2026-07-15 (ultracode). Gate visuel franchi : Pascal a validé les 2 marques dans Chrome (preview locale premium, base jetable Colima seed 2 marques, session mintée `ff_crm_listes_v2`). **Logo LED sidemenu « flou » → cause racine trouvée** (mot « studio » sous-dimensionné à 22px, pas un raster) **et régénéré HD** depuis la vraie police Poppins Bold/SemiBold (fidèle au logo officiel), validé en rendu réel (capture retina 22px + 96px). **Migration `marque` appliquée + vérifiée en prod** (`scripts/apply-marque-migration.mjs`, transaction unique : 12 colonnes + 12 CHECK + 2 clés composites + 3 FK cohérence + fonctions réécrites) ; **non-régression prouvée** = 100 % des lignes existantes en `filmpro` (prospect_leads 312, 0 ligne non-filmpro). Mergé `main` (`48d0e66`), Vercel Ready, **smoke prod vert** (login 200 « Atelier 209 », logo HD servi viewBox 895, `/api/marque` 303). LED vide → Run 3. → [[project_atelier_209_run2_cablage_2026-07-15]] + `docs/ATELIER-209-SUIVI.md`.
