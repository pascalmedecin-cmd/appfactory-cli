...
- **[WATCH] Smoke OTP Pascal (06-30/07-01)** : vérifier en prod le workflow prospect Lot 2 (`aff32cd`), Signaux (`db6182e`), Étiquettes (`e263240`), Centre d'aide (`d402fe7`), Découpe (`0af198d`, `ff_decoupe`), **Campagnes Lot 3** (`/crm/campagnes` : onglet « Ouvertes », statut inline En cours/Active, « Trouver des prospects » → modal pré-coché ; prospection : badge fusée « lancée » sur pastilles actives). Rollback = flag OFF ou `vercel rollback` (migrations idempotentes ; colonne statut peut rester). Lot 1 : colonne dormante `opportunite_associee_id`+FK laissées. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
