...
- **[WATCH] Refonte CRM 1+2+3 activée fondateurs (flag `ffCrmListesV2`)** : surfaces premium (cartes Pipeline, lignes Entreprises/Prospection, Dashboard temporel) pas re-validées une à une → surveiller à l'usage. **Bug 5 résiduel** : menu `...` du slideout Signaux (ouverture vers le haut en conteneur scrollable), à corriger s'il re-saute. Rollback instantané = flag OFF (`raw_app_meta_data`).
- **[WATCH] W26 (vendredi 26/06) = 1er run cron sourcing `9bef41b` (~120 sources) + 1er brief brandé réel antoine@**. Surveiller GHA : `keptByTrust`, densité (cible 8-12, W25=2), part locale, 0 `<cite>`/emoji. **NB : cron sur `main` → N'utilise PAS les 238 sources (web-only sur `deploy-campagnes-editeur`) ; passage à 238 = reconciliation Bloc 0.** → [[project_veille_sourcing_w26_2026-06-23]].
