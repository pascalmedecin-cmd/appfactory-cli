...
**Statut :** Phase C. **REFONTE PAGE SIGNAUX V4 LIVRÉE PROD (scoring sans temporalité + drawer mots-clés overlay + ScorePill saturée gradient + bandeau dominante 3px par card + cards line-clamp 4 + tab underline 3px) + FIX SSR 500 onDestroy→$effect** (S189, 2026-05-13) : suppression intégrale du composant temporel scoring (`scoring.recence` + bloc Récence dans `calculerScore` + tests adaptés + script `rescore_signaux_v2.mjs` aligné), `maxPoints` 12→10, seuils `scoreLabel` 10/7/4→7/4/1 alignés sur `config.scoring.labels`. UX : tab `Tous` retiré, défaut `Nouveau` (inbox métier), sidebar pertinence sticky → drawer overlay 440px droit avec backdrop + Échap, bouton trigger `Mots-clés` toolbar + badge count, toggle « Cacher les hors-scope » avec compteur dynamique `(N masqués)` warning. UI premium : ScorePill premium en classes globales `.signal-score-pill--{chaud,tiede,froid,unscored}` (gradient 135deg + ring inset hairline + drop shadow + tabular-nums) ; bandeau 3px coloré en top de card via `data-dominant=coeur|bonus|eviter|neutral` issue de nouvelle fonction `dominantKeywordCategory` (priorité Cœur > Bonus > Éviter) ; cards `line-clamp 2→4` + padding 16→20 + hover translateY(-3px) + shadow-card-hover ; sort-btn h-28 box-border + ring-primary actif ; tab underline 2→3px primary cross-page CRM. **Bug SSR identifié+corrigé** : `onDestroy(() => window.removeEventListener(...))` plante en SSR Svelte 5 car onDestroy s'exécute aussi côté serveur (cleanup post-render). Capture stack en cycle promote/Chrome MCP/rollback. Fix : `$effect` cleanup callback (client-only). Vite preview ne reproduisait pas (polyfill SSR différent d'adapter-vercel). Mémoire gravée `feedback_svelte5_ondestroy_ssr_window_undefined.md`. **Tests + audits** : 1346/1346 Vitest verts (+14 dominantKeywordCategory + scoreLabel/scoreStyle V4), svelte-check 0 erreur, audit security 0 C/H/M/L (OWASP 11/11) + 2 Info watchlist (memoization highlight + maxlength search), audit contracts 0 C/H + 1 Medium fixé in-session (collision CSS `.score-pill--*` prospection → renommé `.signal-score-pill--*`). **Deploy** : commits `e02bd88` (V4 initiale buggée SSR) + `a6c5440` (fix). Rollback transitoire vers S188 (`filmpro-izyf135qr`, ~5 min) puis re-promote explicite V4-fix après push (l'alias prod se verrouille post-rollback, push suivants buildent sans promouvoir auto). **Rescore prod** : 117 signaux mis à jour V4 via `node scripts/rescore_signaux_v2.mjs --apply` (SIMAP standard `4→2`, top Cœur Vitrage `9→7`). **Smoke prod 12 critères Chrome MCP** : 11/12 ✓ (C4 Zefix `creation_entreprise` non testable aujourd'hui car 0 Zefix actif en BDD — cron Zefix v2 attendu 14/05 matin, validé indirect via rescore prod `4→2` = perte du +2 récence). Spec 12 critères : `notes/refonte-signaux-v4-2026-05-13/spec.md`. **Antérieur** : Refonte Signaux V3 S188 + cron Zefix S187, Page Log CRM S185+S186, refonte page Aide S182+S183, cascade audit 360 fermée S178→S180 (136 findings, 134 fixés), API Google Places S181, cascade golden v9 6/6 (S175-S176ter), migration restructure Voie A+B (S173-S174). Formation IA = sous-projet autonome dans `Formation/`, `cc` option 5.
**Derniere mise a jour :** 2026-05-13 (S189 : Refonte Signaux V4 + fix SSR onDestroy→$effect + rescore 117 signaux)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** S189 livrée (xhigh, ~3h chat + ~30 min diag bug SSR + ~10 min rescore/smoke). Détails dans § Livré ci-dessous.
