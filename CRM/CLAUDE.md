...
**Dernière mise à jour :** 2026-07-17 (cohérence UI - bandeau de page COMPLET : les 10 pages derrière `ff_page_bandeau` OFF + convention kicker 12px live, `e60f1b6`/`d51fd31` déployés prod. Vitest 2824). **Prochain bug :** #001.
...
**Prochaine attaque** : **Cohérence UI - implémenter l'increment b (composants transverses)**. Direction b/c/d **validée par Pascal le 17/07** (Q1 = largeur bornée **ancrée à gauche**) ; normes + mockup → `docs/COHERENCE-UI-BANDEAU.md`. Coder derrière un flag, increment par increment (b→c→d), page par page, QA avant/après - **attention aux espaces** (rythme 8px, critère dur). Sinon : allumer le flag bandeau (a), ou **Run 7** veille LED (cadrage). Run 4/5 bloqués (Hunter/Pingen chez Pascal). **[à valider]** mots-clés secteur LED `secteurs.ts` (token « led » trop court, cf. WATCH). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[EXÉCUTABLE]** Cohérence UI b/c/d - implémenter : direction validée 17/07 (mockup + normes gravées), coder increment par increment derrière un flag. Commencer par **b** (bouton, pastilles, recherche, surfaces, état vide → briques partagées), puis **c** (compteurs, 3 rôles + global/filtré), puis **d** (grille : gouttière unique, largeur ancrée à gauche, 4 zones, rythme 8px). Page par page, QA avant/après, zéro régression, discipline structure (pas la peau). `redesign-skill` + `refactoring-ui`. → `docs/COHERENCE-UI-BANDEAU.md` + [[feedback_gouvernance_benchmark_layout_lisibilite]].
