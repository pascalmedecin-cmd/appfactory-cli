...
**Dernière mise à jour :** 2026-07-02 (**Campagnes : panneau prospects in-page + fix étiquetage import** `c7efdd4` - plus d'A/R obligé vers la Prospection ; étiquetage jamais silencieux (ré-import rattache les déjà-présents, warning explicite) ; 16 leads prod réparés → § Livré + [[project_campagnes_panneau_prospects_fix_etiquetage_2026-07-02]]. Refonte 3 lots livrée la veille : [[project_refonte_signaux_prospects_campagnes_2026-07-01]]). Trunk = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : smoke OTP (workflow prospect + Campagnes : panneau prospects + 16 leads Mailing Commerces). **Prochain bug :** #001.
...
- **[WATCH] Smoke OTP Pascal (06-30→07-02)** : vérifier en prod le workflow prospect Lot 2 (`aff32cd`), Signaux (`db6182e`), Étiquettes (`e263240`), Centre d'aide (`d402fe7`), Découpe (`0af198d`, `ff_decoupe`), **Campagnes Lot 3 + panneau prospects (`c7efdd4`)** (`/crm/campagnes` : clic sur « N prospects » ou le nom → panneau latéral sans navigation ; « Mailing Commerces - Vernis solaire » doit afficher ses **16 prospects réparés** ; ré-import d'un lot déjà présent sous une campagne → message « rattachées à la campagne »). Rollback = flag OFF ou `vercel rollback`. Lot 1 : colonne dormante `opportunite_associee_id`+FK laissées. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]] + [[project_campagnes_panneau_prospects_fix_etiquetage_2026-07-02]].
