...
- **État : Run 1 ET Run 2 DÉPLOYÉS EN PROD (2026-07-15).** Run 2 = cloisonnement bi-marque (sélecteur, chrome teinté LED, étanchéité DB), non-régression prouvée en prod ; LED vide jusqu'au Run 3. **Prochain : Run 3** (import listes LED, sources par marque, source unique secteurs). V6 Hunter / V7 Pingen en attente comptes Pascal. Détail → `docs/ATELIER-209-SUIVI.md`.
...
**Dernière mise à jour :** 2026-07-15 (Run 2 DÉPLOYÉ prod : cloisonnement bi-marque + logo LED HD + migration marque, non-régression prouvée). **Prochain bug :** #001.
...
**Prochaine attaque** : **Atelier 209 - Run 3** (les prospects LED entrent). Run 2 est **en prod** (cloisonnement bi-marque déployé 2026-07-15, LED vide). Run 3 = écran d'import de liste (remplace la dette D2), sources re-paramétrées par marque, **source unique des mots-clés secteur** (D3), valeur source `manuel` (D4). **Gate design d'abord** : maquette d'import validée dans Chrome par Pascal avant toute ligne de code (règle non négociable). Détail → `docs/ATELIER-209-SUIVI.md` (Run 3) + dettes D2/D3/D4.
...
- [x] ~~**Atelier 209 - Run 2 DÉPLOYÉ EN PROD + logo LED corrigé HD**~~ - 2026-07-15 (ultracode). Gate visuel franchi (Pascal a validé les 2 marques dans Chrome via preview locale premium base jetable Colima). **Logo LED sidemenu « flou »** : cause racine = « studio » sous-dimensionné à 22px (pas un raster), **régénéré HD** depuis Poppins Bold/SemiBold (fidèle au logo officiel), validé en rendu réel. **Migration `marque` appliquée + vérifiée en prod** (`scripts/apply-marque-migration.mjs` : 12 colonnes + CHECK + FK composites + fonctions ; **non-régression prouvée** = 100 % des lignes en `filmpro`, 0 fuite). Mergé `main` (`48d0e66`), Vercel Ready, **smoke prod vert**. LED vide → Run 3. → [[project_atelier_209_run2_cablage_2026-07-15]] + `docs/ATELIER-209-SUIVI.md`.
- [x] ~~**Login Atelier 209 : 6 défauts design micro corrigés + process QA design micro établi**~~ - 2026-07-15 (`run2-marque`, redesign-skill). Pascal a rattrapé 2 défauts sur la preview → **feedback de posture gravé** : le micro (grille/états/feedback) est mon ressort, pas ses checks ([[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Fixes mesurés (boutons delta **0px**, bannière « code envoyé », layout-shift réglé, placeholder `123456`, contraste AA). Revue agents 6 réels / 8 réfutés. svelte-check 0/0, build OK.
- [x] ~~**Fix gap avatar→raison sociale sur Entreprises**~~ - 2026-07-15. Cause racine pré-existante (`table-fixed`) : avatar + nom fusionnés dans une cellule (gap 8px), colonne logo retirée. QA avant/après, 31/31. Isolé à Entreprises.
→ Antérieurs : **étapes Run 2** (câblage bi-marque [[project_atelier_209_run2_cablage_2026-07-15]] + QA non-régression 22 agents [[project_atelier_209_run2_qa_non_regression_2026-07-15]] + [[audit_secu_2026-07-15_atelier209_run2_marque]] + fondation) ; Run 1 déployé 15/07 → `archive/claude-md-crm-livre-2026-07-15.md` ; login OTP 14/07 [[project_login_otp_smtp_supabase_2026-07-14]] ; veille 10/07, étiquettes PDF, refonte Lots 1-3, Aide, Découpe, Daily Email → `archive/claude-md-crm-livre-2026-07-14.md` + `-07-03.md` + `-07-02.md` + `-06-26.md`.
