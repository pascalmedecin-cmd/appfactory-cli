...
**Dernière mise à jour :** 2026-07-01 (**Refonte CRM 3 lots** - workflow recherche->prospection->pipeline->entreprise, signaux détachés : **Lot 1 Signaux** `db6182e` + **Lot 2 Prospects+Pipeline** `aff32cd` **LIVRÉS PROD** ; Lot 3 Campagnes = prochaine session ; détail → § Livré + [[project_refonte_signaux_prospects_campagnes_2026-07-01]]). Trunk = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : smoke OTP nouveau workflow prospect. **Prochain bug :** #001.
