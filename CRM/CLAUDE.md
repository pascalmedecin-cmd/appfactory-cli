...
- **Skills design (runs 1+, annoncés au plan)** : `redesign-skill`, `soft-skill`, `theme-factory`, filtre `ANTI-AI-SLOP.md`. **État : Run 1 déployé prod ; Run 2 CÂBLAGE COMPLET + testé sur `run2-marque` (NON déployé)** - cloisonnement de bout en bout, leak-test 10/10 base réelle, Vitest 2562, audit sécu 0 C/H, revue adversariale 18 agents. V6 Hunter / V7 Pingen en attente comptes Pascal. **Reste 2 gestes Pascal (gate prod)** : (1) validation visuelle Chrome des 2 marques, (2) go migration prod (`pg`) + déploiement - voir suivi.
