...
**Derniere mise a jour :** 2026-06-07 (Vague 4d-mojibake cause racine encodage Zefix + Vague 4c a11y structurels + Vague 4b palette `-deep`/radius, branche `portail-session-1`, NON déployé prod [checkpoint Pascal]. Avant : Vague 4a i18n + a11y localisé `fa0c728`. EN PROD : REG-01 suppression entreprise `dd99ae8` [FK lead `SET NULL` + modale dépendances + cascade terrain chiffré I-2, sécu 0 C/H/M/L, smoke 4/4] ; Vague 2 a11y, Vague 1 nav `c51309c`, V5 `364bd1f`).
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** 2026-06-07 - **Vagues 4b + 4c + 4d-mojibake LIVRÉES** (branche `portail-session-1`, non déployé prod, checkpoint Pascal). 4b : 6 tokens `-deep` AA + 13 usages texte migrés (icônes vives) + gate axe étendu + test déterministe ; radius carte 12→10 + ModalForm 16→12 (portail) ; champs 10→8 descopé. 4c a11y structurels : `<th>` logo nommé, Log 1 h1 + `<tr>`→bouton, item Veille `<article>`/`<time>`, combobox Contacts, empty contextuel Entreprises, Kanban `role=list` conditionnel ; M6 Veille h1 accepté en l'état. 4d-mojibake : diagnostic = ingestion Zefix (`resp.json()` force UTF-8 sur SOGC Latin-1 → U+FFFD lossy), pas la migration ; **décision Pascal : pas de backfill (1227 archivées) + fix cause racine** (`lib/server/decode-response.ts` UTF-8+fallback Windows-1252, 2 points ingestion). 1710 Vitest, svelte-check 0, build 0. Gate Playwright non lancé (re-audit seedé). Reste : re-audit seedé + 4d-copy bloqué. Détail « Livré cette session ».
...
- [x] ~~**Vague 4d (volet mojibake) - cause racine encodage Zefix**~~ - Livré 2026-06-07 (xhigh, branche `portail-session-1`, **NON déployé prod**). **Diagnostic** (MCP Supabase read-only) : le mojibake n'est PAS la migration V5 (simple `UPDATE statut='archive'`, ne touche aucun texte) mais l'**ingestion Zefix** — `importZefix` faisait `resp.json()` qui force UTF-8, or SOGC renvoie du Windows-1252/Latin-1 → chaque accent détruit en **U+FFFD avant stockage** (« Rôtisserie »→« R�tisserie », `R 52 / EF BF BD / tisserie`). **Irréversible depuis la base** : octet d'origine perdu. Ampleur : 1227 archivées (632 `maitre_ouvrage`, 527 `description_projet`, 350 `commune`). **Décision Pascal : pas de backfill** (fiches archivées/masquées + Zefix déprioritisé V5 ; UID CHE préservés si re-fetch un jour) **+ fix cause racine**. Fix : helper `lib/server/decode-response.ts` (`decodeResponseText`/`parseJsonResilient` : UTF-8 puis fallback Windows-1252 sur U+FFFD) appliqué aux 2 points d'ingestion Zefix (cron SOGC + recherche manuelle) → toute réactivation future ne re-corrompt plus. Test déterministe `decode-response.test.ts` (6 cas, octets construits, sans réseau) + mocks cron rendus fidèles (vraie `Response` avec `arrayBuffer`). 1710 Vitest (+6), svelte-check 0, build 0. → `docs/QA_FINDINGS_CRM_2026-06-07.md` §E (LIVE-M3).
- [x] ~~**Vague 4c - a11y structurels audit live**~~ - Livré 2026-06-07 (xhigh, branche `portail-session-1`, **NON déployé prod** - checkpoint Pascal). 6 findings : `<th>` logo non-triable nommé (`DataTable.svelte` `aria-label={col.label ? undefined : col.srLabel}`, partagé) ; 2 `<h1>` Log → page h1=h2 (M6 Log, le shell Header porte le h1) ; `<tr>` cliquable Log → **bouton focusable** `aria-expanded`/`aria-controls` (M9, clic souris conservé via `stopPropagation`, id sur détail) ; item Veille `<article>` + `<time datetime>` (M7) ; autocomplete Contacts `role=combobox`/`listbox`/`option` + `aria-selected` (M4, hint « sera créée » sorti du listbox pour ne pas violer `aria-required-children`) ; empty Entreprises contextuel recherche (M1) ; Kanban colonne vide ne porte plus `role=list` (cause `aria-required-children` sur DB quasi-vide). **M6 partie Veille (h1 générique sous-pages) = accepté en l'état** (décision Pascal : hiérarchie h1 section + h2 contenu valide WCAG ; pas d'override h1 global = risque disproportionné). svelte-check 0 erreur (0 nouveau warning), build 0, 1704 Vitest. **Gate Playwright `vague2-a11y` non lancé ce tour** (serveur + session mintée ; findings majoritairement en modale/détail hors scan chargement) → `aria-required-children` enforcement (retrait de `OUT_OF_SCOPE_RULES`) **basculé au re-audit seedé**. → `docs/QA_FINDINGS_CRM_2026-06-07.md` §E/E.2.
- [x] ~~**Vague 4b - palette prospection `-deep` + radius design system**~~ - Livré 2026-06-07 (xhigh, branche `portail-session-1`, **NON déployé prod** - checkpoint Pascal). Palette : 6 tokens `-deep` (`app.css`, valeurs revérifiées WCAG ≥ 4.5 sur `-bg` ET blanc) + **13 usages TEXTE migrés** (badges catégories Veille ×2 pages, chips + bouton Prospection, AlerteModal label/chip/×, EnrichBatchModal %, BatchActionsBar) ; **icônes laissées vives** (SVG, seuil 3:1, jamais flaggées axe). Gate axe étendu (`vague2-a11y.test.ts` STATUS_FG + palette, collision `#5a7190` tracée) + **test déterministe** `src/lib/palette-contrast.test.ts` (8 tests, lit `app.css`, prouve AA hors rendu). **Écart payload tracé** : seules 4/6 teintes servaient de texte (import/enrich/qualify/convert) ; `place` + `tab-regbl` `-deep` ajoutés mais latents (aucun usage texte). Radius : carte signal 12→10 (`SignauxCards`, scoped CRM) + ModalForm 16→12 (`rounded-2xl`→`xl`, **portail entier CRM+Découpe**, décision Pascal). **Champs 10→8 descopé** (décision Pascal) : champs et boutons partagent `rounded-lg` → migrer les champs seuls casserait la cohérence, churn 150 boutons non justifié pour 2px ; 10px reste sur l'échelle. 1704 Vitest (+8), svelte-check 0, build 0, classes `-deep` confirmées dans le CSS buildé. Reste Vague 4 (4c structurels / 4d mojibake) + re-audit seedé. → voir `memory/project_vague4_palette_deep_2026-06-07.md`.
