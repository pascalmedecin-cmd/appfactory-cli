...
**Prochaine attaque** : **Bloc D - Audit UI 360** (SUPERVISÉ, **porte de clôture** finale, à faire avec Pascal dans la boucle) + **Bloc B - Daily Email** (gated Pascal : (1) **smoke auth OTP** car le deploy prod embarque supabase 2.108 + TS6 non testés en prod, (2) **feu vert activation**). **Séquence de clôture refonte CRM** : ~~A~~ → B (gated Pascal) → ~~C (faisables maintenant)~~ → **D (audit UI 360 = porte de clôture)**. **Bloc A LIVRÉ `f357e68`** (mails coupés). **Bloc C partie « faisables maintenant » LIVRÉE `b767056`** (fix pont/RegBL cul-de-sac + PDF de marque + NO-OP décompo LLM ; reste C = dashboard qualité/feedback, **attend W27**). Refonte Vagues 1+2+3 live (flag `ffCrmListesV2` ON fondateurs), prod = trunk `main` (deploy manuel gated Pascal). → [[project_refonte_crm_cadrage_2026-06-18]] + [[project_veille_bloc_c_pont_pdf_2026-06-26]].
...
- [ ] **[BLOQUÉ - smoke auth OTP supabase 2.108 par Pascal d'abord]** **Déployer** = `vercel deploy --prod` (enregistre le cron `daily-email` en env Vercel - no-op tant que gate OFF, zéro envoi). **ATTENTION** : ce deploy pousse pour la 1re fois en prod la mise à jour **supabase 2.108 + TS6** (la prod actuelle est un deploy immuable d'avant la reconciliation). Le **smoke auth OTP manuel de Pascal** doit être fait AVANT (sinon on met un changement d'auth non validé en prod). Pas `[EXÉCUTABLE]` tant que ce smoke n'est pas vert.
- [ ] **[BLOQUÉ - Déployer livré + feu vert Pascal sur activation]** **Activer** = poser `EMAIL_DAILY_ENABLED=true` en env Vercel Prod (zéro redéploiement, lu via `$env/dynamic/private`). Avant ces 2 gestes : aucun envoi possible.

### 2. Bloc C - Veille Lot 3 structurel [AUTO • partie « faisables maintenant » LIVRÉE `b767056`]

- [x] ~~PDF de marque + pont veille→action (fix cul-de-sac chips vers sources dormantes RegBL/SIMAP) + nettoyage enum RegBL + décision NO-OP « décomposer le mono-appel LLM »~~ - LIVRÉ 2026-06-26 (`b767056`, CI verte, **PAS de deploy** = gated Pascal). Workflow adversarial 4 lentilles 0 C/H. → [[project_veille_bloc_c_pont_pdf_2026-06-26]] + [[audit_secu_2026-06-26_bloc_c_veille_pont_pdf]].
...
- [x] ~~**Bloc C veille (faisables maintenant) - fix pont/RegBL cul-de-sac + PDF de marque**~~ - 2026-06-26 (ultracode/xhigh) : commit `b767056` (CI verte, **PAS de deploy** = gated Pascal). (1) **Fix pont veille→prospection + nettoyage enum RegBL** : un chip vers une source dormante (RegBL/SIMAP `enabled:false` V5) n'affiche plus un bouton « Lancer la recherche » garanti d'échouer (403), il bascule en « Copier le terme » (helper `isChipExecutable` + guard defense-in-depth 403 précoce dans `from-intelligence` + labels/icônes regbl corrigés via `chipKindLabel`/`chipKindIcon` centralisés). Réversibilité par flag préservée (vérifiée). (2) **PDF de marque** : endpoint GET auth-gaté `/crm/veille/[id]/brief` réutilisant le rendu brandé de l'email brief (`buildBriefPrintHtml`, email intact) + bouton « Exporter en PDF » (édition à contenu). Adaptateur testé `rowToIntelligenceReport`. (3) **Décision NO-OP gravée** « décomposer le mono-appel LLM » (goulot = rareté du sourcing, pas la densité ; décomposer = +coût LLM contraire à « zéro LLM en plus »). Workflow adversarial 4 lentilles **0 C/H, secu 0 vuln** ; 12 findings traités, 2 refusés tracés ; 2322 tests verts, svelte-check 0/0, build OK. **Revue visuelle Pascal du PDF avant deploy.** → [[project_veille_bloc_c_pont_pdf_2026-06-26]] + [[audit_secu_2026-06-26_bloc_c_veille_pont_pdf]].
- [x] ~~**Bloc A - couper les mails de plantage à la racine**~~ - 2026-06-26 (ultracode/xhigh) : commit `f357e68` (CI vert), 4 fichiers config. (1) `vercel.json` → `git.deploymentEnabled {"dependabot/**": false}` coupe les preview deploys aveugles des PR Dependabot (globstar minimatch prouvé local + source primaire). (2) `dependabot.yml` → gel patch socle (vite, plugin-svelte, kit, svelte-check ; svelte déjà gelé). (3) 4 branches stale supprimées → trunk = `main` seule. (4) résidu daily-email gitignoré. Vérif adversariale 3 lentilles **0 blocker** ; régime live exercé (Dependabot Updates success, 0 PR). Repo : branch protection `main` OFF + security updates OFF. → [[project_bloc_a_mails_plantage_2026-06-26]].
