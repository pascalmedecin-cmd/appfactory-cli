...
**Session courante :** 2026-06-27 - **Bloc D audit 360 LANCÉ** (workflow 112 agents, 90 findings) : **26 fixés V1-V3** (commits LOCAUX non poussés), **63 restants documentés** (`memory/project_bloc_d_findings_restants_handoff.md`). Session arrêtée (quota 7j à 90%, décision Pascal) → finir V4+V5 prochaine session puis porte supervisée + push. **À FAIRE Pascal** (inchangé) : smoke OTP prod + activer `EMAIL_DAILY_ENABLED=true` + revue PDF veille. Détail → § « Livré » + `archive/`.
...
- [ ] **[EXÉCUTABLE]** **V4 + V5 : 63 findings restants** (9 med, 54 low) - détail intégral file:line + preuve + fix → `memory/project_bloc_d_findings_restants_handoff.md`. V4 = cohérence primitives (pastille Signaux→ScorePill, boutons réinventés, onglets campagnes→Tabs) ; V5 = tokens/spacing/easing mécaniques + code mort (fetchIntelligenceSignal, listActiveSources, verifyUrlsBatch, d3-scale) + copy. **Fixer no-debt**, puis **porte supervisée** : présenter à Pascal + push (auto-deploy prod). → voir `memory/project_bloc_d_audit_360_PROGRESS.md`.
