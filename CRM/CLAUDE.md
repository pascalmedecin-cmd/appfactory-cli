...
**Dernière mise à jour :** 2026-07-21 (Cohérence UI **increment d2/d3 LIVRÉ** = rythme vertical échelle de 8, `ff_ui_coherence` OFF, commit `8bc4b6e` ; 7 fixes chirurgicaux, scope resserré par la mesure — l'app avait déjà convergé, KpiStrip→grille/12-col = redessin parqué gate ; QA DOM on/coh-only/off × 1440/1024/1025/1000/390 + revue adversariale 4 lentilles = 3 findings corrigés ; détail → « Livré cette session » + `docs/COHERENCE-UI-BANDEAU.md` § d). **Prochain bug :** #001.
...
**Prochaine attaque** : la **Phase 1 « figer la base visuelle » est quasi close** (lots 2A/2B/2C + d1 + **d2/d3** livrés, tous `ff_ui_coherence` OFF). Deux options sans gate : **(a) CTAs desktop Prospection** (5, `hidden md:inline-flex`, passe dédiée, QA 767px — actionnable seul) ; **(b) allumer `ff_ui_coherence` + `ff_page_bandeau` sur les fondateurs** pour voir toute la cohérence UI en vrai (rendu prouvé, réversible). Le reste Phase 1 est `[GATE mockup]` (reliquats esthétiques, accueil, import mobile) + le **redessin KpiStrip→grille/12-col parqué**. Ensuite **Phase 2 = timeline d'interactions** (`[GATE mockup]`). Contexte : Atelier 209 à **3/7 runs en prod**, 5 phases (figer base, timeline, LED Run 7, pipeline contact, coiffer). → [[project_coherence_ui_d3_grille_2026-07-21]], [[feedback_qa_micro_mon_ressort_gate_pas_excuse]].
...
- [x] ~~Cohérence UI **d2/d3** (suite grille) : rythme vertical échelle de 8~~ - **LIVRÉ 2026-07-21** (`ff_ui_coherence` OFF, commit `8bc4b6e`). 7 fixes chirurgicaux (ws-content/content 32→24, reporting indicators 4/16 + panel.mt-24 32, campagnes toolbar 0/24, aide onglets +4, prospection uniforme 24). **Scope resserré par la mesure** : contacts/pipeline déjà 16/16/24 ; **KpiStrip→grille + 12-col = redessin HORS scope** (mockup « inchangé ») → parqué `[GATE mockup]` ci-dessous. Exemptions signées : dashboard compact, veille magazine, sections aide. Leçon : `ff_ui_coherence`/`ff_page_bandeau` indépendants → fixes adjacents au bandeau scopés `.pband + X`. QA on/coh-only/off × 5 breakpoints + adversarial 4 lentilles (3 findings corrigés). → [[project_coherence_ui_d3_grille_2026-07-21]].
...
- [x] ~~**Cohérence UI increment d2/d3 = rythme vertical échelle de 8**~~ - 2026-07-21 (ultracode, autonome). `8bc4b6e`, `ff_ui_coherence` OFF, 7 fixes chirurgicaux (ws-content/content 32→24, reporting indicators 4/16 + panel.mt-24 32, campagnes toolbar 0/24, aide +4, prospection uniforme 24). Scope resserré par la mesure (app déjà convergée) ; redessin KpiStrip/12-col parqué gate ; leçon flags coherence/bandeau indépendants → fixes bandeau-adjacents scopés `.pband + X`. QA on/coh-only/off × 1440/1024/1025/1000/390, OFF byte-identical, svelte-check 0/0, Vitest 2863, adversarial 4 lentilles (3 findings corrigés). → [[project_coherence_ui_d3_grille_2026-07-21]].
