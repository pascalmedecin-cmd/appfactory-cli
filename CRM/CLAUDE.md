...
- [ ] **[EXÉCUTABLE]** **Cohérence UI globale du CRM** - benchmark **Gouvernance** ([[feedback_gouvernance_benchmark_layout_lisibilite]]) : discipline de structure, **pas la peau** (identité Atelier 209 conservée). **(a) Bandeau : mockup validé + increment 1 (Entreprises) LIVRÉ derrière `ff_page_bandeau`** → dérouler increments 2-4 selon `docs/COHERENCE-UI-BANDEAU.md` (desc 1 ligne, pastille partout, textes à affiner ; mockup `.atelier-209/coherence-ui/bandeau-mockup.html`). **Restent à mocker** : (b) composants transverses, (c) compteurs agrandis, (d) grille 2 niveaux. `redesign-skill` + `refactoring-ui`.
