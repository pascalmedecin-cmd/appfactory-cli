...
**Prochaine attaque** : Bloc 1 - bump modèle veille `opus-4-7` → `4.8` - finding actif relevé cette session, rapide et débloqué (1 ligne + 1 run de validation). Sinon, pistes non cadrées (à formaliser au `/start`) : mobile V3 terrain (cf. [[project_refonte_mobile_v3_terrain]]) ; chantier 3 portail ; durcissement RLS si 4e user non-fondateur ; étendre le corpus golden optimiseur quand le catalogue produits s'enrichit.

### 1. Veille : bump modèle de génération opus-4-7 → 4.8 [SUPERVISÉ • medium • ~30 min + 1 run]
- **Pourquoi** : le pipeline de veille génère avec `claude-opus-4-7` (périmé) ; 4.8 est le dernier Opus. Finding relevé 2026-06-06 pendant le fix résilience veille.
- [ ] **[EXÉCUTABLE]** passer la constante `MODEL` (`src/lib/server/intelligence/generate.ts`, grep `const MODEL`) à `claude-opus-4-8`, valider sur un run réel (qualité de la sortie + coût via `workflow_dispatch`), puis confirmer. Impact qualité/comportement → juger la sortie avant de figer (SUPERVISÉ).

### Livré cette session

- [x] ~~**Veille : validation résiliente par-article** (anti-perte d'édition + rattrapage W23)~~ - Livré 2026-06-06 (xhigh, TDD strict, spec-first + council 4 voix + investigation historique anti-hallu). Le cron veille échouait dès qu'1 article généré violait 1 contrainte de schéma (incident W23 : 1 article à 1 `search_term`) → édition entière perdue, ~5 min Anthropic jetés. Remplacé le `safeParse` global tout-ou-rien par une validation **article-par-article** (`report-validate.ts`, fonction pure, 13 tests) : `meta`/`impacts` stricts, articles validés un par un (conformes gardés, fautifs écartés **jamais réparés**), garde anti-dérive (échec si >3 OU >30 % OU 0 valide) + compteur `schemaDropped` loggé. `search_terms` `min(2)→min(1)` (reclassé préférence produit, pas anti-hallu). **Vérifié (2 agents historiques) : 0 protection anti-hallucination défaite** - le blindage (URL active, fenêtre dates, cross-check verbatim) est en aval, déjà par-article, schéma d'intégrité réutilisé à l'identique. Smoke réel = rattrapage W23 (`workflow_dispatch`) : run vert 7m20, `schemaDropped:0`, cross-check a rejeté 5/8 articles (anti-hallu intact), édition publiée (3 articles, statut `partial`). 1665 Vitest, svelte-check 0, build OK. Commit `e0aef36` (mergé `main`). Spec `.product-architect/veille/resilience-validation-spec.md` (local). → [[feedback_splitter_deterministe_post_llm]].
...
→ Livré antérieur (Découpe pages pleine largeur `8c46e06`, Phase 5 livraison prod, Phase 3 étapes 1-3 + amont portail/V3) → `archive/2026-06-05-decoupe-films.md` + `archive/2026-06-01-sessions.md`.
