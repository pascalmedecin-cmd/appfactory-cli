...
**Dernière mise à jour :** 2026-07-21 (soir 4 : **refonte accueil Atelier 209 CODÉE + QA VERTE, NON POUSSÉE** - fondu masque vertical long 60/40 + hero upscalé fal.ai clarity 2368px + portail 3 colonnes (CRM/Découpe/Coûts) + login boutons égalisés 220×60 / hint 1 ligne ; login mesuré 5 viewports + portail 3, svelte-check 0/0, Vitest 2863, bug-hunter 0 bug ; commit local `6c07889` **ahead 1**, attend ton **sign-off visuel sur `/login`** avant `git push` (push = login live). Règle globale gravée : boutons appariés = mêmes dimensions → [[feedback_paired_action_buttons_equal_dimensions]]. Plus tôt le 21/07 : CTAs desktop Prospection livrés+poussés `727b290`, collision tabs-shell fixée+déployée, d2/d3 `8bc4b6e`, **BUG #001 login OTP résolu par provisioning** (2 comptes `@lamaisoncreativedirection.ch` provisionnés, reste smoke test = geste Pascal → [[project_login_otp_new_user_confirm_signup_2026-07-21]])). Directive permanente : **zéro angle mort = mesurer, pas raisonner** → [[feedback_zero_angle_mort_mesurer_pas_raisonner]].
...
**Prochaine attaque** : **Pousser l'accueil Atelier 209 en prod** après ton sign-off sur le vrai `/login` (ouvert dans ta fenêtre, `localhost:5173/login`). La refonte accueil est **CODÉE + QA VERTE, non poussée** : maquette validée par toi (« c'est validé ») puis portée dans le vrai code - `AtelierShell` (photo en fond + **fondu masque vertical long**, zéro ligne, layout **60/40**, contenu ancré bas mordant le fondu, aération eyebrow↔titre), portail **3 colonnes** (CRM/Découpe/**Coûts**), login **boutons égalisés** (220×60) + hint 1 ligne, **hero upscalé fal.ai** (clarity 2368px) + srcset retina. Mesuré 5 viewports login + 3 portail (eyebrow toujours dans le fondu, 0 clip, 0 débordement), `svelte-check 0/0`, **Vitest 2863**, bug-hunter 0 bug. **PAS poussé** : push `main` = login live (règle review-avant-push + surface auth) → ton OK visuel puis `git push`. Ensuite : reliquats Phase 1 `[GATE mockup]` (esthétiques increment b, redessin KpiStrip/12-col, import mobile) OU Phase 2 timeline d'interactions (mockup d'abord). Contexte : Atelier 209 à **3/7 runs en prod**. Directive gravée : **zéro angle mort = mesurer, pas raisonner** ([[feedback_zero_angle_mort_mesurer_pas_raisonner]]). → [[project_accueil_atelier209_mockup_2026-07-21]], [[feedback_paired_action_buttons_equal_dimensions]].
...
**Phase 0 - BUG PRIORITAIRE — RÉSOLU 21/07** (provisioning, zéro code ; reste le smoke test = geste Pascal, tracé aux Gestes transverses)

- [x] ~~**BUG #001 login OTP**~~ - **RÉSOLU 2026-07-21** (provisioning pur, zéro changement de code login). Vraie cause = le compte n'existait pas (pas le code, qui marchait pour les `@filmpro.ch`) : `signInWithOtp` sur un email autorisé mais sans compte créait le compte + envoyait « Confirm Your Signup » au lieu du code. Fix = **faire exister les comptes** comme les `@filmpro.ch` : `pascal@lamaisoncreativedirection.ch` (existait déjà) → flags miroir posés ; `antoine@lamaisoncreativedirection.ch` (absent) → créé + confirmé (admin API, aucun email) + flags miroir. Les 4 comptes vérifiés en base (existants, confirmés, identité email, flags = miroir). Reste : smoke test réel = geste Pascal. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
...
- [x] ~~**CTAs desktop Prospection**~~ - **LIVRÉ 2026-07-21** (`ff_ui_coherence` OFF, ultracode). Hook `coh-prosp-cta` + override co-localisé (padding 16 / poids 600 / transition 220ms + garde reduced-motion ; jamais class-swap `.ws-btn` ; teinte violette « Enrichir » préservée ; focus ring déjà global). QA exhaustive (off/on/**mixed 2 flags** × 767-1920 × reduced-motion × dormant injecté non-vacue) + workflow 4 lentilles + bug-hunter = SHIP, 0 angle mort. → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]], [[feedback_zero_angle_mort_mesurer_pas_raisonner]].
- [x] ~~**Fix collision onglets↔CTA tabs-shell Prospection**~~ - Fait 2026-07-21 : poussé (`05ee3e6`) + **déployé prod** (wrap content-driven, robuste onglets/boutons ; bug-hunter 1 finding corrigé). Détail → « Livré cette session ». [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
...
- [x] ~~Cohérence UI **d1** (gouttière unique 32px + largeur bornée ancrée à gauche, Veille→gauche)~~ - **LIVRÉ 2026-07-20** (`ff_ui_coherence` OFF, socle centralisateur, **24 fichiers** : la carto sous-estimait — 6 `*Indicators` + primitive `Tabs` + 6 sous-pages en plus des 8 pages, tout-ou-rien sur les 18 routes /crm). QA DOM prouvée : gutter unique 32/24/16 partout ON, borne 1440 ancrée à gauche @2000, legacy inchangé OFF (1440/2000/1000). Revue adversariale 5 lentilles = 4 REFUTED-clean + 1 medium (Tabs `:not(.compact)` 0-3-0 média battait le hook → 24px résiduel ≤1024, corrigé en 0-4-0). svelte-check 0/0, Vitest 2863. → [[project_coherence_ui_d1_grille_2026-07-20]].
- [x] ~~Cohérence UI **d2/d3** (rythme vertical échelle de 8)~~ - **LIVRÉ 2026-07-21** (`ff_ui_coherence` OFF, `8bc4b6e`) : 7 fixes chirurgicaux, scope resserré par la mesure (app déjà convergée), redessin KpiStrip/12-col parqué gate ci-dessous. → [[project_coherence_ui_d3_grille_2026-07-21]].
- [ ] **[EXÉCUTABLE - GATE mockup]** Cohérence UI - **redessin KpiStrip→grille + trame 12-col partout** (D3/D6 du scratch `inventory-d-grille.md`, écartés de d2/d3 car le mockup validé dit « KpiStrip inchangé, rien n'est redessiné » = redessin, pas alignement). À montrer en maquette Chrome si tu veux les rangées KPI en grille alignée. → `docs/COHERENCE-UI-BANDEAU.md` § d.
- [x] ~~Retouches accueil (`AtelierShell.svelte`) + upscale + portail 3-col~~ - **CODÉ + QA 2026-07-21** (ultracode) : hero upscalé fal.ai clarity 2368px, fondu **masque vertical long** (remplace surimpression, zéro ligne), layout **60/40** (image plafonnée min(60vh,100dvh-380), contenu ancré bas mordant le fondu), aération eyebrow↔titre desktop+mobile, portail **3 colonnes** (CRM/Découpe/Coûts), login boutons égalisés + hint 1 ligne (règle [[feedback_paired_action_buttons_equal_dimensions]]). Mesuré 5 viewports, svelte-check 0/0, Vitest 2863, bug-hunter 0 bug. **Non poussé** (attend sign-off /login). → [[project_accueil_atelier209_mockup_2026-07-21]].
...
- [x] ~~Allumer `ff_ui_coherence` + `ff_page_bandeau` sur les fondateurs~~ - Fait 2026-07-21 : allumés sur `pascal@filmpro.ch` en prod (merge jsonb, flags existants préservés), un seul flow (Antoine ne se logue pas encore). Réversible. Option restante : alléger le greeting Dashboard en vue premium si Pascal le veut. → `docs/COHERENCE-UI-BANDEAU.md`.
...
- [ ] **[BLOQUÉ - toi : sign-off visuel /login puis push]** Pousser la refonte accueil Atelier 209 en prod : regarde le vrai `/login` (ouvert dans ta fenêtre, `localhost:5173/login` - dev encore actif ; sinon `npm run dev`) → hero net, fondu sans ligne, blocs/boutons équilibrés ; le portail (même shell) suit. Puis `git push` (auto-déploie le login live). CODÉ + QA vert, **non poussé** par prudence auth. → [[project_accueil_atelier209_mockup_2026-07-21]].
- [ ] **[BLOQUÉ - toi : sign-off]** Smoke test login LMCD : te connecter avec `pascal@lamaisoncreativedirection.ch` → doit envoyer le **code 6 chiffres** (pas « Confirm signup ») + vue premium (flags on). Valide le fix BUG #001. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
- [x] ~~Pousser d1/d2/d3 (+ docs) + le fix collision sur `main`~~ - Fait 2026-07-21 : `git push` (d69ffeb..2ff9b4d), Vercel déployé (Ready, aliasé). d1/d2/d3 invisibles (flag OFF) ; fix collision visible (répare le bug prod).
- [ ] **[BLOQUÉ - toi : arbitrer quoi condenser]** Condensation dédiée `CRM/CLAUDE.md` (~32,3K chars > 20K soft, archive volatile insuffisante) : le surplus est le roadmap 5 phases + watch list (contexte de décision frais), non charcuté autonomement. Décider avec moi quoi archiver.
...
- [x] ~~**Refonte accueil Atelier 209 CODÉE (fondu masque 60/40 + hero upscalé + portail 3-col + login équilibré)**~~ - 2026-07-21 (ultracode, autonome). Maquette validée (« c'est validé ») puis portée : `AtelierShell` réécrit (photo fond absolu + mask-image long, header sorti + pointer-events, contenu ancré bas mordant le fondu, aération eyebrow↔titre), portail 2→3 colonnes (Coûts), login boutons égalisés 220×60 + hint 1 ligne (règle globale). Hero upscalé fal.ai clarity 2368px + srcset retina. QA mesurée 5 viewports login + 3 portail, svelte-check 0/0, Vitest 2863, bug-hunter 0 bug. **Non poussé** (sign-off /login requis). → [[project_accueil_atelier209_mockup_2026-07-21]], [[feedback_paired_action_buttons_equal_dimensions]].
- [x] ~~**CTAs desktop Prospection alignés sur `.ws-btn` (cohérence UI, `ff_ui_coherence` OFF)**~~ - 2026-07-21 (ultracode, autonome). Dernier `[EXÉCUTABLE]` pur de Phase 1. Hook `coh-prosp-cta` appendé (inconditionnel) aux 5 CTAs + override co-localisé `:global(.coherence-ui)` (padding `8px 16px` / poids 600 / transition 220ms `ease-out-expo` + garde `@media(prefers-reduced-motion)` → contrat `.ws-btn` complet). **JAMAIS de class-swap `.ws-btn`** (gotcha `hidden md:inline-flex`) ; override ne touche NI display NI couleur → **teinte violette « Enrichir » préservée**, responsive intact ; focus ring déjà global (`app.css:311-325`, bloc dupliqué mort retiré). Élargissement absorbé par le wrap collision `05ee3e6`. **QA zéro angle mort** : `tests/_qa-coherence-cta-prospection.mjs` mesure off/on/**mixed 2 flags** × largeurs 767-1920 × reduced-motion × dormant enrich injecté (non-vacue, hash de scope copié) + assertions padding H/V, hauteur 40, transition 0.22s, OFF inerte absolu. Double filet adversarial (**workflow 4 lentilles + bug-hunter**) = SHIP, 3 observations toutes fermées. svelte-check 0/0, Vitest 2863. Poussé + déployé : feat `33f3506` + docs `727b290`, Vercel Ready. Latent 5-onglets = pré-existant (watch simap/regbl). Directive Pascal gravée [[feedback_zero_angle_mort_mesurer_pas_raisonner]]. → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
- [x] ~~**BUG #001 login OTP résolu par provisioning (zéro changement de code login)**~~ - 2026-07-21 (ultracode, autonome). Directive Pascal en cours de route : « le login marchait très bien avant, remets ce qui marchait, pas de nœud au cerveau » → 1re piste (`shouldCreateUser:false` + gestion d'erreur) **revertée** (git propre, byte-for-byte l'original). Vraie cause = le compte n'existait pas : les 2 comptes `@lamaisoncreativedirection.ch` **provisionnés** comme les `@filmpro.ch` (pascal LMCD : flags miroir posés via admin API ; antoine LMCD : créé + confirmé, aucun email, flags miroir). 4 comptes vérifiés en base (existants, confirmés, identité email OK, flags = miroir exact du compte filmpro correspondant). Fix = data prod pure, aucun diff code → suite 2863 inchangée par construction. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
- [x] ~~**Fix collision onglets↔CTA prospection (wrap content-driven) + déploiement prod + flags cohérence allumés**~~ - 2026-07-21 (ultracode, autonome). CTA cohérence tenté→reverté (aggravait une collision prod pré-existante ≤1280 sidebar déployée, trouvée par revue adversariale) ; collision **FIXÉE** (`05ee3e6`, wrap content-driven robuste au nombre d'onglets/boutons, bug-hunter 1 finding corrigé) ; **poussé prod** (d1/d2/d3 + fix, `2ff9b4d`, déployé) + `ff_ui_coherence`/`ff_page_bandeau` **allumés sur pascal@**. Découvert **BUG #001 login OTP** (voir Prochaine attaque). QA `_qa-tabs-wrap-collision.mjs`, svelte-check 0/0, Vitest 2863. → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]].
- [x] ~~**Cohérence UI d2/d3 = rythme vertical échelle de 8**~~ - 2026-07-21 (ultracode, autonome). `8bc4b6e`, `ff_ui_coherence` OFF, 7 fixes chirurgicaux ; scope resserré par la mesure (app déjà convergée), redessin KpiStrip/12-col parqué gate. QA on/coh-only/off × 5 breakpoints + adversarial 4 lentilles (3 findings corrigés), svelte-check 0/0, Vitest 2863. → [[project_coherence_ui_d3_grille_2026-07-21]].
