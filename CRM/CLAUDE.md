...
**Derniere mise a jour :** 2026-05-13 (S185 : page Log CRM LIVRÉE - 15 critères binaires verts, 58 nouveaux tests Vitest, 0 C/H/M audits Opus, migration SQL `20260513_001_feedback_entries.sql` à appliquer Supabase prod).
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** Session 185 (CRM, 2026-05-13, `/effort xhigh`) - **Page Log CRM LIVRÉE** : cadrage /dig 360 + spec 15 critères figée (`CRM/notes/page-log-2026-05-13/spec.md`) → exec 5 lots de commit (DB+lib / composants / page+actions / tests / docs) → 1250/1250 Vitest verts, svelte-check 0 err, build OK, 3 audits Opus parallèles (security 0C/0H/0M + 1L fixé in-session + 4 Info ; coverage 7/7 critères § 8 + gap actions serveur fixé via 13 tests `log-actions.test.ts` ; contracts 0C/0H + 1M doc + 4L cosmétiques). Bouton flottant + page /log desktop-only (audit mobile CRM différé en entry idée cockpit `d068a79d`). Entry cockpit `af586294` à livrer.
...
**Prochaine attaque** : Bloc 1 Smoke prod page Log + résidu S183 (Pascal manuel, ~15 min). Bloc 2 Cleanup post-livraison page Log (Claude exécutable, ~1h30, 7 watch items consolidés, non-bloquant). Tâches dormantes possibles (hors-scope nommé S182, à transmettre via UI cockpit) : tour guidé interactif, bouton « ? » contextuel sur les 9 pages, push première-connexion vers /aide.

### 1. Smoke prod page Log + résidu S183 [SUPERVISÉ • low • ~15min]

- **Pourquoi** : la page Log a été livrée S185 (5 commits, push, audits 0 C/H/M) mais la migration Supabase prod n'est pas auto-appliquée. Sans ça, `/log` 500 + bouton flottant échoue silencieusement. Smoke résidu S183 (`/aide` diagrammes refondus + nav mobile fermeture menu) toujours en attente.

- [x] ~~**[PASCAL MANUEL]** Bloquant : appliquer la migration `supabase/migrations/20260513_001_feedback_entries.sql` au Supabase Dashboard~~ — **Appliquée par Claude S185 via skill `supabase` + `pg` lib (npm install --no-save) + script ESM one-shot avec `DATABASE_URL_ADMIN`** (12 statements en 0,1 s). Smoke vérif : `table_count=1` + `policy_count=3` + `index_count=4` + `rls_enabled=true` + `trigger_count=1`. La table `feedback_entries` existe en prod avec RLS active.
- [ ] **[PASCAL MANUEL]** Smoke prod admin (`pascal@filmpro.ch` sur <https://filmpro-crm.vercel.app>) : FAB visible bottom-right → modale s'ouvre → submit `bug + gênant + /contacts + description ≥ 10 chars` → toast vert « Retour envoyé, merci » → entry apparaît dans `/log` triée date desc → click ligne → expand inline avec context (URL/viewport/UA) → click bouton statut `À actionner` → badge orange → click bouton `Exporter en JSON` → téléchargement `feedback-2026-05-13.json` UTF-8 parseable Claude Code.
- [ ] **[PASCAL MANUEL]** Smoke prod user (`antoine@filmpro.ch`) : FAB visible, modale s'ouvre, peut créer un retour, voit la liste lecture seule (pas de boutons statut, pas d'export, pas de note interne).
- [ ] **[PASCAL MANUEL]** Chrome DevTools manuel breakpoint < 1024 px (iPhone 14 Pro Max preset) : FAB absent (display:none via media query), entrée sidebar « Log » absente (`.desktop-only-nav` masquée), page `/log` rend uniquement l'encart « Disponible uniquement depuis ordinateur » avec icône `desktop_windows`.
- [ ] **[PASCAL MANUEL]** Smoke prod résidu S183 : `/aide` (5 diagrammes SVG corrigés + schéma écosystème refondu + chiffres bulles centrés + tirets longs retirés + respiration accrue) + nav mobile (menu latéral se ferme à la sélection d'une page).

### 2. Cleanup post-livraison page Log [AUTO • low • ~1h30]

- **Pourquoi** : la livraison S185 est complète (15/15 critères verts, audits 0 C/H/M), mais 7 watch items non-bloquants ont été identifiés par les 3 audits Opus (security Info-1, contracts M-1 + L-2 + L-4, coverage Low) et le pattern `as never` Supabase à nettoyer. Tous regroupés ici pour 1 passe propre, ROI sécurité + cohérence types.

- [ ] **[EXÉCUTABLE]** Livrer les 7 fix cleanup post-livraison page Log : (a) ajouter `/log/` à la liste `isRateLimitedPath` dans `src/hooks.server.ts:19-23` (Info-1 audit secu, 1 ligne, anti-spam si retour terrain), (b) patcher la spec `notes/page-log-2026-05-13/spec.md` ligne 70 (retirer `NOT NULL` sur `created_by` pour aligner avec choix migration nullable + `ON DELETE SET NULL`, M-1 contracts), (c) normaliser `user.email` à l'insert via `.trim().toLowerCase()` dans `src/routes/(app)/log/+page.server.ts:55-64` (L-2 contracts), (d) normaliser `context` au `load` via `FeedbackContextSchema.parse(row.context ?? {})` pour honorer le type TS strict sur les rows legacy (L-4 contracts), (e) regen types Supabase via `supabase gen types typescript --project-id <ID> > src/lib/database.types.ts`, retirer les 7× `as never` dans `+page.server.ts` (cocher en cascade les dettes V2a M-19 + V2b M-19 + V3a M-19 si possible), (f) ajouter 1 test happy path `errorCapture.install()` avec mock `browser=true` (coverage Low, gain marginal), (g) durcir CHECK SQL `admin_notes IS NULL OR char_length BETWEEN 1 AND 2000` si on veut bannir `''` (L-3 contracts, optionnel). Audits Opus secondaires ciblés sur les fichiers touchés (0 C/H/M attendu). 1250+ Vitest verts, svelte-check 0 err, build OK. Commit unique + push. → voir CRM CLAUDE.md § Watch list S185 (post-page Log)

### 1. Page Log CRM (feedback bugs + améliorations) [AUTO • xhigh • ~3h]

**Pourquoi** : capter les retours d'Antoine (et des fondateurs FilmPro) pendant la prise en main du CRM, permettre à Pascal d'exporter le backlog actionnable en JSON pour traitement Claude Code CLI. Spec figée 2026-05-13 (15 critères binaires, design system GOLDEN v9, RLS Supabase « lecture publique authentifié + update admin seul `pascal@filmpro.ch` », bouton flottant + page desktop-only, audit mobile CRM global différé en entry idée cockpit `d068a79d`). AUTO car spec verrouillée et critères binaires testables ; basculer SUPERVISÉ uniquement si dérive design surgit en cours de code.
**Prérequis** : aucun. Spec, schéma DB, format export, statuts, helpers admin, tests, gates QA tous documentés.
- [x] **[EXÉCUTABLE]** Coder la page Log CRM en 5 lots de commit (migration SQL + lib helpers, composants Feedback*, page /log + form actions, tests Vitest, audits Opus + smoke + push) selon § 12 de la spec. Livraison sur Vercel prod après audits 0 C/H/M. → voir `CRM/notes/page-log-2026-05-13/spec.md` — **Livré S185 2026-05-13** : code complet 5 lots, 1250/1250 Vitest, svelte-check 0, build OK, 3 audits Opus 0C/0H/0M. Reste côté Pascal : appliquer migration Supabase prod + smoke prod desktop + smoke prod mobile (DevTools manuel).



### Watch list S185 (post-page Log)

- **[WATCH] Migration Supabase prod `20260513_001_feedback_entries.sql` à appliquer** (vu 2026-05-13) : le code est livré, la migration vit dans `supabase/migrations/`, mais Supabase prod n'est pas auto-pushé. Pascal doit copier le SQL dans Supabase Dashboard → SQL Editor → exécuter. Sans ça `/log` retourne 0 entry (table inexistante) et le form `create` échoue silencieusement avec une erreur DB opaque. À supprimer cette ligne dès que migration appliquée.
- **[WATCH] Rate limit `/log?/create` non couvert** (audit secu Info-1) : `hooks.server.ts:19-23` limite `/api/prospection/*`, `/api/photos`, `/api/visits`, `POST /login`, mais pas les form actions de `/log`. Un user authentifié peut spammer. Borné par DB CHECK 1000 chars + Zod context 20 KB, donc surface faible (≤ 10 admins @filmpro.ch). Ajouter à la liste `isRateLimitedPath` (1 ligne) si retour terrain ou ouverture aux invités externes.
- **[WATCH] Spec page Log § 4 ligne 70 `created_by NOT NULL`** (audit contracts M-1) : la spec dit `uuid NOT NULL REFERENCES … ON DELETE SET NULL` (contradictoire en soi : SET NULL impossible sur NOT NULL). La migration livrée a tranché `nullable` + `ON DELETE SET NULL` (cohérent § 14 « dénormalisé pour survivre à suppression user ») et `FeedbackEntry.created_by: string | null` est aligné. À patcher la spec (1 ligne : retirer `NOT NULL`) au prochain passage sur le fichier.
- **[WATCH] Normalisation email à l'insert `feedback_entries.created_by_email`** (audit contracts L-2) : `+page.server.ts:55-64` insère `user.email` tel quel sans `.toLowerCase()`, alors que `isAdminEmail` compare en lowercase. Si Supabase renvoie un email à casse mixte (rare en pratique avec @filmpro.ch), une requête SQL ad hoc `created_by_email = ADMIN_EMAIL` casserait. Non-bloquant V1. Mitigation : appliquer `.trim().toLowerCase()` à l'insert ou ajouter une expression-based UNIQUE INDEX.
- **[WATCH] Normaliser `context` au load pour honorer le type TS strict** (audit contracts L-4) : `types.ts:24` `FeedbackContext` exige `{url, viewport, userAgent, recentErrors}`. Une row legacy `context = '{}'::jsonb` aurait les champs `undefined` au runtime alors que le cast `as FeedbackEntry[]` côté `load` ment au TS. Le `FeedbackContextSchema` Zod applique `.default(...)` mais uniquement à l'insert. Mitigation : passer `context` au `FeedbackContextSchema.parse(...)` côté `load` (normalise les rows legacy + valide la forme). Non-bloquant V1.
- **[WATCH] Regen types Supabase `feedback_entries`** (dette `as never` post-livraison) : `+page.server.ts` utilise 7× `from('feedback_entries' as never)` + cast payload `as never`. Pattern aligné `src/lib/server/quota.ts` V2b CRM. Regroupe la dette V2a M-19 + V2b M-19 + V3a M-19. Regen via `supabase gen types typescript --project-id ... > src/lib/database.types.ts` au prochain pass de cleanup.
- **[WATCH] `errorCapture.install()` happy path browser=true non testé** (audit coverage Low) : `error-capture.ts` est gated par `if (!browser || installed) return` ; en Vitest `browser=false` donc `install` est no-op et non testé. Logique triviale (listeners `error` + `unhandledrejection`). Accepter, gain marginal.
