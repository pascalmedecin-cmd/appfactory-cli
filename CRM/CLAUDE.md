...
**Session courante :** 2026-06-30 - **Étiquettes de publipostage sur la page Campagnes**. Menu d'une campagne -> panneau de sélection des prospects étiquetés (recherche, tout/rien, badge « adresse incomplète ») -> **PDF planche Avery 6122** (24/page, police **Outfit**, jsPDF+svg2pdf doctrine Découpe). Adresse lue du stock (enrichie en amont par les recherches search.ch/Zefix), **zéro API à la génération** (contrainte Pascal). QA verte (2365 vitest dont 73 feature, svelte-check 0, build OK, e2e PDF Outfit 3/3) ; revue adversariale 16 agents (sécu 0 finding) + **8 findings med/low/info TOUS corrigés en session**. **Statut : code complet + testé sur l'arbre de travail, NON commité/NON déployé** (push `main` = auto-prod -> attend ton go). **À FAIRE Pascal** : (1) go commit + déploiement, (2) smoke OTP de la feature. Détail → [[project_etiquettes_publipostage_campagnes_2026-06-30]] + § Livré.
...
- [ ] **[CODE PRÊT - non commité/non déployé, attend go Pascal] Étiquettes de publipostage (page Campagnes)** - 2026-06-30 (ultracode/xhigh). Menu d'une campagne -> panneau (SlideOut, **tout sur la page Campagnes, aucune navigation**) de sélection des prospects étiquetés + recherche + tout/rien + badge complétude + bouton **« Compléter »** par ligne (remplit l'adresse manquante via le lookup **unitaire** `search-ch`, sanctionné V5 ; pas de bouton de masse, le batch reste désactivé par design) -> **PDF planche Avery 6122** (24/page 70×36 mm, texte centré, NOM gras, police **Outfit** Google Fonts embarquée TTF). Géométrie portée du pipeline étiquettes Marketing ; moteur jsPDF+svg2pdf (doctrine Découpe). **Adresse lue du stock, zéro API à la GÉNÉRATION** (enrichissement = au moment des recherches / du bouton Compléter, jamais à la prod du PDF). **UI premium** (redesign-skill : cartes, pin localisation, badge, spinner, CTA). Nouveaux : `lib/etiquettes/{pdf-etiquettes,prospect-etiquette,etiquettes-fonts}.ts` + tests, endpoint `api/campagnes/[id]/prospects`, `components/campagnes/EtiquettesPanel.svelte`, `tests/etiquettes-pdf.test.ts`, `scripts/_etiquettes_gen_pdf_fonts.py` ; +dep `@fontsource/outfit`. QA : 2365 vitest (73 feature), svelte-check 0, build OK, e2e PDF Outfit 3/3, visuel Outfit+stress+panneau premium validés. Revue 16 agents : sécu **0 finding**, 8 med/low/info **TOUS corrigés** + race complétion durcie. **Reste : commit + déploiement (sur ton go) + smoke OTP.** → [[project_etiquettes_publipostage_campagnes_2026-06-30]] + [[audit_secu_2026-06-30_etiquettes_publipostage]].
