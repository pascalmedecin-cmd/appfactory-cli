...
**Derniere mise a jour :** 2026-06-05 (Découpe Films étape 3bis-a : golden UI premium VALIDÉ + dispositif QA tolérance zéro)
...
**Prochaine attaque** : Bloc 1 - Découpe Films **portage Svelte du golden v4 VALIDÉ** (étape 3bis), **sous QA tolérance zéro**, AVANT l'étape 4 PDF. **Golden v4 validé par Pascal le 2026-06-05** (4/4 + strip OK + saisie OK) après benchmark MÉTIER (le benchmark SaaS génériques a été écarté comme hors cadre ; refait sur optimiseurs de découpe + découpe rouleau : CutTool/CutList/marker textile). Golden validé archivé : `.product-architect/decoupe/golden-validated-2026-06-05/` ; générateur `scripts/_decoupe_golden.ts` (vrai algo). **Tolérance zéro exigée par Pascal** : dérouler `.product-architect/decoupe/qa-tolerance-zero.md` (9 sections binaires) + `bash scripts/_decoupe_qa.sh` AVANT tout `done`. Acquis (data layer 24 tests, structure 4 écrans, algo, commits `bab1c38`→`142dbb9`) à ne pas refaire. → voir `memory/feedback_decoupe_ui_premium_moderne_riche.md` (RÉSOLU) + `memory/feedback_decoupe_tolerance_zero_qa.md`.
...
- [x] ~~Étape 2 - data layer (migration + types + Zod + builders, TDD)~~ - Livré 2026-06-05 (voir Livré cette session). Form actions CRUD déplacées en étape 3 (indissociables de leur page SvelteKit, accord Pascal).
- [x] ~~Étape 3 - 4 écrans `/decoupe` (structure + form actions + flag + guard + card portail)~~ - Livré 2026-06-05 (structure/données/actions solides ; **langage visuel du résultat rejeté → étape 3bis**).
- [x] ~~Étape 3bis-a - **golden UI premium validé**~~ - Validé Pascal 2026-06-05 (benchmark métier, golden v4, 4/4). Voir Livré + `golden-validated-2026-06-05/`.
- [ ] **[EXÉCUTABLE]** Étape 3bis-b - **portage Svelte du golden v4** dans les 4 écrans `/decoupe` (résultat = `optimisation/+page.svelte` en premier ; puis chantiers/produits/fiche). Refonder le langage visuel (KPI mis en scène, code couleur par pièce, badges 2 styles, chips, strip de remplissage, liste de coupe = cœur opérationnel). Retirer `PlanDecoupeSvg`/`ChuteGauge` (remplacés par le strip + KPI). Extraire les helpers de calcul UI en `.ts` purs (doctrine `.svelte`=e2e only). **Créer `tests/decoupe.spec.ts`** (barrière e2e du dispositif QA, référencée par `_decoupe_qa.sh`, absente aujourd'hui). **Sous QA tolérance zéro** (cf. ci-dessous). → `golden-validated-2026-06-05/` + `feedback_decoupe_ui_premium_moderne_riche.md`.
- [ ] **[EXÉCUTABLE]** **QA tolérance zéro (transversal)** (exigence Pascal 2026-06-05) : dérouler `.product-architect/decoupe/qa-tolerance-zero.md` (9 barrières binaires) + `bash scripts/_decoupe_qa.sh` AVANT de fermer 3bis-b, l'étape 4 et la Phase 4. Inclut invariants algo durs, e2e Playwright + axe, audit sécu Opus 0 H/C/M daté, Lighthouse, fidélité golden↔Svelte. Aucune étape `done` sans 100 % vert. → `memory/feedback_decoupe_tolerance_zero_qa.md`.
- [ ] **[BLOQUÉ - 3bis-b livrée]** Étape 4 - **checkpoint template PDF avec Pascal** (ADR-0005, AC-025) puis export PDF vectoriel client (jsPDF + svg2pdf), miroir de l'écran de résultat refondu.
...
- [x] ~~Découpe Films étape 3bis-a : golden UI premium VALIDÉ par Pascal~~ - Livré 2026-06-05 (xhigh). Benchmark **métier** (le 1er benchmark SaaS génériques Linear/Vercel/Stripe/Retool/Height/Raycast **écarté par Pascal comme hors cadre** → refait sur optimiseurs de découpe CutList/OptiCut/MaxCut + découpe rouleau **CutTool.app** quasi-identique/marker textile Gerber/Optitex). Golden v4 (résultat + saisie) régénéré par `scripts/_decoupe_golden.ts` (vrai algo), **validé 4/4** (premium + strip + saisie). Langage : KPI mis en scène (taux roi color-codé + arc + « comment c'est calculé » + économie vs pose séquentielle), code couleur par pièce, badges 2 styles, chips, strip de remplissage compact, liste de coupe = cœur. Rendu vérifié par screenshot. **Dispositif QA « tolérance zéro » créé** (exigence Pascal) : `.product-architect/decoupe/qa-tolerance-zero.md` (9 barrières binaires) + `scripts/_decoupe_qa.sh`. Golden archivé : `golden-validated-2026-06-05/`, gate signé. → `memory/feedback_decoupe_ui_premium_moderne_riche.md` (RÉSOLU) + `feedback_decoupe_tolerance_zero_qa.md`.
- [x] ~~Découpe Films Phase 3 étape 3 : 4 écrans `/decoupe` + form actions (structure/données/algo)~~ - Livré 2026-06-05 (xhigh, commits `e496d95`/`ac77969`/`142dbb9`). Fondation (flag `ffDecoupe`/guard/shell/`decoupe.css`) + 4 écrans (chantiers, base produit CRUD, fiche+saisie vitres, optimisation) branchés sur les builders. Vitest 1560, build OK. **UI du résultat rejetée 2× par Pascal (dense v1 5/10 ; minimaliste v3 pire) → refonte premium = étape 3bis.** → voir `memory/feedback_decoupe_ui_premium_moderne_riche.md`.
- [x] ~~Découpe Films Phase 3 étape 2 : data layer (migration + types + Zod + builders, TDD)~~ - Livré 2026-06-05 (xhigh, commit `bab1c38`). Migration locale 3 tables `decoupe_*` + RLS mono-tenant plat (idempotente, **NON appliquée prod** = Phase 5) ; `database.types.ts` étendu ; schemas Zod `$lib/schemas` (Zod v4 `error:`, AC-019) ; builders purs `src/lib/server/decoupe/builders.ts`. **24 tests**, Vitest 1560, svelte-check 0. → voir `.product-architect/decoupe/data-model.sql`.
