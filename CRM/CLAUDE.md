...
- [ ] **[EXÉCUTABLE - reste 6 divergences (5 medium + 1 low) ; GATE maquette pour les copies visibles]** Parité bi-marque LED ↔ FilmPro : 2 HIGH livrés 17/07 ; **reste 6** divergences de présentation (PDF liste logo FilmPro, copies vitrage `ImportModal`/`SourceSearchFields`, `activity-types` Google Places, **dropdown campagne vide = bug 2 Pascal**, hero Signaux « vitrage » = Run 7). Copies/logos visibles = « miroir exact + QA avant/après » + checklist parité par marque sur env LED peuplé. **Bug 1 Pascal (bouton import absent LED) = NON reproduit en code** → reproduire sur env LED réel d'abord. Détail file:line + fixes → `docs/ATELIER-209-SUIVI.md` § Parité bi-marque. → [[feedback_bi_marque_parity_qa_en_sortie]].
