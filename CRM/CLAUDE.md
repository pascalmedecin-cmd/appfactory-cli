...
**Prochaine attaque** : **Cohérence UI increment b - PART 2 lot 2 + c + d** (lot 1 livré `7518cc1`, flag OFF ; détail du lot 2 → backlog dev ci-dessous + `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation »). Page par page, derrière `ff_ui_coherence`, QA OFF/ON au DOM (⚠ toggle test : clé `null` pas `delete`), zéro régression, **attention aux espaces** (8px). Sinon : allumer le flag bandeau (a), ou **Run 7** veille LED (cadrage). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
