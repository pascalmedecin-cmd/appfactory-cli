...
**Prochaine attaque** : **Bloc A - couper les mails de plantage** (preview Vercel des PR Dependabot + gel complet du socle + nettoyage branches stale) : le plus urgent. **Séquence de clôture du projet refonte CRM** : A → B (Daily Email deploy) → C (Lot 3 optionnel) → **D (audit UI 360 = porte de clôture avant livraison)**. Refonte Vagues 1+2+3 livrée + déployée + activée prod (flag `ffCrmListesV2` ON fondateurs), prod = trunk `main`. **Livré cette session** : reconciliation `main` + triage Dependabot **complet** (0 PR ouverte, svelte tenu 5.55.5). → [[project_refonte_crm_cadrage_2026-06-18]].
...
### 1. Bloc A - Couper les mails de plantage Vercel + Dependabot [AUTO • high • 1 session]

- **Pourquoi** : chaque PR Dependabot déclenche une preview deploy Vercel qui plante (build cassé) → mail ; et les bumps socle cassants (svelte 5.55.10) rouvrent des PR rouges. Cause **système**, pas PR par PR (régler PR par PR = tâtonner + chaque rebase génère un mail).
- [ ] **[EXÉCUTABLE]** **Vercel** : ajouter dans `CRM/vercel.json` (committé sur `main`) → `"git": { "deploymentEnabled": { "dependabot/**": false } }`. **Globstar `**` obligatoire** (le `*` simple ne traverse pas les `/` des branches `dependabot/npm_and_yarn/CRM/...` - vérifié minimatch 10.2.5). Coupe les previews Dependabot, **garde** les previews des autres branches, **zéro risque** pour le `vercel deploy --prod` manuel (non déclenché par commit). Source : doc Vercel `git-configuration`, vérifié 2026-06-26 (champ `git.deploymentEnabled` = objet `{pattern: false}`, minimatch).
- [ ] **[EXÉCUTABLE]** **Dependabot - geler TOUT le socle (patch inclus)** : `dependabot.yml` ignore déjà socle minor/major ; ajouter `version-update:semver-patch` à `vite`, `@sveltejs/vite-plugin-svelte`, `@sveltejs/kit`, `svelte-check` (svelte déjà fait, `dc1264c`). Raison : un **patch** socle peut casser (svelte 5.55.10 cassait avec le plugin tenu `^6.2.4`). Plus aucun bump socle cassant proposé. → [[feedback_svelte_patch_coupled_vite_plugin]].
- [ ] **[EXÉCUTABLE]** **Branches stale origin** : supprimer `portail-session-1`, `veille-preflight-qa-cite-emoji`, `veille-w25-qualite-selection` (vérifiées mergées dans `main`) ; `chore/deps-ci-hygiene` = source PR #16 (`47c162e`, squashée en `b84b6f5`) → vérifier le contenu présent sur `main` puis supprimer.
- [ ] **[EXÉCUTABLE]** **Résidu** : `CRM/.product-architect/daily-email/preview-daily-email.html` (untracked, pas gitignoré) → l'ajouter au `.gitignore` (durcit contre un futur `git add -A`) ou le supprimer. Décision en session.
- **Critère de succès** : 0 PR ouverte, 0 run CI rouge, trunk vert ; **vérifier au run Dependabot suivant (lundi)** = vert ou aucune PR. Ne PAS relancer de rebase Dependabot manuellement (ça génère un mail).

### 2. Bloc B - Daily Email : déploiement prod + activation [SUPERVISÉ • medium • court]
...
### 3. Bloc C - Veille Lot 3 structurel (reste, optionnel) [AUTO • high • session dédiée]
...
### 4. Bloc D - Audit UI 360 final (PORTE DE CLÔTURE avant livraison complète) [SUPERVISÉ • xhigh • session dédiée]
...
### 5. Bloqués - Vague 4 emailing + migration socle (dépendances réelles, pas dette active) [SUPERVISÉ • xhigh • bloqué]
