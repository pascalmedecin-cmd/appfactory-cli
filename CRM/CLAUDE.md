...
- **Source de vérité de l'exécution → `docs/ATELIER-209-SUIVI.md`** (statut par run, décisions, preuves, règles non négociables, gate prod). À lire au démarrage de toute session du chantier. Le « pourquoi » figé : `~/Claude/Lab/memory/atelier-209/` (non maintenu).
- **Règles non négociables (rappel)** : Pascal valide chaque maquette dans Chrome avant tout code ; non-régression garantie (défaut `marque='filmpro'`) ; zéro dette ; tout sourcé/vérifié. Détail → SUIVI.
- **Skills design (runs 1+, annoncés au plan)** : `redesign-skill`, `soft-skill`, `theme-factory`, filtre `ANTI-AI-SLOP.md`. **État : Run 1 déployé prod ; Run 2 CÂBLAGE COMPLET + testé sur `run2-marque` (NON déployé)** - cloisonnement de bout en bout, leak-test 10/10, Vitest 2562, audit 0 C/H, revue 18 agents. **Reste 2 gestes Pascal (gate prod)** : validation Chrome des 2 marques + go migration/déploiement. V6 Hunter / V7 Pingen en attente comptes Pascal.
...
**Dernière mise à jour :** 2026-07-15 (**Run 2 : câblage cloisonnement COMPLET + testé, branche `run2-marque`, NON déployé** ; reste gate prod Pascal → § Chantier en tête + `docs/ATELIER-209-SUIVI.md`). Trunk = `main` ; Flag `ffCrmListesV2` **ON fondateurs**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : comptes Hunter & Pingen + variable Daily Email (§ Chez Pascal). **Prochain bug :** #001.
...
- [x] ~~**Fix gap avatar→raison sociale sur Entreprises (qualité pro)**~~ - 2026-07-15 (branche `run2-marque`). Cause racine (pré-existant, PAS Run 2) : colonnes à largeur fixe (logo/chevron) absorbaient l'excédent `table-fixed` (les % ne totalisaient pas 100 %) → avatar 40px flottant dans une cellule de 101px (gap 61px premium / 261px non-premium). Fix : avatar + nom dans la même cellule (flex `crm-id`, gap **8px** stable) + colonne logo retirée + % ajustés ~100 %. QA avant/après (premium), svelte-check 0, tests 31/31. Isolé à Entreprises. Note : `resizable` colonnes incohérent (backlog).
- [x] ~~**Atelier 209 - Run 2 : CÂBLAGE cloisonnement bi-marque COMPLET + testé (branche `run2-marque`, NON déployé)**~~ - 2026-07-15 (ultracode, workflows). Cloisonnement de bout en bout : `locals.marque` threadé, 7 hubs + ~50 fichiers prod filtrés/marqués, export CSV dynamique verrouillé (fuite PII), flux public validation scopé par token, `contact_suggestions` par héritage. Chrome teinté (`BrandSwitcher`, pastille+filet, `[data-marque=led]`), golden Inter (Lot B). **Preuve DoD : leak-test 10/10 base réelle** (vrais hubs) ; Vitest 2562, svelte-check 0, build OK. Fix archi : FK composite = embed PostgREST ambigu → DROP FK simples. Revue adversariale 18 agents (6 findings corrigés, dont 5 endpoints oubliés du fan-out). **Reste gate prod (Pascal)**. → [[project_atelier_209_run2_cablage_2026-07-15]] + [[audit_secu_2026-07-15_atelier209_run2_marque]].
- [x] ~~**Atelier 209 - Run 2 : gate design + fondation cloisonnement**~~ - 2026-07-15 (superseded par le câblage ci-dessus). Migration `marque` 12 tables + FK cohérence + RPC + seed, 9/9 tests. → [[project_atelier_209_run2_fondation_2026-07-15]].
