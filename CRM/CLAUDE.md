...
**Prochaine attaque** : Bloc 1 - Smoke iPhone réel Pascal post-déploiement S192bis (commit `040f9f4` poussé : ScorePill 13/10 clampée, /dashboard/couts masqué mobile, tabs scroll-snap+fade, badge chaleur mobile, AideDiagram lightbox) + capture screenshot Lot 3 "caractères incorrects" signaux pour investigation (~20 min SUPERVISÉ). Flag `ff_crm_mobile_v2` posé pour `pascal@filmpro.ch` ce tour (Admin API). Une fois smoke validé → débloquage Phase 4 QA 360.

### 1. Smoke iPhone réel Pascal post-S192bis [SUPERVISÉ • low • ~20min]

- **Pourquoi** : Pascal a remonté 6 bugs mobile lors du smoke initial (note 13/10, barre du haut hors écran 6 pages, badge chaleur cassé, page coût API pas propre, diagrammes aide illisibles, caractères incorrects signaux). 5 lots fixés et déployés (commit `040f9f4`). Validation visuelle requise pour fermer les fix et débloquer Phase 4. Lot 3 (caractères incorrects) attend un screenshot pour fixer le root cause.

- **Prérequis** : Vercel auto-promote terminé (~2-3 min après push). Login obligatoire avec `pascal@filmpro.ch` pour que le JWT contienne `ff_crm_mobile_v2:true` (déconnexion+reconnexion si session active pré-flag).

- [ ] **[EXÉCUTABLE]** Smoke F1 (Signal→prospect→relance) sur iPhone : ouvrir `/signaux` (drawer Keywords masqué), tap signal → SlideOut détail (score max 10/10, jamais 13/10), créer prospect, retour `/prospection` (cards visibles, pas de table, badge chaleur lisible avec icône + valeur).
...
- [ ] **[EXÉCUTABLE]** Spot-check pages MOBILE-OK : `/reporting` (table pipeline + export CSV masqués, bandeau « optimisé ordinateur » visible), `/dashboard/couts` (contenu masqué + bandeau "optimisé desktop" visible), `/` Dashboard (pas d'animation stagger), `/veille` + `/aide` (responsive existant intact, **bouton zoom top-right sur chaque diagramme aide** → modal fullscreen avec close).
- [ ] **[EXÉCUTABLE]** Validation tabs best-in-class sur 6 pages (Signaux, Contacts, Prospection, Entreprises, Pipeline, Reporting) : la barre d'onglets scrolle horizontalement avec dégradé fade gauche+droite (jamais hors écran sans signal visuel), tap sur un onglet le centre, pas de chevron explicite. Si KO : noter quelle page + comportement vu.
- [ ] **[EXÉCUTABLE]** Capturer screenshot iPhone "caractères incorrects" signaux (Lot 3 reporté) : ouvrir `/signaux`, repérer la card avec texte mal rendu, screenshot + envoyer dans le chat de la prochaine session. Sans visuel je ne peux pas identifier le root cause (DB rows samplées saines : apostrophes droites, accents UTF-8 préservés).
...
- [ ] **[BLOQUÉ - smoke OK]** Playwright E2E 3 user flows critiques (F1/F2/F3) sur preset `iPhone 14 Pro Max`. Tests `tests/mobile-flows.spec.ts`. Inclut e2e PipelineMobileAccordion (helpers `.ts` 22 tests verts S192, e2e Playwright reportée ici).
...
- **[WATCH] Lot 3 — Signaux details "caractères incorrects" reporté** : Pascal a vu un texte mal rendu sur cards Signaux iPhone, mais sans screenshot je ne peux pas identifier le root cause. DB samples lus : apostrophes droites, accents préservés, pas de mojibake. Causes possibles non vérifiées : (a) highlight chunks NFD offset cassé sur certains caractères, (b) double-encoding HTML entity dans une source SIMAP, (c) char invisible Unicode, (d) font qui ne rend pas certains glyphes. Tâche prochaine session : capturer screenshot iPhone + me l'envoyer. Voir tâche **[EXÉCUTABLE]** Bloc 1 pour la capture.
...
- [x] ~~S192bis - Fix 5 lots bugs mobile post-smoke iPhone Pascal (ScorePill 13/10 clamp + Tabs scroll-snap+fade + MobileEntityCard badge chaleur + /dashboard/couts mobile + AideDiagram lightbox)~~ - Fait 2026-05-26 (xhigh, ~2h ; Score 3/4 multi-étapes cross-pages + itération coûteuse + UX/UI non-mesurable partiellement). **Trigger** : Pascal lance smoke iPhone post-S192, remonte 6 bugs (note 13/10, barre tabs hors écran 6 pages, badge chaleur cassé, page coût API pas propre, diagrammes aide illisibles, caractères incorrects). **Périmètre livré** : (a) `clampDisplayScore(raw, maxPoints)` helper dans `signauxFormat.ts` + 7 tests ; `signaux/+page.svelte` SlideOut + `SignauxCards.svelte` consomment le helper (display only, DB inchangé pour tri). (b) `scorePillIcon(label)` helper dans `mobile-entity-card.helpers.ts` + 4 tests ; `MobileEntityCard.svelte` rend icône + valeur clampée ; base `.signal-score-pill` ajoutée à `app.css` (padding/radius/font/gap globaux). (c) `Tabs.svelte` best-in-class : `scroll-snap-type: x mandatory` + `scroll-snap-align: center` par tab + `mask-image` fade gauche+droite 16px + `$effect` auto-scrollIntoView({inline:'center'}) onglet actif (pattern Vercel/Linear, pas de chevron old-web). `prefers-reduced-motion` honoré. (d) `/dashboard/couts/+page.svelte` wrap content `.desktop-only` + bandeau `.mobile-only-banner` (pattern S191 /reporting tab=export). (e) `AideDiagram.svelte` refactor avec snippet `svgContent` (zéro duplication markup) + bouton zoom top-right (Icon `open_in_full`, tap 44px) + modal fullscreen `role="dialog" aria-modal` avec backdrop blur + close (X + click outside + Escape) ; `$effect` SSR-safe pour scroll lock body (cf. mémoire `feedback_svelte5_ondestroy_ssr_window_undefined.md`). **Lot 3 reporté** : "caractères incorrects" signaux non identifiable sans screenshot Pascal (DB samples sains via Supabase MCP `SELECT description_projet`). **Cockpit** : entries `b8a43ba1` (helpers Pipeline déjà couverts S192) + `01b8581e` (flag `ff_crm_mobile_v2:true` posé pour pascal@filmpro.ch via Supabase Admin API HTTP 200) delivered avec outcome. **Validation** : `npm run check` 0 erreur (28 warnings baseline inchangé), `npm test` 1404 passed (+11 nouveaux : 7 clampDisplayScore + 4 scorePillIcon) — **17 failures `hooks.server.test.ts` pré-existantes** (Sentry feat `d78ab37`, confirmé via stash + checkout HEAD~1) à fixer hors-scope (cf. WATCH S192bis). Build prod `21.46s` adapter-vercel OK. **Commit pushed** : `040f9f4 fix(crm): bugs mobile post-smoke iPhone Pascal (S192bis, 5 lots)` (11 fichiers, +521/-176). **Restant** : validation visuelle Pascal smoke iPhone post-déploiement (Bloc 1 Prochaine session) + screenshot Lot 3 pour fix futur.
...
- [x] Détail livraisons S187-S189 (Signaux V2/V3/V4 + scoring sans temporalité + drawer mots-clés + bug SSR onDestroy) : `archive/2026-05-25-sessions.md`
