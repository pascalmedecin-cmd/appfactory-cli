...
**Prochaine attaque** : **Lot 2 - Prospects + Pipeline** - cœur du nouveau workflow validé (statuts vide/à contacter/écarté, « à contacter » -> pipeline, retrait priorité). Workflow global + specs Lots 2/3 → [[project_refonte_signaux_prospects_campagnes_2026-07-01]]. **Lot 1 (Signaux) LIVRÉ PROD ce jour** (`db6182e`).
...
- [ ] **[EXÉCUTABLE]** Lot 2 - Prospects + Pipeline (SUPERVISÉ • xhigh • ~1,5 session) : statuts prospect simplifiés VIDE/à contacter/écarté (migration CHECK + défaut) ; « à contacter » crée une opportunité liée au prospect (NOUVELLE colonne `opportunites.prospect_lead_id`) et le sort de prospection ; « écarté » le masque + vue « Réactiver un écarté » ; anti-réimport CONSERVÉ ; sidepane 2 boutons (à contacter + écarté, garder Enrichir tel) ; « Convertir en client » déplacé au pipeline ; édition statut inline ; retrait priorité + filtre température ; fix lien SIMAP prospect périmé. **Payload complet** → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
...
- [ ] **[EXÉCUTABLE]** Réparer le stack Supabase local (tests jetables) : migration `20260416120000_media_library_fal.sql` référence `media_library` avant sa création dans `supabase db reset` -> le stack ne démarre pas from-scratch (contourné au Lot 1 par postgres jetable). Corriger l'ordre/dépendance. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]].
