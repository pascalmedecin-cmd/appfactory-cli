...
### 0. Refonte veille éditoriale - Lots 1+2 LIVRÉS, Lot 3 bloqué [LIVRÉ 2026-06-22]

- **Pourquoi** : audit 360 + baseline empirique. Vrai défaut MESURÉ : veille famélique ~3 items/édition, **57 % rejetés par le filtre**, mix géo inversé (77 % monde), mono-source glassonweb 55 %. Cause racine W25 « 0 item » prouvée : cross-check verbatim rejette la voix d'analyste + URLs mal-formées.
- **Payload** → [[project_veille_refonte_lots12_livree_2026-06-22]] (livré) + [[project_veille_refonte_qualite_cadrage_2026-06-19]] (cadrage) + métier [[project_filmpro_metier]].
- [x] ~~**Lot 1 - qualité éditoriale**~~ - LIVRÉ `de05779` : persona analyste sénior, so-what structuré + filet anti-générique déterministe, actionability objective, direction éditoriale + few-shot, email = vrai brief (édito + action_directe + impacts, EUR retiré du sujet), bouton prospection réparé (chips zefix-only), page index (so-what remonté, line-clamp retiré).
- [x] ~~**Lot 2 - pertinence & volume**~~ - LIVRÉ `de05779` : cross-check faits/interprétation (sépare faits vérifiés mot-pour-mot du so-what, **cause racine W25**), récupération d'URL mal-formée (citations web_search), mix romand garanti + canari de dérive. (Réécriture du chiffre divergent NON faite = reportée Lot 3, trop risquée ; patsnap déjà hors code.)
- [ ] **[BLOQUÉ - après mesure W26+]** **Lot 3 - structurel (optionnel)** : mémoire longue + tableau de bord qualité ; boucle de feedback ; PDF de marque partageable (`filmpro-pdf`) ; pont veille→action fin (tâche/opportunité) ; élargir sources (OFS, associations, salons) ; décomposer le mono-appel LLM (si plafond persiste) ; nettoyage enum RegBL ; **réécriture chiffre divergent** + **résiduel LOW** (fait qualitatif fabriqué ni typé ni exclusivité ni contredit = cas étroit, voir audit). → [[project_veille_refonte_lots12_livree_2026-06-22]].
...
- [x] ~~**Refonte veille éditoriale Lots 1+2**~~ - 2026-06-22 (ultracode/xhigh, run autonome). Commit **`de05779`** poussé `main` (20 fichiers, +1250/-184). Cause racine W25 « 0 item » prouvée sur les 7 vrais items : cross-check verbatim tuait la voix d'analyste (4 locaux vivants jetés) + 3 URLs monde mal-formées. **Recalibrage anti-hallu faits/interprétation** (gate déterministe INCHANGÉ, W18 intacte), récup URL (citations web_search), mix romand + canari, persona analyste + so-what structuré + few-shot, email brief, chips zefix-only, page index, décodage entités num. **Revue adversariale 3 sceptiques → 2 brèches (so-what non vérifié HIGH, superlatif fabriqué MEDIUM) → corrigées + RE-VÉRIFIÉES fermées** sans sur-rejection W25 ; bug/sécu/contrats 0 C/H/M. Rejeu W25 live : 0→6 items. 1927 Vitest verts, svelte-check 0. → [[project_veille_refonte_lots12_livree_2026-06-22]] + [[audit_secu_2026-06-22_veille_refonte_lot1_lot2]]. **Page web pas redéployée (Vercel manuel) ; cron W26 OK depuis main.**
