...
**Prochaine attaque** : **Bloc A - refonte serveur Entreprises** (EXÉCUTABLE, MOYEN ~1 session, plan + invariants prêts) - le livrable codable le plus self-contained ; Bloc B migration socle aussi EXÉCUTABLE mais NON TRIVIAL (peer-deps). Refonte CRM 1+2+3 live, **Bloc D CLOS** (`cd5426e`). 6 tâches → 4 blocs + checklist ci-dessous. **Prod = `main` auto-déployée** → push **après validation** (pattern Bloc D : preview → validation → push). → [[project_refonte_crm_cadrage_2026-06-18]] + [[project_bloc_d_audit_360_PROGRESS]].

> **État vérifié 2026-06-28** : les 6 tâches sont **toutes encore à livrer** (aucune faite/périmée). Baseline saine : **2301 tests verts**, build OK, knip cadré (voir Réserve). **1 bloc = 1 session.** Golden cadrage refonte → [[project_refonte_crm_cadrage_2026-06-18]] + `CRM/.product-architect/refonte-vague3/golden-vague3-v1.html`.

#### Blocs codables en autonomie

##### A. Refonte serveur page Entreprises [EXÉCUTABLE • MOYEN • ~1 session]

- [ ] **[EXÉCUTABLE]** `/crm/entreprises` charge `select('*')` + `.limit(500)` puis filtre/pagine **100 % côté client**. Refonte = filtre+pagination **serveur** (URL params) façon `/prospection`, `count:'exact'` par onglet, KPI globaux séparés. Ampleur **MOYENNE** (pattern `/prospection` réplicable ~85 %), **aucun blocker système** (le « différé si volume grossit » du 27/06 est levé par le mandat « enchaîne en autonome » du 28/06).
- **Invariant à NE PAS casser** : compteurs d'onglets ET KPI itèrent aujourd'hui sur la liste complète → un `.limit()` nu, ou un count avec `.limit()`, les casse **silencieusement**. Règles : counts via requêtes `count:'exact'` séparées **sans limit** ; KPI sur requête séparée **sans filtre d'onglet ni pagination** ; **conserver telles quelles** les requêtes `contacts`+`opportunites` (SlideOut fiche). → `memory/project_bloc_d_findings_restants_handoff.md` + [[feedback_kpi_premium_global_aggregates_not_paginated_page]].
