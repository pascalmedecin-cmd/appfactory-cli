...
**Prochaine attaque** : si on est **vendredi 26/06 (après le cron W26)** → Bloc 0 **reconciliation `main`** (dated must-do). Sinon → Bloc 1 **Vague 3.3 Dashboard temporel** (dernier écran build de la refonte, golden validé, exécutable). **Refonte CRM Vagues 1+2+3 DÉPLOYÉE + ACTIVÉE 25/06** (Option A, flag `ffCrmListesV2` ON pour `pascal@`+`antoine@`) : toute la refonte (Signaux/Pipeline/Entreprises/Prospection premium + Campagnes) est LIVE pour les fondateurs, plus seulement Campagnes. Prod = branche `deploy-campagnes-editeur` (éditeur veille + Campagnes), `main` non touché. → [[project_module_campagnes_vague32_2026-06-22]] + [[project_refonte_crm_cadrage_2026-06-18]].
...
- **Pourquoi** : golden Vague 3 validé Chrome. Vagues 1+2+3.1 + Campagnes 3.2 **LIVRÉS, DÉPLOYÉS ET ACTIVÉS** (25/06, flag ON fondateurs). Reste **Dashboard (3.3)** + **Emailing (Vague 4, bloqué)**.
...
- **Fait + DÉPLOYÉ 25/06** : golden validé ; Vagues 1+2 + 3.1 Signaux (`5e0c369`) + export CSV (`6626818`) + 3.2 Campagnes (`c905952`) **tous live pour `pascal@`+`antoine@`** (flag `ffCrmListesV2` ON, Option A). Revues 0 C/H/M.
- [ ] **[EXÉCUTABLE]** **Vague 3.3 Dashboard temporel** : refonte `/crm/+page.svelte` façon Capsule (En retard / Aujourd'hui / Cette semaine + fil d'activité « Ce qui s'est passé »), flag `ffCrmListesV2`, golden validé. **Une fois codé : déployer sur la branche `deploy-campagnes-editeur` (prod actuelle) jusqu'à la reconciliation Bloc 0, OU après reconciliation directement sur `main`.**
...
- **[WATCH] Refonte CRM Vagues 1+2+3 ACTIVÉE 25/06 pour les fondateurs** : flipper `ffCrmListesV2` ON a activé **toute la refonte** (Signaux/Pipeline/Entreprises/Prospection premium + Campagnes), pas que Campagnes (flag unique partagé). Pascal a validé Campagnes + Signaux + éditeur veille (« tout bon »). Les autres surfaces premium (cartes Pipeline, lignes riches Entreprises/Prospection) ne sont pas explicitement re-validées une à une → surveiller à l'usage. **Bug 5 résiduel** : menu `...` du slideout Signaux (ouverture vers le haut en conteneur scrollable) - à confirmer/corriger s'il re-saute. Rollback instantané = flipper le flag OFF (`raw_app_meta_data`).
- **[WATCH] W26 (vendredi 26/06) = 1er run cron sur le sourcing `9bef41b` (~120 sources) + 1er brief brandé réel antoine@**. Surveiller GHA : `keptByTrust`/`keptByTrustTitles`, densité (cible 8-12, W25=2), part locale (↑ vs 33%), 0 `<cite>`/emoji/underscore. **NB : le cron tourne sur `main` → il N'utilise PAS les 238 sources de l'éditeur veille (qui vivent sur la branche `deploy-campagnes-editeur`, web-only). Le passage à 238 = à la reconciliation post-W26 (Bloc 0).** → [[project_veille_sourcing_w26_2026-06-23]].
