...
**Phase 0 - BUG PRIORITAIRE (avant tout smoke test, ordre Pascal 21/07)**

- [ ] **[EXÉCUTABLE]** BUG #001 login OTP : 1re connexion d'un email autorisé neuf envoie « Confirm Your Signup » au lieu du code : la 1re connexion d'un email autorisé mais sans compte (`pascal@lamaisoncreativedirection.ch`, domaine cible Atelier 209) fait créer le compte par `signInWithOtp` + envoie un magic-link « Confirm Your Signup » au lieu du code OTP 6 chiffres. Corriger AVANT tout smoke test (config templates email Supabase Auth / chemin new-user OTP). Aussi : poser les flags premium sur les comptes cible `@lamaisoncreativedirection.ch`. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
