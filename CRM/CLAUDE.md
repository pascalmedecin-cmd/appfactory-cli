...
- **État : Run 1, Run 2 ET Run 3 DÉPLOYÉS EN PROD.** **Run 3 déployé le 2026-07-16** : import de liste (source `manuel`), onglet « Ma liste », D3 (secteurs source unique marque-aware) + D4 (source `manuel`). D4 appliquée en prod (CHECK `manuel`, 312 leads préservés), merge `4e3f149`, smoke vert. Preuves : Vitest 2801 + 15 intégration base réelle, svelte-check 0/0, build OK ; revue 8 findings corrigés, 0 Critical/High. **Reste = sign-off visuel Pascal, sur la prod directement** (personne ne l'utilise encore). V6 Hunter / V7 Pingen en attente comptes Pascal. Détail → `docs/ATELIER-209-SUIVI.md`.
...
**Prochaine attaque** : **Sign-off visuel Pascal du flux d'import Run 3, SUR LA PROD** (`filmpro-portail.vercel.app`, décision Pascal 16/07 : personne ne l'utilise encore). Ouvrir Prospection → bouton « Importer une liste » (onglet Entreprises) ou onglet « Ma liste » → déposer un CSV (format scrape Google Maps NOM/ADRESSE/NPA/VILLE/TELEPHONE/CATEGORIE/SITE WEB/EMAILS) → vérifier colonnes auto-reconnues → aperçu → importer → les prospects apparaissent dans « Ma liste ». **Si un défaut visuel/UX** : le corriger (Run 3 est déployé mais non validé à l'œil). **Ensuite** : **Run 4** (Hunter, bloqué compte Pascal) / **Run 7** (veille LED - cadrage) ; **[à valider Pascal] mots-clés secteur LED** de `secteurs.ts` (marqués `[à valider]`, valeurs de départ sourcées). Détail → [[project_atelier_209_run3_import_liste_2026-07-16]] + `docs/ATELIER-209-SUIVI.md`.
