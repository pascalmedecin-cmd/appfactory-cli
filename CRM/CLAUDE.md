...
**Dernière mise à jour :** 2026-07-18 (**parité bi-marque : 5 divergences LIVRÉES + DÉPLOYÉES prod** - 2 gate-free `96dc026` (dropdown vide + logo PDF) + **copies métier LED #4/#5/#6 `ae438e2`** (catégories Google LED marque-aware + placeholders signalétique/stand/enseigne, maquette validée Pascal). Vitest 2860, svelte-check 0/0, revue adversariale (1 CONFIRMED fixé), QA réelle 2 marques. **Reste 2 divergences** : bug 1 (bouton import absent LED = repro env LED réel) + #8 hero Signaux = Run 7. → `docs/ATELIER-209-SUIVI.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : **Parité bi-marque - 5/7 divergences LIVRÉES + DÉPLOYÉES 18/07** (`96dc026` bug 2 + logo PDF ; `ae438e2` copies LED #4/#5/#6 : catégories Google LED + placeholders signalétique/stand/enseigne, maquette validée Pascal, QA réelle 2 marques). **Reste parité = 2** : **bug 1** (bouton « Importer une liste » perçu absent sur LED - **NON reproduit en code**, reproduire sur un **env LED réel peuplé** avant tout code) + **#8** hero Signaux « vitrage » = **Run 7**. Autres tracks disponibles : Cohérence UI PART 2 lot 2+c+d (flag `ff_ui_coherence` OFF), flag bandeau (a), **Run 7** veille LED (cadrage, absorbe #8). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
Le « **backlog dev** » ne liste QUE l'actionnable-par-Claude sans dépendance externe (gestes Pascal → § Chez Pascal ; attentes datées → § Parking ; idées → Watch). No-debt strict ; titre bloqué = `[BLOQUÉ - {qui/quoi débloque}] {action}`. → [[feedback_backlog_wip_anti_gonflement]]. Baseline saine (**2827 verts**, build OK).

### Backlog dev (actionnable par Claude)

- [ ] **[BLOQUÉ - toi : reproduire bug 1 sur un env LED réel peuplé]** Parité bi-marque - reste 2/7 : 5 divergences LIVRÉES (2 HIGH 17/07 + #3 logo PDF + bug 2 dropdown `96dc026` + copies LED #4/#5/#6 `ae438e2`). **Reste : bug 1** (bouton « Importer une liste » perçu absent sur LED - **NON reproduit en code** : le bouton est identique aux 2 marques, gate = onglet seul ; causes probables = onglet « Ma liste »/build-cache/fenêtre < md) → reproduire sur un env LED réel AVANT tout code. **#8** hero Signaux « vitrage » = plié dans **Run 7** (veille LED). Détail → `docs/ATELIER-209-SUIVI.md` § Parité bi-marque. → [[feedback_bi_marque_parity_qa_en_sortie]].
...
- [x] ~~**Parité bi-marque - copies métier LED #4/#5/#6 marque-aware DÉPLOYÉES prod (`ae438e2`)**~~ - 2026-07-18 (ultracode). Maquette copies LED validée Pascal (`.atelier-209/parite-copies-led/`). #5 : `activity-types.ts` marque-aware (`ACTIVITY_TYPES_LED` 7 cibles validées + union serveur). #4/#6 : `prospection-copies.ts` source unique (FilmPro **byte-identique**, LED signalétique/stand/enseigne) câblée dans `SourceSearchFields`+`ImportModal` (prop marque, threadée aux 4 sites). Fix revue (1 CONFIRMED) : `$effect` re-ancre la clé d'activité à la bascule à chaud. SIMAP/RegBL déjà coupés V5. Vitest **2860** (+13), svelte-check 0/0, QA réelle 2 marques (7 catégories LED + placeholders rendus navigateur). **Reste parité : 2** (bug 1 repro env LED + #8 Run 7). → [[feedback_bi_marque_parity_qa_en_sortie]] + `docs/ATELIER-209-SUIVI.md`.
- [x] ~~**Parité bi-marque - 2 divergences gate-free (logo PDF liste + dropdown vide) DÉPLOYÉES prod (`96dc026`)**~~ - 2026-07-18 (ultracode). #3 : logo PDF liste prospects marque-aware - `src/lib/pdf/ledstudio-logo.ts` (fragment SVG **verbatim** de l'asset, byte-identique prouvé) + dispatcher `marque-logo.ts` ; `marque` threadé (défaut filmpro = **byte-identique**) aux 2 call sites. Bug 2 : `MultiSelectDropdown` prop `emptyLabel` + `{:else}` → 0 campagne LED = « Aucune campagne » (brand-agnostic). Vitest **2847** (+9), svelte-check 0/0, revue adversariale 4 lentilles **0 finding**, complétude call sites déterministe, **QA réelle 2 marques** (PDF svg2pdf LED magenta net + FilmPro inchangé + dropdown vide/peuplé rendus navigateur), smoke prod vert. **Reste 4** divergences (Pascal-gated). → [[feedback_bi_marque_parity_qa_en_sortie]] + `docs/ATELIER-209-SUIVI.md`.
- [x] ~~**Parité bi-marque LED ↔ FilmPro - les 2 HIGH corrigés + déployés prod (`2b27819`)**~~ - 2026-07-17 (ultracode). Page validation externe pilotée par marque + scoring marque-aware (LED score son secteur au lieu de 0 ; FilmPro inchangé). Vitest 2838, QA réelle 2 marques, audit sécu 0 H/C, bug-hunter 0 bug. Reste 6 divergences. → [[feedback_bi_marque_parity_qa_en_sortie]] + `docs/ATELIER-209-SUIVI.md` § Parité bi-marque.
- [x] ~~**Cohérence UI - increment b PART 2 lot 1 (états vides + recherches)**~~ - 2026-07-17 (ultracode, `7518cc1`, flag OFF). 7 états vides→`EmptyState` + 5 recherches→`SearchInput` (7 fichiers page), component-swap OFF byte-verbatim. Inventaire + routage fan-out + revue adversariale 4 dims → 2 findings réels (fix ancrage recherche campagnes `.coh-search` ; a11y clear-focus différé, non flag-gated). QA OFF/ON au DOM, svelte-check 0/0, Vitest 2827. Reste lot 2. → [[project_coherence_ui_increment_b_part2_2026-07-17]] + `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - part 1 (`3976d6e`) + mockup b/c/d + bandeau 10 pages**~~ - 2026-07-17 (ultracode, flags OFF). Part 1 : boutons→`.ws-btn`/pastilles radius-full/recherche 40px. Mockup b/c/d validé Chrome (direction + normes « attention aux espaces » gravées). Bandeau de page complet (10 pages, kicker 12px, `ff_page_bandeau` OFF, `5e0eea3`→`e60f1b6`). → [[project_coherence_ui_increment_b_part1_2026-07-17]] + `docs/COHERENCE-UI-BANDEAU.md`.
...
- **[WATCH] Parité bi-marque LED ↔ FilmPro (18/07)** : **5/7 divergences corrigées + déployées** (2 HIGH + #3 logo PDF + bug 2 dropdown + copies LED #4/#5/#6). **Reste 2** : bug 1 (bouton import perçu absent LED, non reproduit en code → repro env LED réel) + #8 hero Signaux « vitrage » = Run 7. → backlog dev « Parité bi-marque » + `docs/ATELIER-209-SUIVI.md` § Parité bi-marque + [[feedback_bi_marque_parity_qa_en_sortie]].
- **[WATCH] Bug 1 « bouton import absent sur LED » non reproduit en code (17/07)** : bouton `Importer une liste` identique pour les 2 marques (gate = onglet seul, `data.tab !== 'maliste'` ; onglet défaut = config statique non marque-aware). Cause probable : onglet « Ma liste » (bouton = CTA bleu), build/cache, ou fenêtre < `md` (`hidden md:inline-flex`). Reproduire sur env LED réel avant tout code.
...
- **[WATCH] Gotchas techniques persistants** (détail en mémoire) : jamais `rm package-lock.json` (CI Dependabot → [[project_fix_deps_ci_vercel_2026-06-22]]) ; Svelte 5 `onDestroy` en SSR → cleanup via `$effect` ([[feedback_svelte5_ondestroy_ssr_window_undefined]]) ; réactiver une source V5 coupée = re-vérifier Zod/quota/rate-limit/anti-hallu ([[audit_secu_2026-06-07_v5_signaux_prospection]]) **+ scoring marque-aware** (regbl/simap volontairement NON threadés : sources coupées, métier construction=FilmPro ; si réactivées en contexte LED, câbler `marque` sinon sur-score `+3 construction` stocké ≠ breakdown LED).
