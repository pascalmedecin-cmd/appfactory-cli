...
**Dernière mise à jour :** 2026-06-30 (refonte exhaustive Centre d'aide LIVRÉE prod `d402fe7`, QA tolérance zéro verte ; détail → § Livré). Trunk unique = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`** (Vagues 1+2+3 live). Prod = `filmpro-portail.vercel.app` : **push `main` auto-déploie en production** (vérifié 26/06). **Prochain bug :** #001.
**Session courante :** 2026-06-30 - **refonte exhaustive de la page Aide** (`/crm/aide`). Contenu remis à jour de bout en bout (portail multi-outils, Campagnes, Entreprises serveur, éditeur veille, Daily Email, 6 crons, Prospection V5, Coûts API recadrés, Log, auth OTP) + UI premium (masthead, rail sommaire, profondeur, raccourci `/`) + fix 2 diagrammes pré-existants (flèches invisibles). LIVRÉ prod `d402fe7`. QA tolérance zéro verte (2327 vitest, svelte-check 0, build OK, **gate a11y permanent axe 4/4**, fact-checker adversarial 0 finding, sécu 0 C/H/M). **À FAIRE Pascal** : smoke OTP authentifié de `/crm/aide` (3 niveaux) ; (gestes en attente : smoke Découpe+PDF, allumer Daily Email). Détail → § Livré.
...
- [x] ~~**Refonte exhaustive du Centre d'aide (`/crm/aide`) : contenu à jour + UI premium**~~ - 2026-06-30 (**LIVRÉ PROD** `d402fe7`, ultracode/xhigh). Demande Pascal : « revoir exhaustivement toute la partie aide, à jour des derniers changements CRM + UX/UI, rubrique claire, UI premium ». Méthode : workflow de découverte (7 lecteurs, état réel de chaque écran) → rédaction main-thread → workflow de vérification adversariale (4 lentilles). **Contenu** (fact-checker 0 finding) : portail multi-outils + diagramme « tu es ici » ; fiche Campagnes (N-N) ; Entreprises 100 % serveur ; éditeur veille + génération GitHub Actions ; Daily Email (récap fondateurs, 0 email prospects) ; **6 crons** réels (avant 5) ; Prospection V5 (SIMAP/RegBL retirées du manuel, quota Google Places) ; Coûts API recadrés (veille IA, pas images) ; Log ; auth OTP 6 chiffres. **UI premium** (golden v9, 0 animation scroll) : masthead + badge niveau, rail sommaire actif, profondeur sections/diagrammes, raccourci `/`, typo/espacements calés golden. **Fix collatéral** : 2 diagrammes pré-existants (cycle, veille) avaient leurs flèches invisibles (tête peinte sous la boîte opaque) → corrigé. **QA** : 2327 vitest, svelte-check 0, build OK, **gate a11y permanent** (`tests/aide-a11y.test.ts` + `playwright.aide.config.ts`, axe 4/4, 0 débordement 375px), sécu 0 C/H/M. Zéro dette/orphelin/code mort (knip clean). 8 fichiers (6 modif + 2 gate). **Clôture zéro-dette (Pascal « livre 100 % »)** : (1) garde-fou prerender rendu déterministe (`crm/aide/+page.ts` `prerender = false`) ; (2) 3 tests e2e portail (AC-001/003/024) rendus **robustes au flag** (Découpe + premium) - ils étaient figés sur l'état flags-OFF, périmé depuis la sortie Découpe + Vague 3 ; suite portail **34/34** verte. → [[project_refonte_aide_crm_2026-06-30]] + [[feedback_svg_flow_arrow_paint_order]] + [[feedback_e2e_tests_robustes_au_feature_flag]].
- [x] ~~**Refonte UI Découpe Films (diagrammes + hiérarchie typo + lignes-cartes + header/KPI unifiés)**~~ - 2026-06-30 (**LIVRÉ PROD** `0af198d`, ultracode/xhigh). Demande Pascal : diagrammes « pas jolis/lisibles » + hiérarchie texte. **Golden validé du 1er coup** (workflow 3 variantes → jugement 4 lentilles → synthèse). Diagrammes refondus = **échelle PARTAGÉE** entre films + cote laize/règle longueur + pièces étiquetées V1/V2/mono + chute hachurée + légende synchronisée (`diagramFilms`/`makeVitreRef`, PDF `stripGeometry` intact). Liste Chantiers → lignes-cartes groupées + recherche. Un seul header/KPI sur 5 écrans. **QA tolérance zéro VERTE** (2327 tests, e2e 13/13 axe 0 violation 5 écrans, sécu 0 C/H/M/L, CLS 0, fidélité golden sur rendus réels). 2 bugs fixés (`:global()` invalide en CSS plain → icône search géante ; libellé d'axe tronqué films courts). → [[project_decoupe_refonte_ui_2026-06-30]] + [[feedback_global_invalid_in_plain_css_file]].
- [x] ~~**Re-vérification délog 7j PROUVÉ en prod + re-audit 360 indépendant (0 faille)**~~ - 2026-06-29 (ultracode/xhigh, **0 code modifié**). « Pas de délog » = **timing** (session < 7 j, plafond compté depuis le dernier login), pas un bug : **prouvé en prod** (session mintée sans OTP + `curl /crm`, 3 scénarios isolés → `login_at` absent/8j = `303 expired`, frais = `200`). `+layout.server.ts` racine force le hook à chaque nav (SPA incluse). Re-audit 360 (12 agents, 5 axes + sceptiques) : **7 Low réfutés → 0 exploitable (PASS)**. 2317 verts. Preuve manuelle : DevTools → supprimer `login_at` → recharge → délog. → [[audit_secu_2026-06-29_session_delog_7j]].
- [x] ~~**Fix expiration session 7j (« pas de délog ») + audit sécu 360**~~ - 2026-06-29 (**LIVRÉ PROD** `c7545ed`). Plafond 7j était **code mort** (cookie `login_at` maxAge 7j s'auto-supprimait pile au seuil, session survivait sur `sb-*` 400j) → fix fail-secure (absence/falsification = expiré) + 2 tests, audit 360 (23 agents) **0 exploitable**. → [[audit_secu_2026-06-29_session_delog_7j]] + [[feedback_cookie_maxage_equals_threshold_dead_check]].
...
- [x] ~~**Bloc A - refonte serveur page Entreprises** (filtre/pagination/tri/recherche, 100 % serveur)~~ - 2026-06-28 (**LIVRÉ PROD** `4b3974e`). Revue 5 relecteurs 0 High/Critical, 3 MEDIUM corrigés, parité counts/KPI exacte. → [[project_bloc_a_refonte_serveur_entreprises_2026-06-28]] + [[audit_secu_2026-06-28_entreprises_refonte_serveur]].
→ Antérieurs archivés (Backlog consolidé `6c477cf` 28/06 ; Bloc D audit 360 `cd5426e` 27/06 → [[project_bloc_d_audit_360_PROGRESS]] ; Fix icônes `a6ae0e1` + veille pont/PDF `b767056` 26/06 ; Bloc A `f357e68`, Bloc 0 `1065fb1`, Bloc 2 Dependabot, Daily Email `d1db821`, Vague 3.3 `ebacea6`, Campagnes V3.2) → `archive/claude-md-crm-livre-2026-06-26.md`. Plus anciens (24/06-) → mémoires veille 06-22/23/24 + `archive/`.

### Watch list active après pivot

- **[WATCH] Refonte Centre d'aide déployée prod (`d402fe7` + clôtures, 2026-06-30)** : build Vercel prod **Ready** vérifié, smoke routes OK (`/crm/aide` 303 → login, `/aide` 308) ; rendu authentifié OK (portail AC-004 `/crm/aide rend sans erreur`). Reste le **smoke OTP authentifié** par Pascal des 3 niveaux (vérifié pré-deploy via session mintée + axe). Gate a11y permanent `playwright.aide.config.ts` (nécessite `node tests/mint-session.mjs` avant run). `vercel rollback` si souci. **Sécu : garde-fou désormais déterministe** - `src/routes/crm/aide/+page.ts` pose `prerender = false` (le niveau 3 ne peut plus être figé en HTML public, il reste derrière l'auth). → [[project_refonte_aide_crm_2026-06-30]].
