...
**Prochaine attaque** : **Cohérence UI - bandeau de page increments 1 + 2 LIVRÉS prod** (Entreprises puis Contacts/Pipeline/Signaux/Campagnes, calques exacts, derrière `ff_page_bandeau` OFF, `5e0eea3`). **DÉCISION DESIGN en attente (Pascal) - increment 3** : 5 pages à identité forte/layout spécifique où le bandeau générique n'est PAS un drop-in - **Prospection** (layout pleine hauteur `100dvh`), **Dashboard** (greeting « inbox du matin »), **Reporting** (hero), **Veille** (masthead magazine « Filtrer l'essentiel »), **Aide** (`.aide-head` + titre dynamique par niveau). Arbitrage par page : bandeau générique (uniformité) vs préserver l'identité signée → **maquette + validation avant code** (règle non négociable). Sinon **Run 7** veille LED (cadrage en session). Run 4/5 bloqués (comptes Hunter/Pingen chez Pascal). **[à valider Pascal] mots-clés secteur LED** `secteurs.ts` (token « led » trop court, cf. WATCH). Micro sous ma responsabilité validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Détail → `docs/COHERENCE-UI-BANDEAU.md` + [[project_atelier_209_run3_import_liste_2026-07-16]].
...
- [ ] **[EXÉCUTABLE - GATE maquette Pascal sur increment 3]** **Cohérence UI globale du CRM** - benchmark **Gouvernance** ([[feedback_gouvernance_benchmark_layout_lisibilite]]) : discipline de structure, **pas la peau** (identité Atelier 209 conservée). **(a) Bandeau : increments 1 (Entreprises) + 2 (Contacts/Pipeline/Signaux/Campagnes) LIVRÉS prod derrière `ff_page_bandeau` (OFF)** ; **increment 3 = 5 pages à identité éditoriale/layout (Prospection layout `100dvh`, Dashboard, Reporting, Veille, Aide) = DÉCISION DESIGN + maquette Pascal** (pas un drop-in mécanique), spec `docs/COHERENCE-UI-BANDEAU.md`. **Restent à mocker** : (b) composants transverses, (c) compteurs agrandis, (d) grille 2 niveaux. `redesign-skill` + `refactoring-ui`.
