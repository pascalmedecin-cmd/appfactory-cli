...
- [x] ~~**Refonte exhaustive du Centre d'aide (`/crm/aide`) : contenu à jour + UI premium**~~ - 2026-06-30 (**LIVRÉ PROD** `d402fe7`, ultracode/xhigh). Demande Pascal : « revoir exhaustivement toute la partie aide, à jour des derniers changements CRM + UX/UI, rubrique claire, UI premium ». Méthode : workflow de découverte (7 lecteurs, état réel de chaque écran) → rédaction main-thread → workflow de vérification adversariale (4 lentilles). **Contenu** (fact-checker 0 finding) : portail multi-outils + diagramme « tu es ici » ; fiche Campagnes (N-N) ; Entreprises 100 % serveur ; éditeur veille + génération GitHub Actions ; Daily Email (récap fondateurs, 0 email prospects) ; **6 crons** réels (avant 5) ; Prospection V5 (SIMAP/RegBL retirées du manuel, quota Google Places) ; Coûts API recadrés (veille IA, pas images) ; Log ; auth OTP 6 chiffres. **UI premium** (golden v9, 0 animation scroll) : masthead + badge niveau, rail sommaire actif, profondeur sections/diagrammes, raccourci `/`, typo/espacements calés golden. **Fix collatéral** : 2 diagrammes pré-existants (cycle, veille) avaient leurs flèches invisibles (tête peinte sous la boîte opaque) → corrigé. **QA** : 2327 vitest, svelte-check 0, build OK, **gate a11y permanent** (`tests/aide-a11y.test.ts` + `playwright.aide.config.ts`, axe 4/4, 0 débordement 375px), sécu 0 C/H/M. Zéro dette/orphelin/code mort (knip clean). 8 fichiers (6 modif + 2 gate). → [[project_refonte_aide_crm_2026-06-30]] + [[feedback_svg_flow_arrow_paint_order]].
...
- **[WATCH] Refonte Centre d'aide déployée prod (`d402fe7`, 2026-06-30)** : build Vercel prod **Ready** vérifié, smoke routes OK (`/crm/aide` 303 → login, `/aide` 308). Reste le **smoke OTP authentifié** par Pascal des 3 niveaux (contenu auth-gated, vérifié pré-deploy via session mintée + axe, pas re-smoké connecté en prod par Claude). Gate a11y permanent `playwright.aide.config.ts` (nécessite `node tests/mint-session.mjs` avant run). `vercel rollback` si souci. **Garde-fou sécu** : ne jamais passer `/crm/aide` en prerender (le niveau 3 documente l'infra ; OK tant qu'auth-gated). → [[project_refonte_aide_crm_2026-06-30]].
