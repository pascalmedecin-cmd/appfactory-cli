...
**Derniere mise a jour :** 2026-06-05 (Découpe Films : étape 4 export PDF CLÔTURÉE + Phase 4 QA 360 complète. Reste Phase 5 = mise en prod, go Pascal. Commits `a389d1d` + `6f07af6` poussés sur `portail-session-1`)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** 2026-06-05 - Découpe Films **étape 4 export PDF CLÔTURÉE** + **Phase 4 QA 360 complète** (commits `a389d1d` + `6f07af6` poussés). Détail dans « Livré cette session » ci-dessous. Reste Phase 5 uniquement (mise en prod, go Pascal).
...
**Prochaine attaque** : Bloc 1 - Découpe Films **Phase 5 livraison** uniquement (étape 4 export PDF CLÔTURÉE + Phase 4 QA 360 COMPLÈTE cette session). **Go Pascal requis** (action outward-facing). Reste : flag `ffDecoupe` ON pour tous (app_metadata des utilisateurs `@filmpro.ch`) + `vercel deploy --prod` (alias canonique `filmpro-portail.vercel.app` ; migration prod déjà appliquée 2026-06-05). Smoke prod post-deploy (créer chantier → optimiser → exporter PDF). → `memory/feedback_decoupe_tolerance_zero_qa.md`.

### 1. Chantier 2 Découpe Films - Phase 5 livraison prod [SUPERVISÉ • xhigh • go Pascal]

- **Pourquoi** : étape 4 export PDF CLÔTURÉE + **Phase 4 QA 360 complète** (RLS réelle vérifiée Postgres + perf LCP/CLS authentifiée + sécu Opus 0 H/C/M + e2e 12/12 + axe 0/4). Reste uniquement la mise en prod (outward-facing).
- **Prérequis levé** : Phase 4 complète (cette session). Pack specs `.product-architect/decoupe/`.
- [ ] **[EXÉCUTABLE - go Pascal]** Phase 5 livraison : flag `ffDecoupe` ON pour tous les utilisateurs `@filmpro.ch` (script `scripts/_set-decoupe-flag.mjs`) + `vercel deploy --prod` + smoke prod (chantier → optimiser → **exporter PDF**, vérifier le donut sur le PDF téléchargé). Migration prod déjà appliquée. → `memory/feedback_decoupe_tolerance_zero_qa.md`.
