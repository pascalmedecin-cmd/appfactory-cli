...
**Prochaine attaque** : **Cohérence UI PART 2 lot 2+c+d** (`ff_ui_coherence` OFF, autonome, mockup b/c/d déjà validé) - c'est le plus gros track actionnable maintenant que la **parité bi-marque est CLOSE hors veille** (WP-A/B/C déployés 18/07 `584e937`+`0f27023`, bug 1 réglé ; reste #8 = Run 7 gated). Autres : bandeau (a), option mobile import (gate mockup), Run 7 veille LED. Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
