...
- [x] ~~**Lot 2 - Prospects + Pipeline : refonte statut + entrée pipeline**~~ - 2026-07-01 (**LIVRÉ PROD** `aff32cd`+`28b4848`, ultracode/xhigh ; migration appliquée prod en transaction : 161 nouveau->vide, 1 interesse->a_contacter + 1 opp backfillée, 0 orphelin ; auto-deploy Vercel Ready). Statut prospect vide/a_contacter/ecarte/transfere ; migration (remap + colonne `opportunites.prospect_lead_id` + RPC `mark_lead_for_contact` atomique + backfill opps + `transfer_lead_to_crm` FOR UPDATE), prouvée postgres réel ; Zod durci ; query (file de tri vide, vue Écartés, tri date_import, retrait température) ; triage->RPC ; `convertToClient` au pipeline (récupérable) ; UI sidepane 2 boutons + Réactiver, batch contextuel, retrait colonne priorité/filtre température, édition statut inline, bouton Convertir client, fix lien SIMAP, liens dashboard. **Audit 14 agents : 9 findings TOUS corrigés, 0 H/C.** 2362 tests verts, svelte-check 0/0. → [[project_refonte_signaux_prospects_campagnes_2026-07-01]] + [[audit_secu_2026-07-01_lot2_prospects_pipeline]].
