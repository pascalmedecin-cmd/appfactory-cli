...
**Dernière mise à jour :** 2026-07-21 (soir 2 : **CTAs desktop Prospection LIVRÉS** derrière `ff_ui_coherence` OFF - alignement sur `.ws-btn` (padding 16 / poids 600 / transition 220ms + garde reduced-motion), teinte violette « Enrichir » préservée, débloqué par le fix collision déployé. QA « zéro angle mort » : tout mesuré (off/on/mixed 2 flags × 767-1920 × reduced-motion × dormant injecté), double filet adversarial (workflow 4 lentilles + bug-hunter) = SHIP. Directive Pascal gravée : exigences inchangées + pas de contrainte de temps → zéro raté, adversarial = filet à garder, mesurer plutôt que raisonner → [[feedback_zero_angle_mort_mesurer_pas_raisonner]]. Plus tôt le 21/07 : collision tabs-shell fixée + déployée, d2/d3 LIVRÉ `8bc4b6e`, BUG #001 login OTP résolu par provisioning). **BUG #001 login OTP RÉSOLU (21/07, ultracode)** SANS toucher au code login (git propre - « remets ce qui marchait », directive Pascal) : la vraie cause était « le compte n'existait pas » (pas le code, qui marchait pour les `@filmpro.ch`). Les 2 comptes `@lamaisoncreativedirection.ch` sont désormais **provisionnés** (existants + confirmés + identité email + flags miroir de leur homologue `@filmpro.ch`) → 1er login = code 6 chiffres, plus jamais « Confirm signup ». Reste : le smoke test réel (recevoir le code) = geste Pascal. → [[project_login_otp_new_user_confirm_signup_2026-07-21]].
...
**Prochaine attaque** : **Retouches accueil - Variante A** (maquette EN COURS, PAS codé). A validée par Pascal (vertical, hero agrandi). Reprendre par : rendre le **fondu vertical (masque)** visible + **décoller les eyebrows** du bandeau, à valider **dans la fenêtre Chrome de Pascal** (pas le Chrome du MCP - leçon dure de la session) ; puis **upscale hero fal.ai** (GO Pascal) ; puis coder (AtelierShell + portail grille 3 colonnes CRM/Découpe/Coûts). Tout consigné → [[project_accueil_atelier209_mockup_2026-07-21]]. _(Avant : CTAs desktop Prospection LIVRÉS+poussés 21/07 `727b290`, `ff_ui_coherence` OFF - dernier `[EXÉCUTABLE]` pur de Phase 1.)_ Tout ce qui reste en Phase 1 est **`[GATE mockup]`** (maquette Chrome à valider par Pascal AVANT code) : reliquats esthétiques increment b, redessin KpiStrip/12-col parqué, retouches accueil `AtelierShell`, import mobile. **Prochain pas = produire un mockup pour l'un d'eux** (à cadrer avec Pascal : la cible), OU passer à **Phase 2 = timeline d'interactions** (table `activites` dormante, mockup d'abord aussi). Contexte : Atelier 209 à **3/7 runs en prod** ; `d1/d2/d3` + fix collision **EN PROD** (flags cohérence allumés sur pascal@). BUG #001 login OTP résolu (reste smoke test = geste Pascal). Directive permanente gravée : **zéro angle mort = mesurer, pas raisonner** ([[feedback_zero_angle_mort_mesurer_pas_raisonner]]). → [[project_coherence_ui_cta_prospection_bloque_collision_2026-07-21]], [[feedback_qa_micro_mon_ressort_gate_pas_excuse]].
...
- [ ] **[EXÉCUTABLE]** Retouches accueil (`AtelierShell.svelte`) - maquette EN COURS, pas codé : **Variante A VALIDÉE** par Pascal (vertical, hero agrandi ~52%). Maquette `.atelier-209/accueil-maquettes/accueil.html`. Reste : (a) **fondu VERTICAL** (masque `mask-image` sur `.banner img`, mot de Pascal « le fondu doit être vertical ») à rendre nettement **visible** côté néon + **décoller les eyebrows** du bandeau (Pascal l'a redit 2×) - **valider dans SA fenêtre Chrome, pas le Chrome du MCP** ; (b) **upscale hero fal.ai** (flou à 52%, GO Pascal demandé) ; (c) coder AtelierShell + portail **grille 3 colonnes** (CRM/Découpe/**Coûts**). → [[project_accueil_atelier209_mockup_2026-07-21]].
