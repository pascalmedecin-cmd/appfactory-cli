...
**Dernière mise à jour :** 2026-07-18 (parité bi-marque - **ré-audit exhaustif** (workflow, toute la surface visible) : le 1er audit était scopé prospection/campagnes et avait manqué **~15 divergences**. **WP-A/B mécaniques DÉPLOYÉS prod** (`584e937`, k29rjd2so Ready) : titres/hero Reporting/Log/Coûts `marqueLabel` + teinte échappée navy→token (halos, 15 tints campagnes, accents PDF, scrim étiquettes). FilmPro byte-identique, LED teinté magenta. Vitest **2863**, svelte-check 0/0, QA réelle 2 marques, revue adversariale (1 LOW scrim corrigé). **bug 1 RÉGLÉ** (pas un bug de marque, prouvé sur env LED réel). Reste parité : **WP-C** 6 copies métier (gate maquette ouverte dans Chrome) + #8 Run 7. → `docs/ATELIER-209-SUIVI.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : **Parité bi-marque - ré-audit exhaustif fait + WP-A/B DÉPLOYÉS 18/07** (`584e937` : titres/hero marqueLabel + teinte échappée navy→token ; FilmPro byte-identique, LED magenta ; Vitest 2863, revue adversariale 1 LOW corrigé). **bug 1 RÉGLÉ** (repro env LED réel = pas un bug de marque). **Reste parité = WP-C + #8** : **WP-C** = 6 copies métier LED (LeadExpress ×2, PipelineQuickAdvance, PhotoGallery, ImportModal Google helper, modèle CSV) → **maquette ouverte dans Chrome, attend « ok » Pascal** avant code (une passe via `prospection-copies.ts`) ; **#8** hero Signaux = Run 7. Autres tracks : Cohérence UI PART 2 lot 2+c+d (`ff_ui_coherence` OFF), bandeau (a), Run 7 veille LED. Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[BLOQUÉ - toi : « ok » sur la colonne LED de la maquette WP-C ouverte dans Chrome]** Parité bi-marque WP-C - 6 copies métier LED (LeadExpress entreprise+note, PipelineQuickAdvance action, PhotoGallery vide, ImportModal helper Google, modèle CSV). Code prêt en une passe via `prospection-copies.ts` (FilmPro byte-identique) dès ton go. Maquette : `.atelier-209/parite-copies-led-wp-c/maquette-wp-c.html`. → `docs/ATELIER-209-SUIVI.md` § Parité + [[feedback_bi_marque_parity_qa_en_sortie]].
...
- **[WATCH] Parité bi-marque LED ↔ FilmPro (18/07)** : **ré-audit exhaustif fait** (workflow, toute la surface visible - le 1er audit ne couvrait que prospection/campagnes). Original 1-7 + WP-A/B **déployés** ; **bug 1 fermé** (repro env LED réel : bouton import identique aux 2 marques, gaté par le breakpoint `md` seul - pas un bug de marque). **Reste = WP-C + #8** : WP-C = 6 copies métier LED (maquette Chrome, attend « ok » Pascal) ; #8 hero Signaux « vitrage » = Run 7. **Leçon** : un audit scopé rate une classe entière - le ré-audit full-surface a trouvé ~15 divergences de plus. → backlog + `docs/ATELIER-209-SUIVI.md` § Parité + [[feedback_bi_marque_parity_qa_en_sortie]].
