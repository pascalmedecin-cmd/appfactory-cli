...
**Dernière mise à jour :** 2026-06-28 (socle vite 8 / rolldown). Trunk unique = `main` (reconciliation `1065fb1`, branche supprimée ; Dependabot trié, 0 PR ouverte). Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`** → refonte Vagues 1+2+3 live. Prod = `filmpro-portail.vercel.app` : **push `main` auto-déploie en production** (vérifié live 26/06 ; détail → WATCH Vercel). Livraisons → § « Livré cette session ». **Prochain bug :** #001.
**Session courante :** 2026-06-28 - **Bloc B migration socle SvelteKit LIVRÉ PROD** (`e75919d`, merge PR #24, smoke prod public verte) : vite 8 (rolldown) + vps 7 + kit 2.68 + svelte 5.56.4, `ignore` socle `dependabot.yml` levés. + **pause backlog** : restructuration 4 bacs + règle WIP anti-gonflement → **backlog dev désormais VIDE**. **À FAIRE Pascal** : coup d'œil authentifié socle (PDF Découpe + graphes layerchart, OTP) - rollback armé si souci. Détail → § Livré + [[project_fix_deps_ci_vercel_2026-06-22]].
...
- [ ] **[BLOQUÉ - toi : 1 login OTP en prod]** Faire le smoke OTP de production end-to-end (@filmpro.ch ; boot + page login déjà OK). Valide aussi le fix expiration session 7j livré 29/06 (`c7545ed`). **Si KO → `vercel rollback`.**
...
- [x] ~~**Fix expiration session 7j (« pas de délog ») + audit sécu 360**~~ - 2026-06-29 (ultracode/xhigh, **LIVRÉ PROD** `c7545ed`, CI verte). Plafond 7j = **code mort** : cookie `login_at` maxAge 7j s'auto-supprimait pile au seuil → check `loginAt && age>7j` jamais vrai, session survivait sur cookies Supabase `sb-*` (400j défaut). Fix fail-secure (absence/falsification cookie = expiré) + 2 tests. **Audit 360** (23 agents, 7 axes + sceptiques) : **0 C/H/M/L exploitable** (16→0, mono-locataire annule). 2317 verts, check 0/0, build OK. Reste : smoke OTP (geste Pascal) ; optionnel time-box Supabase Dashboard. → [[audit_secu_2026-06-29_session_delog_7j]] + [[feedback_cookie_maxage_equals_threshold_dead_check]].
- [x] ~~**Bloc B - migration socle SvelteKit : vite 8 (rolldown) + vps 7 + kit 2.68 + svelte 5.56.4 + svelte-check 4.7.1**~~ - 2026-06-28 (ultracode/xhigh, **LIVRÉ PROD** `e75919d`, PR #24). Socle sorti de l'état « tenu vite 7 ». ERESOLVE = `vite-plugin-svelte-inspector` (peer vite^7) intégré/élagué en vps 7 ; deadlock cassé via `--legacy-peer-deps` puis `npm install` plain auto-cohérent. Le « bug TS-stripping » 06-22 = artefact regen lockfile, **PAS vite 8**. **Jamais `rm package-lock.json`**. `ignore` socle `dependabot.yml` levés. Gates verts (build, check 0/0, test **2315**, npm ci, CI runner, Vercel preview+prod) ; revue **0 finding** supply-chain, **smoke prod public verte** ; reste = coup d'œil authentifié Pascal (PDF/graphes, OTP). + **pause backlog** (règle WIP, `7ecb046`). Détail intégral → [[project_fix_deps_ci_vercel_2026-06-22]].
...
- [x] ~~**Backlog consolidé en blocs 1-session + zéro code mort (knip cadré)**~~ - 2026-06-28 (ultracode, `6c477cf` poussé prod 28/06). 6 tâches → 4 blocs « 1 session » + checklist ; raison de blocage Bloc B corrigée (peer-deps, pas issues #928/#1313 fermées) ; knip 72→3 findings (5 morts + 2 scripts media retirés). 2301 verts. → [[feedback_knip_verify_grep_before_delete]].
