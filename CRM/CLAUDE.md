...
**Prochaine attaque** : **[GATE macro Pascal] Revue MACRO de la prod Atelier 209** (`filmpro-portail.vercel.app`). Pascal regarde **l'ensemble et la direction** (est-ce que ça va dans le bon sens, à l'échelle produit), **PAS les détails fins** : la validation des détails micro (UX, cohérence, défauts fins, accords FR, états) est **sous MA responsabilité** - je les valide AVANT qu'il regarde, il ne doit jamais avoir à les chasser (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Périmètre de la revue macro : le portail + le flux d'import Run 3 (déjà QA + 7 défauts micro corrigés le 16/07, `c106a67`, version nettoyée) → Prospection → « Importer une liste » / onglet « Ma liste » → CSV → colonnes auto → aperçu → import → « Ma liste ». **Ensuite** : Run 4 (Hunter, bloqué compte Pascal) / Run 7 (veille LED, cadrage) ; **[à valider Pascal] mots-clés secteur LED** de `secteurs.ts`. Détail → [[project_atelier_209_run3_import_liste_2026-07-16]].
...
- [ ] **[BLOQUÉ - toi : revue macro]** Revue MACRO de la prod Atelier 209 : regard d'ensemble sur la direction/version en prod (`filmpro-portail.vercel.app`), PAS les détails - les micro (UX, cohérence, accords FR, états) sont sous MA responsabilité, déjà validés. Gate avant d'attaquer Run 4+. → [[project_atelier_209_run3_import_liste_2026-07-16]].
