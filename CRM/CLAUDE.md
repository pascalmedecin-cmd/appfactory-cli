...
**Dernière mise à jour :** 2026-07-01 (**Refonte CRM 3 lots COMPLÈTE et LIVRÉE PROD** - workflow recherche->prospection->pipeline->entreprise, signaux détachés : **Lot 1 Signaux** `db6182e` + **Lot 2 Prospects+Pipeline** `aff32cd` + **Lot 3 Campagnes** (statut En cours/Active, recherche prospects embarquée, refonte page, badge statut prospection) **LIVRÉS PROD** ; détail → § Livré + [[project_refonte_signaux_prospects_campagnes_2026-07-01]]). Trunk = `main` ; Flag `ffCrmListesV2` **ON `pascal@`+`antoine@`**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : smoke OTP nouveau workflow prospect + Campagnes Lot 3. **Prochain bug :** #001.
...
→ Antérieurs (Centre d'aide `d402fe7` + Découpe `0af198d` 30/06 → [[project_refonte_aide_crm_2026-06-30]] / [[project_decoupe_refonte_ui_2026-06-30]] ; Fix session 7j `c7545ed` 29/06 ; Bloc B vite 8 `e75919d`, Bloc A `4b3974e` 28/06 ; Bloc D `cd5426e` ; veille pont/PDF `b767056` 26/06 ; Daily Email `d1db821`) → `archive/claude-md-crm-livre-2026-06-26.md`.

### Watch list active après pivot

- **[WATCH] Smoke OTP Pascal + livrables prod récents (06-30/07-01)** : vérifier le **nouveau workflow prospect Lot 2** (`aff32cd` : sidepane À contacter/Écarter/Réactiver, toggle Écartés desktop+mobile, édition statut inline, pipeline « Convertir en client »), Signaux (`db6182e` : bouton Statut À suivre/Archivé + lien SIMAP `project-detail/{source_id}`), Étiquettes (`e263240` : PDF Avery + destinataire ; ellipse 0.62 em + quota search.ch), Centre d'aide (`d402fe7`), Découpe (`0af198d`, flag `ff_decoupe`), **Campagnes Lot 3** (`/crm/campagnes` : onglet « Ouvertes », contrôle statut inline En cours/Active, bouton « Trouver des prospects » → modal pré-coché, KPI ; **prospection** : badge fusée « lancée » sur pastilles de campagnes actives). Rollback = flag OFF ou `vercel rollback` (migration Lot 3 = ADD COLUMN idempotente, se remappe ; la colonne statut peut rester sans risque). Lot 1 : colonne dormante `opportunite_associee_id` + FK laissées (nettoyage ultérieur). → [[project_refonte_signaux_prospects_campagnes_2026-07-01]] + mémoires respectives.
