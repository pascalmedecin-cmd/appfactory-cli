...
**Prochaine attaque** : **Bandeau de page (a) COMPLET** - les 10 pages + convention kicker 12px livrées prod (derrière `ff_page_bandeau` OFF ; kicker live). Reste : **allumer le flag** quand tu veux (option : alléger le greeting Dashboard) ; puis mocker bandeau (b) composants transverses / (c) compteurs / (d) grille 2 niveaux. Sinon **Run 7** veille LED (cadrage en session). Run 4/5 bloqués (comptes Hunter/Pingen chez Pascal). **[à valider Pascal]** mots-clés secteur LED `secteurs.ts` (token « led » trop court, cf. WATCH). Micro sous ma responsabilité validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]). Détail → `docs/COHERENCE-UI-BANDEAU.md`.
...
- [ ] **[EXÉCUTABLE - reste (b)(c)(d) + allumage flag]** **Cohérence UI globale du CRM** (benchmark Gouvernance, [[feedback_gouvernance_benchmark_layout_lisibilite]]) : discipline de structure, pas la peau. **(a) Bandeau COMPLET** (10 pages + kicker 12px, flag OFF) → reste allumer le flag (option : alléger greeting Dashboard). **À mocker** : (b) composants transverses, (c) compteurs, (d) grille 2 niveaux. Spec `docs/COHERENCE-UI-BANDEAU.md`. `redesign-skill` + `refactoring-ui`.
...
- [x] ~~**Cohérence UI - chantier bandeau de page COMPLET : les 10 pages + convention kicker 12px**~~ - 2026-07-17 (ultracode). Increments 1→3 + Veille (bandeau `flush` + peau magazine conservée) derrière `ff_page_bandeau` (OFF) : source unique `isBandeauActive` (Header + page jamais désync), OFF = rendu d'origine strict, 1 seul `h1`/page prouvé. Convention kicker/sur-titre = 12px (live). QA : svelte-check 0/0, Vitest 2824, build OK, 2 revues adversariales. Commits `5e0eea3`/`b4fe644`/`d51fd31`/`e60f1b6`. → `docs/COHERENCE-UI-BANDEAU.md` + archive `claude-md-crm-livre-2026-07-17.md`.
→ Antérieurs (bandeau inc 1→Veille détaillé, Campagnes doublon, e2e validation, Retours macro, Run 3 QA du 16/07 ; Run 3 import + Run 2 + Run 1, login OTP 14/07, veille/étiquettes/refonte Lots 1-3/Aide/Découpe/Daily Email) → `archive/claude-md-crm-livre-2026-07-17.md` + `-07-16.md` + `-07-15.md` + `-07-14.md` + `-07-03.md` + `-07-02.md` + `-06-26.md` ; mémoires [[project_atelier_209_run3_import_liste_2026-07-16]], [[project_atelier_209_run2_cablage_2026-07-15]], [[project_login_otp_smtp_supabase_2026-07-14]].
