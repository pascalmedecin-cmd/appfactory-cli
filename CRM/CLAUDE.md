...
- [x] ~~**Bloc B - migration socle SvelteKit vite 8 (rolldown) + vps 7 + kit 2.68 + svelte 5.56.4**~~ - 2026-06-28 (ultracode/xhigh, **LIVRÉ PROD** `e75919d`, merge PR #24 + smoke prod public verte). Socle sorti de l'état « tenu vite 7 » : vite 7.3.6→**8.1.0** (bundler rolldown), vps 6.2.4→**7.1.2**, kit 2.59.1→**2.68.0**, svelte 5.55.5 pin→**5.56.4 dé-pinné**, svelte-check 4.4.8→**4.7.1**. **Cause ERESOLVE** = `vite-plugin-svelte-inspector@5` (peer vite^7), intégré dans vps 7 → disparu de l'arbre ; deadlock cassé via `--legacy-peer-deps` puis `npm install` plain auto-cohérent. Le « bug TS-stripping » 06-22 = artefact regen lockfile, **PAS vite 8** (compilateur Svelte 5 natif strippe sous rolldown). **Jamais `rm package-lock.json`** (bump en place). `dependabot.yml` : ignore socle **LEVÉS** (en-tête 06-22 maj ; typescript+supabase conservés). Gates : build, check 0/0 (5222), test **2315/2315**, npm ci propre, **CI runner PASS**, **Vercel preview+prod SUCCESS**. Revue adversariale : supply-chain **0 finding** (paquets officiels latest, lockfile registry-only integrity), régression runtime **FAIBLE** ; **smoke prod public verte** (0 erreur console, tous chunks 200, init Supabase OK). Reste = coup d'œil authentifié Pascal (PDF Découpe / graphes layerchart, OTP-gated). 2 vulns moderate **préexistantes** (dompurify via jspdf, tar build) hors scope. + **pause backlog** : restructuration 4 bacs + règle WIP anti-gonflement (`7ecb046`). → [[project_fix_deps_ci_vercel_2026-06-22]].
