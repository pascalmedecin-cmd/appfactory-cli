...
**Prochaine attaque** : Bloc 1 - Découpe Films **Phase 3 étape 3** (écrans `/decoupe` homogènes CRM + form actions CRUD branchées sur les builders), puis checkpoint PDF. Data layer (étape 2 : migration `decoupe_*` + RLS + types + Zod + builders) livré 2026-06-05.
...
- [ ] **[EXÉCUTABLE]** Étape 3 - écrans `/decoupe` **homogènes CRM** (réutiliser `DataTable`/`Badge`/`EmptyState`/`FormField`/`Select`/`ConfirmModal`/`Icon`) + **form actions CRUD** (load + create/update/delete branchées sur `src/lib/server/decoupe/builders.ts` + schemas `$lib/schemas`) : liste chantiers, fiche+saisie vitres, base produit, écran optimisation (plan SVG + taux de chute + liste commande + alertes). Ajouter flag `ffDecoupe` + guard de route + activer la card portail. → voir `.product-architect/decoupe/DESIGN.md` + `api-contracts.ts` §4.
