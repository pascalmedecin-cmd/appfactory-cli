...
- [ ] **[EN COURS - maquette, pas codé]** Retouches accueil (`AtelierShell.svelte`) : **Variante A VALIDÉE** par Pascal (vertical, hero agrandi ~52%). Maquette `.atelier-209/accueil-maquettes/accueil.html`. Reste : (a) **fondu VERTICAL** (masque `mask-image` sur `.banner img`, mot de Pascal « le fondu doit être vertical ») à rendre nettement **visible** côté néon + **décoller les eyebrows** du bandeau (Pascal l'a redit 2×) - **valider dans SA fenêtre Chrome, pas le Chrome du MCP** ; (b) **upscale hero fal.ai** (flou à 52%, GO Pascal demandé) ; (c) coder AtelierShell + portail **grille 3 colonnes** (CRM/Découpe/**Coûts**). → [[project_accueil_atelier209_mockup_2026-07-21]].
...
- [ ] **[BLOQUÉ - toi : arbitrer quoi condenser]** Condensation dédiée `CRM/CLAUDE.md` (25726 chars > 20K soft) : le surplus est le roadmap 5 phases + watch list (contexte de décision frais), non charcuté autonomement. Décider avec moi quoi archiver.
