...
**Dernière mise à jour :** 2026-07-03 (**validation externe : workflow révisé `d629cd3`** - décidés affichés, « Envoyer la validation » → badge « Validation reçue », jamais bloquant → § Livré). Trunk = `main` ; Flag `ffCrmListesV2` **ON fondateurs**. Prod = `filmpro-portail.vercel.app` (push `main` auto-déploie, intermittent - cf. Watch). **À FAIRE Pascal** : variable Daily Email (§ Chez Pascal). **Prochain bug :** #001.
...
- **[WATCH] Veille W28 (vendredi 10/07)** = 1er cron auto post-fix pause_turn : doit passer sans intervention (2e cron du soir = idempotent_skip) ; densité à l'œil (W27 : 6 puis 4 items, cible 8-12 jamais atteinte). → [[feedback_pause_turn_reprise_pas_echec]] + archive 07-03.
