...
**Derniere mise a jour :** 2026-06-25. Prod = `filmpro-portail.vercel.app` sur `291ae02` (veille W25 mergée+déployée : libellés humanisés + tri d'importance + strip enum-en-prose ; refonte CRM flag `ffCrmListesV2` OFF). **Derniere revue /optimize :** 2026-04-05. **Prochain bug :** #001.
**Session courante :** 2026-06-24/25 (session 6) - **Vague 3.2 Campagnes UI COMPLÈTE LIVRÉE** (`c905952`+`9e1817f`, branche `campagnes-vague32`, flag OFF, non mergé/non déployé) : liste/fiche/import/écran dédié `/crm/campagnes`/export + gate premium serveur ; revue adversariale 5 lentilles (11 findings corrigés) ; 2162 tests verts, svelte-check 0, build OK. Prod inchangée (`291ae02`, campagnes non déployées). → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]].
...
**Prochaine attaque** : **Vague 3.3 Dashboard temporel** OU **déployer la refonte CRM** (décision Pascal). **Campagnes Vague 3.2 : UI COMPLÈTE LIVRÉE 2026-06-24** (`c905952`, cette branche `campagnes-vague32`, flag OFF, non mergé/non déployé ; revue adversariale 5 lentilles, 11 findings corrigés) → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]]. Veille W25 **TERMINÉE + DÉPLOYÉE** (prod `291ae02`, bloc 0). **W26 (vendredi 26/06) = 1er run cron sur le code W25 + 1er brief réel antoine@**, à surveiller (densité, ordre local-actionnable, 0 underscore, so-what sans cible inventée, et que le cron ne re-dumpe plus l'enum en prose).
...
- [x] ~~**Chantier veille W25 - TERMINÉ + DÉPLOYÉ 2026-06-23**~~ : round-2 trust-by-source (`47841d9`) + 5 commentaires Pascal (humanisation enums via `veilleFormat.ts`, so-what sans cible non sourcée, cap 10→12, tri d'importance déterministe) + flake fix (`d99beab`) + strip enum-en-prose (`291ae02`, 6e défaut attrapé au smoke prod ; garde liste close + cause racine prompt) ; 2 deploys prod, data W25 nettoyée live, smoke authentifié clean, revue adversariale 3 sceptiques. → [[project_veille_fix_w25_commentaires_2026-06-23]] + [[project_veille_trust_by_source_2026-06-23]].
...
- [x] ~~**Vague 3.2 Module Campagnes (multi-étiquetage N-N) - UI COMPLÈTE 6/6 phases**~~ - LIVRÉ 2026-06-24 (`c905952`, cette branche `campagnes-vague32`, flag OFF, **non mergé/non déployé**). Liste (colonne multi-pastilles + filtre relationnel) + `CampagneCombo` réutilisable accessible (combobox ARIA + clavier ; fiche + import) + écran dédié `/crm/campagnes` CRUD + sidebar gatée + export colonne + **gate premium serveur** (defense-in-depth). Revue adversariale 5 lentilles : 11 findings **corrigés + testés**. 2162 tests, svelte-check 0, build OK. RESTE = déploiement (tâche ci-dessous). Fast-follow optionnel : combo inline dans la ligne (aujourd'hui le « + » ouvre la fiche). → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]].
...
- [x] ~~**Chantier veille W25 TERMINÉ + DÉPLOYÉ**~~ - 2026-06-23 (session 5) : merge FF `8b03f07` + flake `d99beab` + strip enum-en-prose `291ae02`, 2 deploys prod, data W25 nettoyée live, smoke authentifié clean. 6e défaut (enum dumpé en prose) attrapé au smoke prod et fermé (cause racine prompt + garde liste close, revue adversariale 3 sceptiques), 2098 tests. → [[project_veille_fix_w25_commentaires_2026-06-23]].
- [x] ~~**Module Campagnes Vague 3.2 - UI complète 6/6**~~ - 2026-06-24 (`c905952`) : liste/fiche/import/écran dédié `/crm/campagnes`/export + gate premium serveur, revue adversariale 5 lentilles (19 agents, 11 findings corrigés), 2162 tests verts, svelte-check 0, build OK. Flag OFF, non mergé/non déployé. → [[project_module_campagnes_vague32_2026-06-22]] + [[audit_secu_2026-06-24_vague32_campagnes_ui]].
→ Livrés antérieurs : Veille trust-by-source round-2 (`47841d9`, prod `lut2v4cy1`), W25+emails+chips+dedash (`1c7e484`/`8aa9547`), Lots 1+2 (`de05779`), hygiène working tree (`407d7b8`/`0eedeab`), Vague 3.1 Signaux (`5e0c369`)/export CSV (`6626818`)/Vague 2 (`8c5d84d`)/fiches (`87a1842`)/P1-P3 (`6c4230a`)/Pipeline (`35d7df7`)/Vague 1 (`676a9d4`) → `archive/claude-md-crm-livre-2026-06-19.md` + `_2026-06-18.md` + mémoires veille 06-22/23.
