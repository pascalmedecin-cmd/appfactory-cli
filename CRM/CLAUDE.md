...
**Session courante :** 2026-06-26 - **Fix icônes « ? » prod LIVRÉ + AUTO-DÉPLOYÉ** (`a6ae0e1`, CI+Vercel verts ; 10 noms mappés + test lint anti-régression). **À FAIRE Pascal** : smoke OTP prod (supabase 2.108 live) + activer `EMAIL_DAILY_ENABLED=true` (Daily Email) + revue visuelle PDF veille. Détail + antérieurs → § « Livré » + `archive/`.
...
- [x] ~~**Fix icônes « ? » en prod + garde anti-régression ICON_MAP**~~ - 2026-06-26 (ultracode/xhigh) : commit `a6ae0e1`, **poussé + auto-déployé prod** (CI verte, alias `filmpro-portail` promu vérifié live). **10 noms Material Symbols** présents dans l'UI mais absents d'ICON_MAP (rendaient un « ? » via `FALLBACK_ICON`) mappés dans `icon-map.ts` (détail mappings dans le commit ; `inventory_2`→Archive). Sweep indépendant (workflow) = **+3 vs le SIGNAL** (8→10, feeders `icon:`) ; vérif adversariale par-icône (1 réfutation retenue). **Test lint** `icon-map.test.ts` (littéraux + ternaires + props `icon:`, red-green prouvé, aurait attrapé `picture_as_pdf`). 2324 tests verts, svelte-check 0/0, build OK. → payload = commit `a6ae0e1`.
