...
**Dernière mise à jour :** 2026-07-18 (parité bi-marque : ré-audit exhaustif (~15 divergences manquées par le 1er audit) puis **WP-A/B/C tous déployés** (`584e937` + `0f27023`) ; **bug 1 réglé**. FilmPro byte-identique, LED teinté + copies. Vitest **2863**, svelte-check 0/0. **Parité UX/UI close hors veille (#8=Run 7).** → `docs/ATELIER-209-SUIVI.md`). **Prochain bug :** #001.
...
**Prochaine attaque** : **Cohérence UI PART 2 lot 2+c+d** (`ff_ui_coherence` OFF, autonome, mockup b/c/d déjà validé) - c'est le plus gros track actionnable maintenant que la **parité bi-marque est CLOSE hors veille** (WP-A/B/C déployés 18/07 `584e937`+`0f27023`, bug 1 réglé ; reste #8 = Run 7 gated). Autres : bandeau (a), option mobile import (gate mockup), Run 7 veille LED. Run 4 (enrichissement **Dropcontact** validé, actionnable - cf. Chez Pascal) / Run 5 Pingen bloqué chez Pascal. Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [x] ~~**Parité bi-marque WP-C - 6 copies métier LED DÉPLOYÉ prod (`0f27023`)**~~ - 2026-07-18 (ultracode). Maquette validée Pascal (#5 corrigé). `prospection-copies.ts` étendu (LeadExpress/Pipeline/PhotoGallery/ImportModal/CSV), FilmPro byte-identique, QA runtime 2 marques. **Parité UX/UI close hors veille.** → `docs/ATELIER-209-SUIVI.md` + [[feedback_bi_marque_parity_qa_en_sortie]].
- [x] ~~**Parité bi-marque - ré-audit exhaustif + WP-A/B DÉPLOYÉS prod (`584e937`)**~~ - 2026-07-18 (ultracode). Ré-audit workflow (~15 divergences manquées par le 1er audit) : titres/hero `marqueLabel` + teinte navy→token (halos, tints campagnes, PDF, scrim). FilmPro byte-identique, LED magenta. Vitest 2863, revue 1 LOW corrigé. **bug 1 fermé** (breakpoint `md`, pas la marque). → `docs/ATELIER-209-SUIVI.md`.
...
→ Antérieurs (Cohérence UI b part1/2, Campagnes, e2e validation, Retours macro, Run 3 QA/import + Run 2 + Run 1, login OTP, veille/étiquettes/refonte/Aide/Découpe/Daily Email) → archives `claude-md-crm-livre-2026-07-{18,17,16,15,14,03,02}.md` + `-06-26.md` ; mémoires [[project_coherence_ui_increment_b_part2_2026-07-17]], [[project_atelier_209_run3_import_liste_2026-07-16]], [[project_atelier_209_run2_cablage_2026-07-15]].
...
- **[WATCH] Parité bi-marque LED↔FilmPro - CLOSE hors veille (18/07)** : WP-A/B/C déployés, bug 1 fermé ; reste #8=Run 7. Toute nouvelle surface LED passe la checklist de parité. Leçon : un audit scopé rate une classe (ré-audit full-surface = +15 divergences). → `docs/ATELIER-209-SUIVI.md` + [[feedback_bi_marque_parity_qa_en_sortie]].
