...
- **État : Run 1, Run 2 ET Run 3 DÉPLOYÉS EN PROD.** Run 3 : import de liste + « Ma liste » + D3/D4, puis **QA visuelle + e2e + resize colonnes déployés le 16/07** (`c106a67`, 7 défauts micro corrigés, smoke vert, 0 C/H). **Revue MACRO Pascal faite le 16/07** (8 retours livrés prod `a987f6d`) ; le micro (détails UX/FR/états) est **déjà validé de mon côté**, il n'a pas à le chasser. V6 Hunter / V7 Pingen en attente comptes Pascal. Détail → `docs/ATELIER-209-SUIVI.md` + [[project_atelier_209_run3_import_liste_2026-07-16]].
...
**Dernière mise à jour :** 2026-07-17 (cohérence UI - **increment b PART 1 livré (commité, pas poussé)** : boutons→primitive `.ws-btn` sur 5 pages + pastilles Badge/SourcePill radius-full + SearchInput 40px, derrière `ff_ui_coherence` OFF. QA OFF/ON prouvée au DOM, revue adversariale 5 lentilles/7 findings corrigés dont 2 régressions OFF. Vitest 2827). **Prochain bug :** #001.
...
**Prochaine attaque** : **Cohérence UI increment b - PART 2** (part 1 livrée, commité non poussé). Reste, par ordre de valeur : **INC-7 états vides** (~14 réimpl → `EmptyState`), **INC-5 recherches recodées** (campagnes/aide/campagnes[id]/veille/editeur → `SearchInput`), **prospection CTAs desktop** (`hidden md:*` - passer `workspace.css` en `@layer components` pour lever le piège cascade non-layered AVANT de router), INC-6 surfaces, INC-8 sur-titres restants, INC-9 titres, INC-10 `Select`/`FormField`, INC-4 modale. Puis **c** (compteurs) et **d** (grille). Tout derrière `ff_ui_coherence`, page par page, QA OFF/ON au DOM (⚠ toggle test : mettre la clé `null`, pas `delete`) - **attention aux espaces** (8px). Détail + gotchas → `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation ». Sinon : allumer le flag bandeau (a), ou **Run 7** veille LED (cadrage). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
Le « **backlog dev** » ne liste QUE l'actionnable-par-Claude sans dépendance externe (gestes Pascal → § Chez Pascal ; attentes datées → § Parking ; idées → Watch). No-debt strict ; titre bloqué = `[BLOQUÉ - {qui/quoi débloque}] {action}`. → [[feedback_backlog_wip_anti_gonflement]]. Baseline saine (**2824 verts**, build OK).
...
- [ ] **[EXÉCUTABLE]** Cohérence UI b/c/d - **PART 2 + c + d** (part 1 de b livrée, commité non poussé : boutons `.ws-btn`/pastilles/recherche). Reste b : INC-7 états vides → `EmptyState`, INC-5 recherches recodées → `SearchInput`, prospection CTAs desktop (lever le piège cascade `.ws-btn` non-layered via `@layer components` d'abord), INC-6 surfaces, INC-8/9 sur-titres/titres, INC-10 champs form, INC-4 modale. Puis **c** (compteurs 3 rôles + global/filtré), **d** (grille). Derrière `ff_ui_coherence`, page par page, QA OFF/ON au DOM (toggle test : clé `null` pas `delete`), zéro régression, discipline structure (pas la peau). `redesign-skill` + `refactoring-ui`. → `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation » + [[feedback_gouvernance_benchmark_layout_lisibilite]].
...
- [x] ~~**Cohérence UI - increment b PART 1 (boutons + pastilles + recherche)**~~ - 2026-07-17 (ultracode, commité non poussé). Flag `ff_ui_coherence` (patron ffCrmListesV2). Boutons inline routés vers la primitive EXISTANTE `.ws-btn` (workspace.css, +3 variants soft/outline/danger-ghost + `:disabled`) sur entreprises/contacts/pipeline/signaux/prospection ; pastilles Badge (`crm-badge`)/SourcePill → radius-full ; SearchInput → 40px ; classe `.eyebrow`. Mécanique class-swap (handlers source unique) + overrides co-localisés `:global(.coherence-ui)`. **Workflow 4 agents** (routing parallèle) + **revue adversariale 5 lentilles → 7 findings corrigés** dont **2 régressions OFF** (Badge `:global(.badge)` FeedbackTable + `.eyebrow` global portail/login). QA OFF/ON **prouvée au DOM** (badge 10px/src 8px/search 39px OFF → full/full/40px ON), svelte-check 0/0, Vitest 2827. Reste = PART 2 (états vides, recherches recodées, prospection CTAs, surfaces, titres). → `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation ».
