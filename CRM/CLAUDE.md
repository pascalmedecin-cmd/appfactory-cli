...
**Prochaine attaque** : **Bloc B - Daily Email**, mais entièrement gated sur Pascal → (1) son **smoke auth OTP** (le deploy prod embarque supabase 2.108 + TS6 non encore testés en prod), (2) son **feu vert activation**. Si avancer en autonomie sans Pascal : **Bloc C** (veille Lot 3) ou **Bloc D** (audit 360 = porte de clôture) sont `[EXÉCUTABLE]`. **Séquence de clôture refonte CRM** : ~~A~~ → B (gated Pascal) → C (Lot 3 optionnel) → **D (audit UI 360 = porte de clôture)**. **Bloc A LIVRÉ `f357e68`** (mails de plantage coupés à la racine). Refonte Vagues 1+2+3 live (flag `ffCrmListesV2` ON fondateurs), prod = trunk `main`. → [[project_refonte_crm_cadrage_2026-06-18]] + [[project_bloc_a_mails_plantage_2026-06-26]].
