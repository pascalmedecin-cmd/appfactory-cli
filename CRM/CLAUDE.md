...
**Prochaine attaque** : Bloc 1 - merge `portail-session-1` → `main` - hygiène git légère (prod déjà déployé depuis la branche, `main` en arrière). Sinon pas de gros chantier ; reliquat 4d-copy reste bloqué (re-cadrage V5).
...
- [ ] **[EXÉCUTABLE]** merger la branche dans `main` (Vagues 4 + retrait Sentry + fix Pipeline déjà en prod), pousser.
