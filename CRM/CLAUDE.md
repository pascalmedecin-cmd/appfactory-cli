...
**Dernière mise à jour :** 2026-07-20 (Cohérence UI **lot 2C = increment c « compteurs »** livré derrière `ff_ui_coherence` OFF - ProspectionTabs aligné sur la primitive Tabs (radius `--radius-full` + count actif `--color-primary`, garde `:not(.tab--empty)`) + pastille bandeau signaux `filteredSignaux.length`→`data.signaux.length` (global de vue) ; contacts/pipeline/entreprises/filter-badge déjà conformes, veille exemptée rôle C. QA DOM OFF/ON (3 sondes, cas onglet-actif-vide) + adversarial 4 lentilles (1 CSS + 1 veille adressés). Vitest **2863**. → `docs/COHERENCE-UI-BANDEAU.md`). **Prochain bug :** #001.
...
- [ ] **[EXÉCUTABLE - GATE décision `--content-max`]** Cohérence UI **d1** (gouttière unique + largeur bornée/ancrée à gauche, Veille→gauche) : **cartographié + planifié 2026-07-20** (mécanique « socle centralisateur », ~13 fichiers, tout-ou-rien sur les 10 pages). Prêt à coder derrière `ff_ui_coherence` OFF. Reste à trancher : `--content-max` (reco 1440px). Plan file:line : `.atelier-209/coherence-ui/inventory-d1-implementation-plan.md` + `docs/COHERENCE-UI-BANDEAU.md` § d. Le plus structurel/risqué (spacing-critique, critère dur « attention aux espaces ») → lot dédié, QA avant/après par page. Puis d2 (zones/trame) + d3 (rythme 8).
...
- [x] ~~**Cohérence UI lot 2C = increment c « compteurs »**~~ - 2026-07-20 (ultracode). Derrière `ff_ui_coherence` OFF (2 fichiers prod) : ProspectionTabs aligné sur la primitive Tabs (override co-localisé `:global(.coherence-ui)` : `.tab-count` radius-full + count actif `--color-primary`, **garde `:not(.tab--empty)`** pour préserver le « 0 » gris muted d'un onglet actif-vide) ; pastille bandeau signaux `filteredSignaux.length`→`data.signaux.length` (global de vue, plus le compte filtré client). contacts/pipeline (non paginés) + entreprises + filter-badge = déjà conformes ; veille = exemption rôle C signée (cappé 3 par design, pas de nav vers l'ancien). QA DOM OFF/ON 3 sondes (dont onglet-actif-vide « terrain » = border-gray OFF+ON) + revue adversariale 4 lentilles (off-inertness clean, 2× refuted, 1 CSS + 1 veille adressés), svelte-check 0/0, Vitest 2863. → [[project_coherence_ui_lot2c_2026-07-20]].
...
- [x] ~~**Cohérence UI lot 2A - modales/surfaces/eyebrows/titres**~~ - 2026-07-20 (ultracode). `4e45bb8` derrière `ff_ui_coherence` OFF (16 fichiers) : INC-4 modales radius-2xl, INC-6 shadow-card, INC-8 21 eyebrows, INC-9 titres 700. QA DOM + adversarial 3 lentilles clean, Vitest 2863. → [[project_coherence_ui_lot2a_2026-07-20]].
