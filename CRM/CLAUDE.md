...
**Dernière mise à jour :** 2026-07-17 (cohérence UI - **increment b PART 2 lot 1 livré** `7518cc1`, flag `ff_ui_coherence` OFF : 7 états vides→`EmptyState` + 5 recherches→`SearchInput` + fix ancrage recherche campagnes. Inventaire + routage fan-out, revue adversariale 4 dims → 1 fix placement, 1 a11y différé. QA OFF/ON au DOM, svelte-check 0/0, Vitest 2827). **Prochain bug :** #001.
...
**Prochaine attaque** : **Cohérence UI increment b - PART 2 lot 2** (lot 1 livré `7518cc1`, poussé + déployé prod, flag OFF : 7 états vides→`EmptyState` + 5 recherches→`SearchInput` + fix placement recherche campagnes). Reste b, par ordre de valeur : **états vides différés** (composants partagés SignauxCards/EntreprisesCards/FeedbackTable = passer `coherence` en prop ; panneaux dashboard compacts + widgets premium signés ; `PipelineEmptyState` échelle colonne ; charts/tables/modale/inline), **a11y SearchInput clear-focus** (fix 1 ligne `bind:this`+`focus()` dans `handleClear`, **non flag-gated** = change à part avec sa propre QA - déjà en prod), **recherche Aide** (raccourci `/`+`<kbd>` non exposés par `SearchInput`), **prospection CTAs desktop** (piège cascade `.ws-btn` non-layered - **`@layer components` écarté** : blast-radius app-wide casse OFF ; approche co-localisé/wrapper), INC-6 surfaces, INC-8 sur-titres restants, INC-9 titres, INC-10 `Select`/`FormField`, INC-4 modale. Puis **c** (compteurs) et **d** (grille). Tout derrière `ff_ui_coherence`, page par page, QA OFF/ON au DOM (⚠ toggle test : mettre la clé `null`, pas `delete`) - **attention aux espaces** (8px). Détail + différés → `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation ». Sinon : allumer le flag bandeau (a), ou **Run 7** veille LED (cadrage). Run 4/5 bloqués (Hunter/Pingen chez Pascal). Micro validé AVANT Pascal (→ [[feedback_qa_micro_mon_ressort_gate_pas_excuse]]).
...
- [ ] **[EXÉCUTABLE]** Cohérence UI b/c/d - **PART 2 lot 2 + c + d** (lot 1 livré `7518cc1` : 7 états vides→`EmptyState`, 5 recherches→`SearchInput`, fix ancrage campagnes). Reste b : **états vides différés** (composants partagés SignauxCards/EntreprisesCards/FeedbackTable = passer `coherence` en prop ; panneaux dashboard compacts + widgets premium signés ; `PipelineEmptyState` échelle colonne ; charts/tables/modale/inline), **a11y `SearchInput` clear-focus** (fix 1 ligne `bind:this`+`.focus()` dans `handleClear`, **NON flag-gated** = change à part avec sa QA, déjà en prod), recherche Aide (raccourci `/`+`kbd`), prospection CTAs desktop (**`@layer components` écarté** - blast-radius casse OFF ; approche co-localisé/wrapper ; bouton « Enrichir » sans variante `.ws-btn`), INC-6/8/9/10/4. Puis **c** (compteurs 3 rôles + global/filtré), **d** (grille). Derrière `ff_ui_coherence`, page par page, QA OFF/ON au DOM (toggle test : clé `null` pas `delete`), zéro régression, discipline structure (pas la peau). `redesign-skill` + `refactoring-ui`. → `docs/COHERENCE-UI-BANDEAU.md` § « État d'implémentation » + [[feedback_gouvernance_benchmark_layout_lisibilite]].
...
- [x] ~~**Cohérence UI - increment b PART 2 lot 1 (états vides + recherches)**~~ - 2026-07-17 (ultracode, `7518cc1`, flag OFF). 7 états vides→`EmptyState` + 5 recherches→`SearchInput` (7 fichiers page), pattern component-swap OFF byte-verbatim. Inventaire fan-out (8 agents) + routage fan-out (7 agents) + revue adversariale 4 dims → 2 findings réels (fix ancrage recherche campagnes `.coh-search` ; a11y `SearchInput` clear-focus différé, non flag-gated). QA OFF/ON au DOM (veille carte→EmptyState 48px, campagnes search 40px ancré), svelte-check 0/0, Vitest 2827. Reste lot 2. → [[project_coherence_ui_increment_b_part2_2026-07-17]] + `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - increment b PART 1 (boutons + pastilles + recherche)**~~ - 2026-07-17 (ultracode, `3976d6e` poussé/déployé, flag OFF). Boutons→primitive `.ws-btn` (5 pages, +3 variants), Badge/SourcePill→radius-full, SearchInput→40px, derrière `ff_ui_coherence` OFF. Workflow 4 agents + revue adversariale 5 lentilles/7 findings (dont 2 régressions OFF), QA OFF/ON prouvée au DOM, Vitest 2827. Reste part 2. → [[project_coherence_ui_increment_b_part1_2026-07-17]] + `docs/COHERENCE-UI-BANDEAU.md`.
- [x] ~~**Cohérence UI - mockup b/c/d validé (direction)**~~ - 2026-07-17 (ultracode). Inventaire CRM réel + benchmark Gouvernance + critique adversariale 6 lentilles (27 corrections) → mockup validé Chrome (bascule FilmPro/LED). Direction + Q1 (largeur bornée ancrée à gauche) validées ; normes b/c/d + « attention aux espaces » gravées → `docs/COHERENCE-UI-BANDEAU.md`. Reste : coder (increment b).
- [x] ~~**Cohérence UI - bandeau de page COMPLET (10 pages + kicker 12px)**~~ - 2026-07-17 (ultracode). Increments 1→3 + Veille derrière `ff_page_bandeau` (OFF) : source unique `isBandeauActive`, OFF = rendu d'origine strict, 1 `h1`/page. QA : svelte-check 0/0, Vitest 2824, build OK. Commits `5e0eea3`/`b4fe644`/`d51fd31`/`e60f1b6`. → `docs/COHERENCE-UI-BANDEAU.md`.
→ Antérieurs (bandeau/Veille, Campagnes, e2e validation, Retours macro, Run 3 QA ; Run 3 import + Run 2 + Run 1, login OTP, veille/étiquettes/refonte/Aide/Découpe/Daily Email) → archives `claude-md-crm-livre-2026-07-{17,16,15,14,03,02}.md` + `-06-26.md` ; mémoires [[project_atelier_209_run3_import_liste_2026-07-16]], [[project_atelier_209_run2_cablage_2026-07-15]], [[project_login_otp_smtp_supabase_2026-07-14]].
