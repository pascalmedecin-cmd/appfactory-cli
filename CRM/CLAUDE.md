...
- [ ] **[EXÉCUTABLE - prochaine session, décision prise]** Cohérence UI **d1** (gouttière unique + largeur bornée/ancrée à gauche, Veille→gauche) : **cartographié + planifié 2026-07-20**, **`--content-max = 1440px` confirmé Pascal 20/07**. Mécanique « socle centralisateur », ~13 fichiers, tout-ou-rien sur les 10 pages. À coder derrière `ff_ui_coherence` OFF, QA avant/après par page + captures pour validation Pascal (critère dur « attention aux espaces »). Plan file:line : `.atelier-209/coherence-ui/inventory-d1-implementation-plan.md` + `docs/COHERENCE-UI-BANDEAU.md` § d. Puis d2 (zones/trame) + d3 (rythme 8).
