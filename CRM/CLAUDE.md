...
**Derniere mise a jour :** 2026-06-23. Prod = alias `filmpro-portail.vercel.app` sur le deploy de `8aa9547` (correctifs veille + page /crm/veille + logo email déployés ; refonte CRM listes flag `ffCrmListesV2` toujours OFF). **Derniere revue /optimize :** 2026-04-05. **Prochain bug :** #001.
**Session courante :** 2026-06-23 - **Veille W25 + 2 emails + chips + dedash LIVRÉS** (`1c7e484`+`8aa9547` poussés main, déployés prod ; W25 régén forcée 0→1 item, « — » retiré ; 2e email brief brandé logo+date → antoine@+pascal@ ; chips redirect `tab=` ; dedash déterministe ; revue 0 H/C/M ; 1988 tests verts). **Pascal revient avec commentaires veille ; W26 = 1er brief réel.** → [[project_veille_w25_emails_chips_dedash_2026-06-23]]. WIP non commité : Campagnes Vague 3.2 (3/6). → [[project_module_campagnes_vague32_2026-06-22]].
...
- **Tests** : Vitest 1988 (dernier run vérifié 2026-06-23) + Playwright e2e (suite + P1/P2/P3 Prospection). Accessibilité : focus trap + ConfirmModal partout, axe-core 0 violation modale P3. Sécurité : Zod sur 20 form actions/endpoints, rate limiting 10/min, headers CSP/XFO/referrer, timing-safe secrets
...
**Prochaine attaque** : **Campagnes Vague 3.2** (multi-étiquetage N-N, 3/6 in-flight, WIP non commité, migration N-N à appliquer). → [[project_module_campagnes_vague32_2026-06-22]]. **Veille W25 + 2 emails + chips + dedash LIVRÉS 2026-06-23** (`1c7e484`+`8aa9547` poussés main, déployés prod). **Pascal revient avec des commentaires veille prochaine session** ; **W26 (vendredi 26/06) = 1er run cron de la refonte + 1er brief brandé réel à antoine@, à surveiller**. → [[project_veille_w25_emails_chips_dedash_2026-06-23]].
...
### 0. Veille - W25/emails/chips/dedash LIVRÉS (2026-06-23), commentaires Pascal + Lot 3 [BLOQUÉ]

- **Livré 2026-06-23** (`1c7e484`+`8aa9547` poussés main, déployés prod ; détail → « Livré cette session ») : fix racine W25 (régén forcée 0→1 item, « — » retiré) + 2e email brief brandé (logo+date) + chips Prospection alignés (bug redirect `tab=`) + dedash déterministe. → [[project_veille_w25_emails_chips_dedash_2026-06-23]] + [[audit_secu_2026-06-23_veille_w25_emails_chips_dedash]].
- [ ] **[BLOQUÉ - commentaires Pascal + mesure W26]** **Veille suite** : Pascal revient avec des commentaires veille prochaine session ; W26 (26/06) = 1er run cron naturel de la refonte + 1er brief brandé réel à antoine@. Surveiller densité items + rendu brief (Gmail/Outlook). → [[project_veille_w25_emails_chips_dedash_2026-06-23]].
- [ ] **[BLOQUÉ - après mesure W26+]** **Lot 3 - structurel (optionnel)** : mémoire longue + dashboard qualité ; boucle de feedback ; PDF de marque (`filmpro-pdf`) ; pont veille→action fin ; élargir sources (OFS, associations, salons) ; décomposer le mono-appel LLM (si plafond persiste) ; nettoyage enum RegBL ; **réécriture chiffre divergent** (cross-check rejette les chiffres non verbatim → W25 n'a sorti qu'1 item) + résiduel LOW. → [[project_veille_refonte_lots12_livree_2026-06-22]].
...
- [x] ~~**Veille - W25 + 2 emails + chips + dedash**~~ - 2026-06-23 (ultracode/xhigh). Commits **`1c7e484`** (feat) + **`8aa9547`** (fix build : exports non-standard interdits depuis route SvelteKit) poussés main, déployés prod (logo HTTP 200, depuis worktree propre = Campagnes WIP exclu). **T1** fix racine W25 = enregistrement figé pré-`de05779` → `--force`/`--no-email` + régén GHA → **0→1 item, « — » retiré** (1 item car cross-check rejette 4/5 chiffres non verbatim = zéro-hallu OK). **T2** 2 emails : récap admin inchangé + brief brandé (navy + logo blanc nuancé + « Semaine du 22 juin 2026 » + signaux + impacts) → antoine@+pascal@ si ≥1 item ; `parseRecipients` multi-dest, `email.to`→`string[]`. **T3** chips : `buildRedirect` pose `tab=` (leads importés enfin visibles) + zefix-only. **T4** dedash déterministe « no tiret long » sur tout le texte publié. Revue adversariale 0 H/C/M + 2 LOW corrigés ; 1988 verts, build OK. Logo sauvegardé durablement → [[reference-filmpro-logos]] + README `shared/filmpro/logos/`. → [[project_veille_w25_emails_chips_dedash_2026-06-23]] + [[audit_secu_2026-06-23_veille_w25_emails_chips_dedash]] + [[feedback-sveltekit-no-extra-route-exports]].
...
- **[WATCH] W26 (vendredi 26/06) = 1er run cron de la refonte veille + 1er brief brandé RÉEL à antoine@** (`8aa9547` sur main, déployé). Surveiller : densité items (W25 régénérée n'a sorti qu'**1 item** = cross-check rejette les chiffres non verbatim sur la page, zéro-hallu OK mais à l'œil sur semaine pleine → candidat « réécriture chiffre divergent » Lot 3), brief reçu + rendu côté Gmail/Outlook (logo PNG), log `phase=mix`/`mix_drift`, `relevance_demoted`. **Pascal revient avec des commentaires veille.** → [[project_veille_w25_emails_chips_dedash_2026-06-23]].
