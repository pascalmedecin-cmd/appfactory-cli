...
**Statut :** Phase C, Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + email récap cron veille (gated) + export/import CSV + page /reporting + sous-projet `formation-ia/` (**S1 → S7 parcours marketing LIVRÉS, 12/12 modules en prod** + **roadmap V2 refondue S105 en 9 blocs** post 6 demandes Pascal, Bloc 0 Pipeline images Phase 1-4 livrée avec 12 images seedées prod scores 7-9/10) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-19 (session 105 formation-ia : refonte roadmap V2 en 9 blocs + Bloc 0 Pipeline images Phase 1-4 livrée. 7 commits prod `edc1822` + `141cf98` + `a74eebc` + `7bb05b6` + `d56b142` + `69e2ac6` + `b77a8c5`. 12 images module marketing seedées prod scores Vision 7-9/10. Combo Flux validé A/B/C : raw=true + enhance_prompt=false + préfixe hyperrealistic. Fix anti-golden brass→pewter. Gates svelte-check 578/0/0, vitest 25/25. 0 modif côté CRM.)
