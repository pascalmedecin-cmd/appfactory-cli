...
**Prochaine attaque** : Bloc #1 - Cascade gabarit /prospection sur 6 pages CRM (/dashboard, /contacts, /entreprises, /pipeline, /signaux, /veille). /prospection validée en prod S163 (Chrome MCP autonome 4 parcours OK + 4 fixes V4 chaînés no-debt commit `612b1ac`) = page modèle. Backlog Vague 4 résiduelle (H-19 sticky multi-col + 11 H bench externe + sweep M-04..M-28) reste reportable post-cascade ou en parallèle session courte selon priorité.
...
**Pourquoi** : /prospection validée en prod S163 = page modèle. Cascade ordonnée des patterns golden v8 (ARIA tabs, dense table, ScorePill, ConfirmModal, focus-visible globaux, empty states contextuels, aria-label tr) sur les 6 autres pages CRM. Évite la dérive de design system d'une page à l'autre.
**Prérequis** : aucun (page modèle figée). Cadrage cascade requis avant 1re page (priorité, ordre, scope par page).

- [ ] **[EXÉCUTABLE]** Cadrage cascade gabarit : audit éclair des 6 pages cibles vs golden v8 (5-10 min/page). Identifier écarts critiques par page (a11y, sémantique, tokens, composants partagés). Proposer ordre d'attaque (priorité usage métier × écart). → voir `notes/audit-uiux-prospection-2026-05-01/verdict.md` § page modèle
...
### 2. Vague 4 résiduelle /prospection [SUPERVISÉ • xhigh • ~5h]

**Pourquoi** : 11 H + ~30 M tracés en backlog en fin Vague 3 (commit `4af587c`) + cadrage dédié H-19 sticky multi-col (incompatible resizable actuel sans casser persistance localStorage). Reportable post-cascade gabarit ou en parallèle session courte.

- [ ] **[EXÉCUTABLE]** Vague 4 résiduelle : H-19 sticky 1ère colonne raison sociale (refactor multi-col, cadrage dédié), H-21 raccourcis clavier global Cmd+K + J/K + ?, M-04..M-11 spacing résiduel (LeadSlideOut, BatchActionsBar, EnrichBatchModal, ImportModal, AlerteModal, RecherchesPanel), M-13..M-18 hauteurs boutons (h-10 unifié hors mobile), M-20..M-22 typo hors échelle (TriageQueue, count tabs), M-23..M-28 opacités custom statiques, M-32..M-37 sémantique résiduelle (EnrichBatchModal aria-labelledby, RecherchesPanel boutons icônes aria-label, items répétés `<ul>/<li>`, `<a target=_blank>` rel=noreferrer LeadSlideOut). → voir `notes/audit-uiux-prospection-2026-05-01/{findings,actionplan}.md` § Vague 4 backlog
...
- [x] ~~Tests E2E Phase 2 prospection - dette honnête S157 fermée~~ - Fait 2026-05-01 (S158, high, ~30 min) : 1 commit `7b859a9` push origin/main. `template/tests/prospection-phase2.spec.ts` ajouté avec 3 tests Playwright contre prod (preset desktop 1440x900 + iPhone 14 Pro Max spread `...devices['iPhone 14 Pro Max']` apportant userAgent Safari iOS + viewport 430×932 + DPR 3 + isMobile + hasTouch). Périmètre : (a) sélecteur entrées/page, (b) colonnes resizables, (c) switch onglet Terrain potentiellement vide + retour SIMAP. **QA** : 6/6 verts contre prod, vitest 406/406 inchangé. **Migration règles tests mobile** global méta → projet AppFactory CLAUDE.md.
