...
- [ ] **[EXÉCUTABLE]** Gate `npm run build` pré-push CRM : ajouter dans `.git/hooks/pre-push` (ou husky si configuré) un check qui run `cd template && npm run build` et bloque le push en cas d'échec. Évaluer impact temps (build ~40s sur Vercel, possiblement 60-90s local) vs valeur (zéro deploy ERROR silencieux). Alternative : check Vercel deploy status via gh API après push avec notification. Décider option avant implémentation. → voir `memory/feedback_vercel_deploy_status_before_audit.md` + `memory/feedback_template_literal_backtick_escape.md`
...
- [ ] **[BLOQUÉ - Session C livrée]** Session D V1 mobile : dashboard reflow grid cartes + aide vérif simple, Lighthouse mobile 8 pages comparaison budgets perf (cf SPEC §5), fix P1-6 boutons modale 104×36 → 44 hauteur, extension golden v5 → annexe mobile (screenshots 390×844), update CLAUDE.md V1 livré. Critères : tous budgets perf respectés ou justifiés, golden mobile capturé, doc à jour. ~1h MIXTE. → voir `memory/project_crm_mobile_v1_cadrage.md` + `notes/audit-mobile-v1-session-A/findings.md`
