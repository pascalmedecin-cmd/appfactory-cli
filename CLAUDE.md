...
**Statut :** Phase C, Skills et templates HTML + module Veille sectorielle refondu LEAN (S112, prod) + email récap cron veille (gated, actif) + export/import CSV + page /reporting + Golden Standards CRM v3 livré + Phase 4 application `/prospection` en prod (S114) + Migration Material Symbols → Lucide complète (S115) + Cascade golden v3 Bloc 1a Phase 3+4 LIVRÉE INTÉGRALEMENT 6/6 pages CRM (S116-S117, 8 commits push main) + **Sweep palette opacités primary + Badge variant info + Select.svelte partagé + golden v4 doc-only (S118, 3 commits + 1 doc)**. Formation IA est un sous-projet autonome dans `formation-ia/`, accessible directement via `cc` option 2.
**Derniere mise a jour :** 2026-04-26 (session S118 CRM xhigh : Bloc 1a sweep opacités primary + Bloc 1e Badge accent → info + Bloc 1b Select.svelte composant partagé + Bloc 1c golden v4 doc-only. 3 commits push main. Vitest 344/344, svelte-check 109/37 baseline maintenue.)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** Session 118 (CRM, 2026-04-26, `/effort xhigh`). 3 commits push main + 1 livrable doc. **Bloc 1a Sweep opacités primary** commit `e9eaf4d` : décision option (b) zéro nouveau token, 31 substitutions / 10 fichiers (`bg-primary/{5,8,10}` → `bg-primary-light`, `border-primary/{12,15,20,30}` static → `border-primary`, hover/focus/ring transients conservés). Badge.svelte (variant default + accent) + 7 pages CRM + 2 composants prospection. **Bloc 1e Badge variant accent → info** commit `5844633` : type Badge `'accent'` retiré + variant `'info'` ajouté (`bg-info-light text-info border-info/15`) + 5 helpers TS basculés (prospection-utils, signaux, contacts, pipeline, entreprises) + Badge Prescripteur 'default'. ModalForm headerVariant 'accent' conservé (pattern indépendant). **Bloc 1b Select.svelte composant partagé** commit `ba2db12` : `template/src/lib/components/Select.svelte` (props label, value bindable, options, placeholder, required, id, name) + 4 selects natifs migrés (pipeline x3 + signaux x1). CantonSelect, FormField et filtres signaux conservés (gabarits différents). **Bloc 1c Golden v4 doc-only** : v4 JSON + .md créés (pas de modif CSS), 2 sections nouvelles `paletteEditorial` (primary-dark/light Sidebar/login/hero) + `typographyEditorial` (mag-* /veille), section `knownDivergences` documente conflit info ardoise (#5A7190) vs sky (#2E90FA), décisions 6 + 7 ajoutées, symlink basculé v3 → v4. **Bilan** : 4 livrables Blocs 1a/1b/1c/1e clos, 1 finding nouveau (divergence info ardoise/sky → Bloc 1 next session). Vitest 344/344, svelte-check 109/37 baseline maintenue.
**Session -1 :** Session 117 (CRM, 2026-04-26, `/effort xhigh`, post-crash CLI précédent). 6 commits majeurs. Cascade Bloc 1a Phase 3+4 4 pages restantes + sweeps cross-app inputs h-[34px] + ghost destructive/annuler + cleanup tokens accent + header 48px golden v3. STOP-cascade règle S116 appliquée 2 fois.
**Session -2 :** Session 116 (CRM, 2026-04-26, `/effort xhigh`). 2 commits majeurs. Bloc 1a Phase 3+4 cascade pages 1-2 (`/dashboard` + `/contacts`) + sweep structurel cross-app `bg-accent → bg-primary` 158 substitutions / 24 fichiers + SlideOut 560 + token --color-primary-hover.
**Session -3 :** Session 115 (CRM, 2026-04-26, `/effort xhigh` AUTONOME). Bloc 1b Migration Material Symbols → Lucide : 207 occurrences éliminées commit atomique `f1a54c5`, wrapper `<Icon name="...">` + `icon-map.ts` (92 mappings).
...
**Prochaine attaque** : Bloc 1 - trancher divergence info ardoise FilmPro vs sky (token `--color-info`). Issu directement v4 golden documenté S118 (knownDivergences). Décision triviale, low effort, débloque cohérence palette pour futur Bloc 2 refonte. Alternative session courte si date ≥ 2026-05-01 : Bloc 2 validation cron veille W18.

### 1. Trancher divergence info ardoise FilmPro vs sky [SUPERVISÉ • low • ~30 min]

**Pourquoi** : v4 golden (livré S118) documente `knownDivergences.info-token-ardoise-vs-sky` : `palette.info = #5A7190` ardoise FilmPro (golden v3+v4) vs `--color-info code = #2E90FA` sky blue (`template/src/app.css`). Badge variant 'info' ajouté S118 rend en sky, pas en ardoise prévu golden. 3 options : (a) réaligner code → golden (`--color-info` → `#5A7190` + `--color-info-light` → `#EDF1F5`), (b) réaligner golden → code (info sky pour cohérence palette web standard), (c) dual-track. Reco (a) : cohérence avec charte FilmPro, intent originel golden v3, impact visuel limité (Badge variant 'info' uniquement post-S118 + alerte signaux dashboard `eddb04b`).
**Prérequis** : aucun.

- [ ] **[EXÉCUTABLE]** Décision Pascal sur 3 options + sweep code OU update golden v5. Vérification chrome MCP /signaux statut « En analyse » + dashboard alerte signaux pour valider rendu final. → voir `memory/project_info_token_ardoise_vs_sky.md` (3 options détaillées + reco a) + `.claude/goldens/audit-uiux-golden-v4-2026-04-26.json` section knownDivergences + `template/src/app.css` L33-34.

### 2. Évaluation cron veille 01/05 + clôture S112 [SUPERVISÉ • low • ~15 min]
...
- [x] ~~Bloc 1c Doc golden v4 (typography.editorial + palette.editorial + knownDivergences)~~ - Fait 2026-04-26 (S118, xhigh) : v4 JSON + .md créés doc-only (pas de modif CSS). 2 sections nouvelles : `paletteEditorial` (primary-dark/light Sidebar/login/hero), `typographyEditorial` (mag-* /veille). Section `knownDivergences` documente conflit info ardoise (#5A7190) vs sky (#2E90FA) → finding ouvre Bloc 1 next session. Décisions 6 + 7 ajoutées (sweep S118). Symlink `audit-uiux-golden-current.json` basculé v3 → v4. JSON validé.
- [x] ~~Bloc 1b Composant Select.svelte partagé~~ - Fait 2026-04-26 (S118, xhigh) : commit `ba2db12`. Composant `template/src/lib/components/Select.svelte` (props label, value bindable, options, placeholder, required, id, name) + 4 selects natifs migrés (pipeline x3 entreprise/contact/etape + signaux x1 type_signal). CantonSelect, FormField et filtres signaux conservés (gabarits différents). Vitest 344/344, svelte-check 109/37 baseline.
- [x] ~~Bloc 1e Badge variant 'accent' → 'info'~~ - Fait 2026-04-26 (S118, xhigh) : commit `5844633`. Badge.svelte type variant `'accent'` retiré + variant `'info'` ajouté (`bg-info-light text-info border-info/15`) + dotColors.info. 5 helpers TS basculés (prospection-utils, signaux, contacts, pipeline, entreprises). Badge Prescripteur 'default'. ModalForm headerVariant 'accent' conservé (pattern indépendant : header bleu plein bg-primary, sémantique distincte).
- [x] ~~Bloc 1a Sweep opacités primary custom → tokens sémantiques~~ - Fait 2026-04-26 (S118, xhigh) : commit `e9eaf4d`. Décision option (b) zéro nouveau token. 31 substitutions / 10 fichiers : `bg-primary/{5,8,10}` → `bg-primary-light`, `border-primary/{12,15,20,30}` static → `border-primary`. Hover/focus/ring transients conservés (pattern UA standard). Badge.svelte (variant default + accent), 7 pages CRM (dashboard, contacts, entreprises, pipeline, signaux, veille, prospection), 2 composants prospection (RecherchesPanel, TemperatureSelect).
- [x] ~~Cascade Bloc 1a Phase 3+4 LIVRÉE INTÉGRALEMENT 6/6 pages CRM~~ - Fait 2026-04-26 (S117, xhigh) : 6 commits push main (`7671a52` cleanup tokens accent + sweep space-y-6, `3b8a735` --header-height 48px, `1f56416` /entreprises + sweeps ghost, `03f8815` /pipeline + sweep inputs h-[34px], `ae8ae78` /signaux 5 CTA + paddings, `8447c05` /veille mappings tokens + 2 CTA). 4 pages auditées + STOP-cascade règle S116 appliquée 2 fois.
