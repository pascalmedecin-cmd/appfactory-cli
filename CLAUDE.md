...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + email récap cron veille (gated) + export/import CSV + page /reporting + sous-projet `formation-ia/` (**S1 + S2 + S3 + S4 parcours marketing livrées** : catalogue + ressources + matière pédago 111 entrées + 6 modules en prod (3 fondations + 3 production : brief-produit, ton-de-marque, social-media) + bug UI « 5 questions » fixé, prêt pour S5) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-17 (session 76 : S4 ingestion production 1 + fix label hardcodé `5 questions`, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente :** Session 76 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort xhigh`). **S4 ingestion production 1 + fix UI label livrés bout en bout** (~50 min, time-box 2h respecté). (1) **Fix label « 5 questions » hardcodé** : commit formation-ia `7805be0` + deploy auto Ready. Page `/parcours/[slug]/jour/[n]/+page.svelte` ligne ~81 affichait « 5 questions » en dur alors que J1-J3 ont 4 questions. Fix : count Supabase `quiz_questions.jour_id` dans `+page.server.ts` + rendu dynamique singulier/pluriel `{quizCount} question{s}` + fallback `Quiz en préparation`. (2) **S4 ingestion 3 modules production** : seed prod Supabase (zéro commit code, JSON gitignoré). 8 outils (4 existants + NotebookLM + Jasper + Metricool + Buffer) + 3 jours (J4 brief-produit, J5 ton-de-marque, J6 social-media) + 9 exercices + 19 liens outils + 12 questions quiz. Stratégie ressources = 13 URLs documentaires officiels (pivot S3 maintenu : YouTube géo CH bloque). **3 stats VÉRIFIÉES HubSpot 2026** exploitées (80% création contenu, 75% production média) + AI Act Article 50 (sanctions 15 M€ ou 3% CA mondial) + Loi Influence 9 juin 2023 + CNIL + Anthropic Projects/news + Google NotebookLM + BDM 2025. Gates PEDAGOGIE auto 0 errors / 0 warns sur 11 gates. Smoke test local 7/7 routes en 200, prod 303 (auth normale, DB partagée local/prod garantit live). Côté CRM : aucune modif.
**Session precedente -1 :** Session 75 (scope formation-ia, docs-only côté CRM). Deux tâches priorité bouclées avant S4. (1) **Fix markdown brut dans `exercices.consigne`** : commit formation-ia `8790751` + deploy auto Ready. Option C retenue (mini-parser maison) — helper `src/lib/formatting/consigne.ts` + 7 tests Vitest + câblage `{@html consigne_html}` + styles scoped. Gates : check 0/0, Vitest 53/53, build OK. Audit local + prod : 4 `<li>`, 2 `<strong>`, zéro astérisque brut. (2) **Rotation `SUPABASE_SERVICE_ROLE_KEY`** : nouvelle clé format `sb_secret_*`, MAJ `.env` local via `open -e` TextEdit + MAJ Vercel prod via `vercel env rm/add` (valeur piped), redeploy `af05658` Ready 14s, smoke test prod OK, ancienne clé supprimée. Zéro impact CRM FilmPro (projet Supabase séparé). Feedback consigné : Pascal préfère GUI simple (TextEdit) aux éditeurs terminal ou scripts sed/Python pour actions manuelles (`memory/feedback_prefer_gui_over_scripts.md`). Côté CRM : aucune modif.
**Session precedente -2 :** Session 74 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort xhigh`). **S3 ingestion fondations livrée bout en bout** : commit formation-ia `b85010c`, push + deploy auto Vercel Ready (4 files +245/-9). (1) 3 modules famille `fondations` seedés en prod Supabase (J1 `comment-ca-marche`, J2 `assistants-panorama`, J3 `prompt-5-blocs`) : 9 exercices + 12 questions quiz + 21 liens outils, 4 outils (Claude, ChatGPT, Gemini, Mistral Le Chat) avec plan_gratuit_note. (2) Câblage `ressources` + `check_questions` dans page exercice (dette S1 résorbée) + fix SSR `MiniCheck.svelte` (init directe vs `$effect`). (3) Script `scripts/seed-parcours.mjs` idempotent ESM pur. (4) Pivot transparent zéro vidéo embed : gate YouTube géo CH bloque WebFetch, ressources = 100% liens documentaires officiels (7 URLs VÉRIFIÉES : Anthropic platform docs, Google AI prompting, Anthropic news, Mistral news, BDM, CNIL, HubSpot 2026). (5) Gates verts : check 0/0, Vitest 46/46, build OK, audit Chrome MCP local + prod, console clean, non-régression accueil + allowlist. **2 dettes flaggées AVANT S4** (toutes deux résolues session 75) : markdown brut dans consignes + rotation `SUPABASE_SERVICE_ROLE_KEY`. Côté CRM : aucune modif.
**Session precedente -3 :** Session 73 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort high`). **S2 matière pédago livrée bout en bout** : commit formation-ia `b7aa108`, push OK, deploy auto Vercel Ready. (1) **`docs/SOURCES_MARKETING.md`** (207 lignes, committé) : 111 entrées consolidées (12 VÉRIFIÉ + 71 TOLÉRÉ AVEC DISCLAIMER + 28 CHAÎNES CANDIDATES). 6 sources VÉRIFIÉ via WebFetch direct : AI Act Art. 50 (2 août 2026), CNIL + PANAME (26 fév 2026), Anthropic news (Opus 4.7 + Sonnet 4.6 + Skills), Mistral news (Voxtral TTS 23 mars 2026), HubSpot State of Marketing 2026, BDM comparatif IA (20 nov 2025). (2) **`content/research-synthesis-marketing.md`** (476 lignes, gitignoré via `.gitignore` MAJ) : 12 fiches modules exhaustives (cas ≥ 2, stats ≥ 1, vidéos candidates, angles pédago 2-3/module). (3) **Pivot méthodologique transparent** : vérification HEAD 200 des vidéos spécifiques reportée à S3-S7 au moment de l'embed. Côté CRM : aucune modif. Temps 1h20.
**Session precedente -4 :** Session 72 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort xhigh`). **S1 parcours marketing livrée bout en bout** : commit formation-ia `9966112`, 11 fichiers, +874 / -68. (1) **Migration SQL additive** `20260417000001_catalog_refactor.sql` appliquée en prod Supabase : 4 colonnes NULLables (`jours.duree_estimee`, `jours.famille`, `exercices.ressources` jsonb, `exercices.check_questions` jsonb) + contraintes CHECK + index `jours_famille_idx`. (2) **Helpers purs** `src/lib/catalog.ts` + tests. (3) **Schémas Zod** `src/lib/schemas/content.ts`. (4) **Composants** `RessourceVideo.svelte` et `MiniCheck.svelte`. (5) **Refonte UI** `/parcours/[slug]` : grille catalogue groupée par famille. **Gates verts** : 46/46 Vitest, check 0/0, build OK. Deploy auto Ready. Côté CRM : aucune modif.
**Session precedente -5 :** Session 71 (scope formation-ia uniquement, docs-only côté CRM). **Cadrage parcours marketing validé** : pivot paradigme acté - le parcours marketing devient une **bibliothèque de 12 modules indépendants** (pas un parcours linéaire 5 jours). **Livrables session (commit formation-ia `65c40d5` docs-only)** : `docs/PEDAGOGIE.md` mis à jour + `docs/PLAN_PARCOURS_MARKETING.md` créé (payload exhaustif 7 sessions autonomes S1-S7) + `docs/SOURCES_MARKETING.md` créé (squelette) + `formation-ia/CLAUDE.md` réécrit. **Garde-fous actés** : zéro régression + zéro hallucination nuancée. Payload : `memory/project_formation_ia_plan_parcours_marketing.md`.
**Session precedente -6 :** Session 70 (scope CRM FilmPro + méta - batch autonome validé par Pascal en Opus 4.7 xhigh + `--dangerously-skip-permissions`). **4 commits CRM pushés** (`1c15e4a..12d8bc5`), 411/411 tests verts (77 nouveaux), zéro régression typecheck. (1) **Fix latent cron veille post-migration Opus 4.7** - commit `e1353ba` : retire `temperature: TEMP_PHASE{1,2}` rejeté 400 sur 4.7 + bump `effort: 'high'` → `'xhigh'`. (2) **Email récap veille post-cron** - commit `e0f0b32` : modules `cost-tracker.ts` + `email-recap.ts` + hook `run-generation.ts`. (3) **Export CSV + Import CLI + Reporting** - commit `12d8bc5` : 3 endpoints GET `/api/export/[entity]` + script `scripts/import-csv.ts` CLI + page `/reporting`. **Audit 1j (rapport seul, pas de commit)** : `.claude/commands/cadrage.md` + `deploy.md` à actualiser - tracé en tâche `[EXÉCUTABLE - ~30 min]`.
