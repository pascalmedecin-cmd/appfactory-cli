...
**Statut :** Phase C, dette dedup `createExpress` multi-sites FERMÉE (S159 2026-05-01 high : fail(409) ambiguous + step modale UI + force_create=1 + 4 tests vitest). E2E Phase 2 prospection FERMÉE (S158). Phase 2 LIVRÉE (S157). V2 mobile terrain CLOS (S127 α + S129 β + S130 γ). V1 MOBILE CLOS (S125). Formation IA = sous-projet autonome dans `formation-ia/`, `cc` option 2.
**Derniere mise a jour :** 2026-05-01 (S159 CRM high : dette dedup createExpress fermée. 1 commit `96575ec` push origin/main. Server `+page.server.ts` désambiguïsé via `fail(409, ambiguous=true, candidates[])` quand telNorm<6 ET candidates>1 ; flag `force_create=1` bypasse. UI `LeadExpress.svelte` : step désambiguation liste candidats + retour formulaire intact. 4 tests vitest mock Supabase chainable thenable offline. vitest 410/410, build prod 17s, svelte-check 0 nouvelle erreur).
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** Session 159 (CRM, 2026-05-01, `/effort high`, Bloc #1 dedup `createExpress`).
...
**Prochaine attaque** : Bloc #1 - Audit UX/UI 360 /prospection (page modèle). Phase 2 LIVRÉE (S157) + dette E2E + dedup createExpress fermées (S158/S159) → /prospection est candidate à devenir page modèle interne. Audit exhaustif desktop + mobile (DevTools manuel Pascal pour qualitatif, Playwright preset iPhone 14 Pro Max pour objectifs) sur axes hiérarchie / spacing / a11y / feedback states / cohérence golden v8 → produire findings actionnables. Blocs B1 (cron veille W18) et B2 (Phase 3 généalogie + dashboard coûts) restent BLOQUÉS.
...
### 1. Audit UX/UI 360 /prospection - page modèle [SUPERVISÉ • xhigh • session dédiée]

**Pourquoi** : /prospection est candidate à devenir page modèle interne (référence golden v8) après livraison Phase 2 + dedup. Audit exhaustif desktop + mobile sur axes hiérarchie / spacing / a11y / feedback states / cohérence charte → produire findings actionnables avant tentation de dupliquer le pattern sur les autres pages CRM.
**Prérequis** : aucun. Validation visuelle Pascal Phase 2 prod en parallèle (1-2 jours) débloque Phase 3.

- [ ] **[EXÉCUTABLE]** Audit UX /prospection : (a) hiérarchie visuelle 4 onglets + tableau dense + indicateurs flat + TriageQueue, (b) responsive desktop 1440 / tablette 1024 / mobile 430 (DevTools manuel Pascal pour qualitatif, Playwright preset iPhone 14 Pro Max pour objectifs), (c) a11y axe-core sur tous les états (vide / avec data / loading / error / 4 onglets), (d) feedback states (loading skeleton, empty state, filtre incompatible, success/error toasts), (e) cohérence golden v8 (tokens couleurs, scale spacing, typographie). Findings groupés par axe et sévérité.
