...
- **S80-S104** (formation-ia, 2026-04-17 / 2026-04-19) : chantier V2 formation-ia (post livraison parcours marketing 12/12 S79). Audit V1 vs best-practices + bench 15 sites + roadmap V2 11 tâches + cadrage 4 maquettes UX/UI. 2 specs : `docs/V2_UX_UI_SPEC.md` 738 lignes + `docs/COCKPIT_ADMIN_SPEC.md` 538 lignes. Hotfix prod S95 RLS allowlist. **S98** T6 Pack 1 MVP V2. **S99** T7 Pack 2 V2 (DashboardResume + AnchorNav + 5 sections + 13 covers SVG + 19 logos + endpoint PDF pdfkit + 7 composants + 2 migrations DB). **S100** Groupe dette 1D (a11y 9→0 axe-core + tokens --deep + scripts/audit-a11y/ + anti-hype intros). **S101** T8 Cockpit vidéos v1+v2 livré prod bout en bout ~4h xhigh (pages admin/videos + endpoint source + cron + 3 migrations + RPC atomique + 7 fixes UX). **S102** T8 `[VALIDÉ]` règle « Livrable fermé » : 7 bugs Pascal + nouvelle page emplacements + presets j1-j12 + axe-core 0 serious 11 routes. **S103** T5 ThemeCardPlaceholder (variante photo grise, wizard port 3337, commit `07c3318`) + T9 Cockpit niveaux apprenant v1 barrière descendante admin via `allowlist.niveau_max` (commit `ba26631`, invalidé en S104). **S104** Refonte T9 : `niveau_max` → `niveaux_ouverts text[]` multi-select libre + modal conflit retrait si user a commencé un module du niveau retiré. 2 commits prod `f71efc1` (audit code T9 : barrière serveur 3 routes + cadratin retiré + toast enhance callback) + `4adab94` (refonte multi-select : migrations 7+8, helpers + guards + form action conflit + `NiveauxCheckboxes.svelte` + `ConfirmModal danger`). Gates svelte-check 575/0/0, vitest 169/169 (+12), build 1.79s. Modal conflit 100% unit-tested, 0 smoke live (0 progression DB prod). État roadmap V2 post-S104 : **T1-T9 VALIDÉ (9/11)**, reste T10 upload + T11 analytics + T7.5 éditorial objectifs. Dette héritée T1-T9 à zéro. Règle « Livrable fermé » ajoutée à `~/.claude/rules/methodology.md` § Garde-fous (S99). 0 modif côté CRM. Détails session par session : `formation-ia/CLAUDE.md`.
