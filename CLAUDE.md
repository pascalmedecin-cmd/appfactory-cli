...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + email récap cron veille (gated) + export/import CSV + page /reporting + sous-projet `formation-ia/` (**S1 + S2 + S3 + S4 + S5 + S6 parcours marketing livrées** : catalogue + ressources + matière pédago 111 entrées + 10 modules en prod sur 12 (3 fondations + 7 production : brief-produit, ton-de-marque, social-media, visuels-marketing, presentations-pro, video-courte, audio-voix) + 22 outils, prêt pour S7 analyse/cadre) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-17 (session 78 : S6 ingestion `video-courte` + `audio-voix`, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente :** Session 78 (scope formation-ia uniquement, zéro modif côté CRM - session autonome `/effort xhigh`, time-box ~25 min sur 2h budget). **S6 ingestion 2 modules `production` livrés bout en bout** : seed prod Supabase (zéro commit code, JSON `content/marketing-production-3-2026-04-17.json` gitignoré, ~440 lignes). 6 nouveaux outils (`google-veo`, `runway`, `heygen`, `synthesia`, `elevenlabs`, `descript`) + 3 existants réutilisés (`claude`, `notebooklm`, `mistral-le-chat` pour Voxtral TTS mars 2026) + 2 jours (J9 `video-courte`, J10 `audio-voix`) + 8 exercices (4 par module) + 17 liens outils + 8 questions quiz (4 par module). Stratégie ressources = 12 URLs documentaires officiels (DeepMind Veo + SynthID, Runway Research, HeyGen avatars, CNIL biométrique, ElevenLabs pricing/voice-cloning, NotebookLM + Google Blog Audio Overviews, Descript, AI Act Art. 50, Anthropic prompting, BDM 2025). Sources VÉRIFIÉES/TOLÉRÉES exploitées : HubSpot 2026 (75% production média), AI Act Article 50 (15 M€ ou 3% CA mondial, 2 août 2026), CNIL voix = donnée biométrique RGPD article 9, Artificial Analysis leaderboard Runway Gen-4 Elo 1247, NotebookLM Audio Overviews FR depuis 29 avril 2025, Mistral Voxtral TTS 23 mars 2026, ElevenLabs plans Free/Starter 5 USD/Creator 22 USD, Tennessee ELVIS Act 2024 + California Civil Code 3344, SynthID + watermark Veo 3.1 double couche, HeyGen G2 Fastest Growing 2026 + 140-230 avatars 160+ langues, Synthesia SOC 2 + RGPD + ISO 27001. Rédaction Opus tenue en une passe. Gates PEDAGOGIE auto 0 errors / 0 warns sur 11 gates. Smoke local 8/8 routes en 200, prod 8/8 en 303 (auth normale, DB partagée). DB prod : 10 modules J1-J10 + 22 outils + 34 exercices + 40 quiz questions + 76 liens outils. Côté CRM : aucune modif.
**Session precedente -1 :** Session 77 (scope formation-ia, docs-only côté CRM - session autonome `/effort xhigh`, ~30 min). **S5 ingestion 2 modules `production` longs livrés bout en bout** : seed prod Supabase (zéro commit code). 10 outils (2 existants + 8 nouveaux : `midjourney`, `adobe-firefly`, `ideogram`, `canva`, `gamma`, `beautiful-ai`, `plus-ai`, `copilot-powerpoint`) + 2 jours (J7 `visuels-marketing`, J8 `presentations-pro`) + 8 exercices + 19 liens + 8 quiz. Sources VÉRIFIÉES : HubSpot 2026 (75% production média), AI Act Article 50, C2PA standard, Adobe Firefly (Adobe Stock + domaine public, indemnisation IP plan Enterprise). Gates 0/0. Pas de splittage Opus séquentiel nécessaire. DB prod à fin session 77 : 8 modules J1-J8 + 16 outils. Côté CRM : aucune modif.
**Sessions précédentes (condensé)** - détail complet : `archive/decisions-sessions-70-75.md`.

- **Session 76** (formation-ia, 2026-04-17) : S4 ingestion production 1 + fix UI label « 5 questions » hardcodé (commit `7805be0`). 3 modules (J4-J6 brief-produit/ton-de-marque/social-media), 8 outils, 9 exercices, 19 liens, 12 quiz. Gates 0/0.
- **Session 75** (formation-ia, 2026-04-17) : hygiène avant S4 - fix markdown brut consignes (commit `8790751`) + rotation `SUPABASE_SERVICE_ROLE_KEY` (`sb_secret_*`).
- **Session 74** (formation-ia, 2026-04-17) : S3 ingestion fondations - 3 modules seedés (commit `b85010c`, 9 exercices + 12 quiz + 21 liens, script idempotent, pivot zéro vidéo embed).
- **Session 73** (formation-ia, 2026-04-17) : S2 matière pédago - `docs/SOURCES_MARKETING.md` (111 entrées) + `content/research-synthesis-marketing.md` (476 lignes, 12 fiches) (commit `b7aa108`).
- **Session 72** (formation-ia, 2026-04-17) : S1 infra technique - migration SQL additive 4 colonnes + helpers + schémas Zod + composants + refonte UI catalogue par famille (commit `9966112`).
- **Session 71** (formation-ia, 2026-04-17) : cadrage parcours marketing - pivot **bibliothèque 12 modules indépendants** (commit `65c40d5`).
- **Session 70** (CRM FilmPro + méta, 2026-04-17) : batch autonome xhigh - fix cron veille Opus 4.7 (`e1353ba`) + email récap post-cron (`e0f0b32`) + export CSV + import CLI + reporting (`12d8bc5`). 411/411 tests verts.
