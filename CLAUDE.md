...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + sous-projet `formation-ia/` (S2 livrée + UI refresh + générateur retiré + dette <title> résolue + GitHub→Vercel lié) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-16 (session 69 : formation-ia uniquement — ajout `<title>` sur toutes les routes + activation deploys auto GitHub→Vercel, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente -2 :** Session 68ter (docs-only côté CRM ; grosse livraison côté formation-ia). Refonte `/start` AppFactory en **dispatcher 2 branches** : menu racine affiche `[1] CRM FilmPro` / `[2] Formation IA`, chaque branche bascule vers le `/start` complet scopé au sous-projet (tâches, idées, options projet). Règle : tâches linéaires une fois dans un sous-menu, plus de groupement cross-projet. Côté formation-ia, workflow conversationnel d'ingestion de contenu pédagogique créé : `docs/PEDAGOGIE.md` (8 règles + anti-patterns + gates bloquants, adapté Enseignement) + `docs/INGESTION.md` (protocole 6 étapes Opus 4.6, validation par jour, seed idempotent) + `.claude/start-menu.md` Formation IA avec option `[3] Intégrer un parcours`. Multi-thèmes prévu (marketing aujourd'hui, opération/commercial demain, même pipeline). **Règle modèle** : production contenu pédago = Opus 4.6 obligatoire, jamais Sonnet, jamais API in-app (cf. `memory/feedback_opus_for_pedago_content.md`). **En attente** : deep research marketing Claude chat (Pascal la produit côté Claude chat, fichier iCloud initial reçu vide suite à crash Claude AI). Commits : AppFactory `820cb44` + formation-ia `6980086` pushés. (Session 67 archivée : pipeline images 4 niveaux Flux 1.1 Pro Ultra + retrait Pexels/Unsplash + **QA visuel images rejeté en fin de session** — contexte conservé dans `memory/project_qa_images_veille.md`, récupérable via `git log` pour le détail technique.)
**Session precedente -1 :** Session 68quater (scope formation-ia uniquement, docs-only côté CRM). Audit santé prod formation-ia (5 pages auditables : accueil, parcours/marketing, générateur, historique, admin/allowlist) en attente deep research → zéro régression, console clean, dette mineure `<title>` vide identifiée (jamais settée en S2/68bis). Décision Pascal : **retrait complet du générateur de prompts** (« inutile, on ajoutera un module de formation sur best prompting »). Commit formation-ia `a384c75` : suppression routes `/generateur-prompts/*` + nav items + section Outils accueil + 3 refs `prompt_templates` dans INGESTION.md. Tables `user_prompts` + `prompt_templates` conservées inertes (pattern badges). Deploy prod manuel `dpl_CMFdiiFsMTs6ZdLSKGuPjTQjxfvN` après confirmation que push GitHub seul ne déclenche aucun deploy (repo non lié à Vercel). Côté CRM FilmPro : aucune modif code. **Note hors session** : `scripts/image-rework/` reste untracked à la racine (hérité d'une session précédente d'expérimentation pipeline images, scripts datés 2026-04-16 13h, à statuer avec Pascal — commit, gitignore, ou suppression).
**Session precedente :** Session 69 (scope formation-ia uniquement, docs-only côté CRM). Deux tâches exécutables bouclées en attente de la deep research marketing. (1) **Dette SEO + a11y `<title>`** résolue : `<svelte:head><title>` ajouté sur les 7 routes (accueil, login, admin/allowlist, parcours/[slug], jour/[n], exercice/[m], quiz). Titres dynamiques pour `[slug]` / `[m]` via data. Commit formation-ia `32c74cd` + deploy `dpl_B8g7kuwBu8HUi9hGqLfZvejjsjfF`. (2) **Friction 68quater levée** : repo `onboarding-ia` lié au projet Vercel (Vercel GitHub App perms étendues côté GitHub → `vercel git connect` CLI), test bout-en-bout avec commit vide `5bf14ba` → deploy auto `dpl_7au1agn0h` en 15s. Plus de `npx vercel --prod` manuel pour formation-ia. **Quiproquo mi-session** : Pascal a d'abord connecté le mauvais repo (`appfactory-cli` au lieu de `onboarding-ia`) au projet Vercel onboarding-ia, corrigé dans la foulée — leçon captée en mémoire projet (pas-à-pas Vercel multi-projets doit nommer le repo cible sans ambiguïté). Côté CRM FilmPro : aucune modif code **de cette session**. `scripts/image-rework/` toujours untracked. **Migration `generate.ts` Opus 4.6 → 4.7 commitée hors session** : commit `499f137` (Pascal via terminal parallèle, 2026-04-16 18:42). Phase 1 + Phase 2 passent en Opus 4.7 avec `thinking: {type: 'adaptive'}` + `output_config: {effort: 'high'}` (spread cast TS via `Record<string, unknown>` car SDK 0.88 ne type pas encore `output_config`). Cohérente avec règle globale `feedback_latest_model_version.md` (« dernière version à prix égal »). Pilote le cron veille hebdo — à observer sur le 1er run post-migration (coût/latence vs baseline 4.6, 3 ERRORs TS préexistantes signalées dans le message de commit, indépendantes).
