...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + email récap cron veille (gated) + export/import CSV + page /reporting + sous-projet `formation-ia/` (**S1 + S2 + S3 parcours marketing livrées** : catalogue + ressources + matière pédago 111 entrées + 3 modules fondations seedés en prod, prêt pour S4) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-17 (session 75 : hygiène formation-ia — fix markdown consignes + rotation service_role_key, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente :** Session 75 (scope formation-ia, docs-only côté CRM). Deux tâches priorité bouclées avant S4. (1) **Fix markdown brut dans `exercices.consigne`** : commit formation-ia `8790751` + deploy auto Ready. Option C retenue (mini-parser maison) — helper `src/lib/formatting/consigne.ts` + 7 tests Vitest + câblage `{@html consigne_html}` + styles scoped. Gates : check 0/0, Vitest 53/53, build OK. Audit local + prod : 4 `<li>`, 2 `<strong>`, zéro astérisque brut. (2) **Rotation `SUPABASE_SERVICE_ROLE_KEY`** : nouvelle clé format `sb_secret_*`, MAJ `.env` local via `open -e` TextEdit + MAJ Vercel prod via `vercel env rm/add` (valeur piped), redeploy `af05658` Ready 14s, smoke test prod OK, ancienne clé supprimée. Zéro impact CRM FilmPro (projet Supabase séparé). Feedback consigné : Pascal préfère GUI simple (TextEdit) aux éditeurs terminal ou scripts sed/Python pour actions manuelles (`memory/feedback_prefer_gui_over_scripts.md`). Côté CRM : aucune modif.
**Session precedente -5 :** Session 70 (scope CRM FilmPro + méta - batch autonome validé par Pascal en Opus 4.7 xhigh + `--dangerously-skip-permissions`). **4 commits CRM pushés** (`1c15e4a..12d8bc5`), 411/411 tests verts (77 nouveaux), zéro régression typecheck. (1) **Fix latent cron veille post-migration Opus 4.7** - commit `e1353ba` : retire `temperature: TEMP_PHASE{1,2}` rejeté 400 sur 4.7 + bump `effort: 'high'` → `'xhigh'`. (2) **Email récap veille post-cron** - commit `e0f0b32` : modules `cost-tracker.ts` + `email-recap.ts` + hook `run-generation.ts`. Gate par `EMAIL_RECAP_ENABLED` désactivé par défaut. (3) **Export CSV + Import CLI + Reporting** - commit `12d8bc5` : 3 endpoints GET `/api/export/[entity]` + script `scripts/import-csv.ts` CLI + page `/reporting` (4 KPIs + 2 bar charts SVG inline). **Audit 1j (rapport seul, pas de commit)** : `.claude/commands/cadrage.md` + `deploy.md` à actualiser - tracé en tâche `[EXÉCUTABLE - ~30 min]`.
**Session precedente -4 :** Session 71 (scope formation-ia uniquement, docs-only côté CRM). **Cadrage parcours marketing validé** : pivot paradigme acté - le parcours marketing devient une **bibliothèque de 12 modules indépendants** (pas un parcours linéaire 5 jours). **Livrables session (commit formation-ia `65c40d5` docs-only)** : `docs/PEDAGOGIE.md` mis à jour (règle source nuancée + Claude/ChatGPT autorisés + 2 nouveaux gates bloquants) + `docs/PLAN_PARCOURS_MARKETING.md` créé (payload exhaustif 7 sessions autonomes S1-S7) + `docs/SOURCES_MARKETING.md` créé (squelette) + `formation-ia/CLAUDE.md` réécrit. **Garde-fous actés** : zéro régression + zéro hallucination nuancée. Payload : `memory/project_formation_ia_plan_parcours_marketing.md`.
**Session precedente -3 :** Session 72 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort xhigh`). **S1 parcours marketing livrée bout en bout** : commit formation-ia `9966112`, 11 fichiers, +874 / -68. (1) **Migration SQL additive** `20260417000001_catalog_refactor.sql` appliquée en prod Supabase : 4 colonnes NULLables (`jours.duree_estimee`, `jours.famille`, `exercices.ressources` jsonb, `exercices.check_questions` jsonb) + contraintes CHECK + index `jours_famille_idx`. (2) **Helpers purs** `src/lib/catalog.ts` (formatDuree, groupByFamille, labelStatut, FAMILLE_ORDER) avec 14 tests Vitest. (3) **Schémas Zod** `src/lib/schemas/content.ts` : `ressourceSchema` union discriminée video/lien + `checkQuestionSchema` 1-2 Q. 11 tests. (4) **Composants** `RessourceVideo.svelte` (iframe 16:9 youtube-nocookie + vimeo dnt, sandbox strict, conversion auto URLs, fallback lien) et `MiniCheck.svelte` (feedback immédiat, pas de persistence, `$effect` fix warning Svelte 5). (5) **Refonte UI** `/parcours/[slug]` : grille catalogue groupée par famille, libellé « jour » → « module », empty state gracieux. **Gates verts** : 46/46 Vitest (25 nouveaux), svelte-check 0 erreur, build OK. **Audit prod + local** : accueil + admin/allowlist identiques before/after, empty state rendu correctement, console clean. Deploy auto Vercel Ready (15s après push). **CSP non ajoutée** : décision loggée (aucune CSP globale existante, sandbox iframe suffit). Côté CRM : aucune modif. `scripts/image-rework/` toujours untracked à la racine AppFactory.
**Session precedente -2 :** Session 74 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort xhigh`). **S3 ingestion fondations livrée bout en bout** : commit formation-ia `b85010c`, push + deploy auto Vercel Ready (4 files +245/-9). (1) 3 modules famille `fondations` seedés en prod Supabase (J1 `comment-ca-marche`, J2 `assistants-panorama`, J3 `prompt-5-blocs`) : 9 exercices + 12 questions quiz + 21 liens outils, 4 outils (Claude, ChatGPT, Gemini, Mistral Le Chat) avec plan_gratuit_note. (2) Câblage `ressources` + `check_questions` dans page exercice (dette S1 résorbée) + fix SSR `MiniCheck.svelte` (init directe vs `$effect`). (3) Script `scripts/seed-parcours.mjs` idempotent ESM pur. (4) Pivot transparent zéro vidéo embed : gate YouTube géo CH bloque WebFetch, ressources = 100% liens documentaires officiels (7 URLs VÉRIFIÉES : Anthropic platform docs, Google AI prompting, Anthropic news, Mistral news, BDM, CNIL, HubSpot 2026). (5) Gates verts : check 0/0, Vitest 46/46, build OK, audit Chrome MCP local + prod, console clean, non-régression accueil + allowlist. **2 dettes à traiter AVANT S4** : (a) markdown brut dans consignes exercices (astérisques affichés en clair) - fix ~30 min, payload `memory/project_formation_ia_markdown_consigne_dette.md`. (b) Rotation `SUPABASE_SERVICE_ROLE_KEY` recommandée : un `sed` de masquage `.env` mal échappé a exposé la clé dans les tool-outputs cette session - payload `memory/feedback_sed_env_masking_exposed_key.md`. Côté CRM : aucune modif.

**Session precedente -3 :** Session 73 (scope formation-ia uniquement, docs-only côté CRM - session autonome `/effort high`). **S2 matière pédago livrée bout en bout** : commit formation-ia `b7aa108`, push OK, deploy auto Vercel Ready. (1) **`docs/SOURCES_MARKETING.md`** (207 lignes, committé) : 111 entrées consolidées (12 VÉRIFIÉ + 71 TOLÉRÉ AVEC DISCLAIMER + 28 CHAÎNES CANDIDATES). 6 sources VÉRIFIÉ via WebFetch direct : AI Act Art. 50 (2 août 2026), CNIL + PANAME (26 fév 2026), Anthropic news (Opus 4.7 + Sonnet 4.6 + Skills), Mistral news (Voxtral TTS 23 mars 2026), HubSpot State of Marketing 2026 (chiffres différents de 2025 cités dans deep research, privilégier 2026), BDM comparatif IA (20 nov 2025). (2) **`content/research-synthesis-marketing.md`** (476 lignes, gitignoré via `.gitignore` MAJ) : 12 fiches modules exhaustives (cas ≥ 2, stats ≥ 1, vidéos candidates, angles pédago 2-3/module). Audit couverture OK sur les 12 modules. (3) **Pivot méthodologique transparent** : WebFetch YouTube bloqué par page consent (géo CH) + WebSearch `allowed_domains:youtube.com` = zéro lien exploitable. Décision : vérification HEAD 200 des vidéos spécifiques reportée à S3-S7 au moment de l'embed (gate PEDAGOGIE §5 s'applique au choix de la vidéo, pas en amont). (4) Gates PEDAGOGIE respectés : 96 tirets longs détectés après rédaction → corrigés (replace_all ` — ` → ` - `), grep final 0 match. Côté CRM : aucune modif. Temps 1h20 (time-box 1h-1h30 respecté).
