...
**Derniere mise a jour :** 2026-04-19 (session 101 formation-ia : T8 Cockpit vidéos v1+v2 livré prod bout en bout - infra cockpit `/admin/videos*` + endpoint source + cron mensuel + RPC atomique + 4 composants réutilisables + refonte logique cours-dirigé `video_slots` + 7 fixes UX. 7 commits prod `6af3311` → `09280b3`. T8 [LIVRÉ sous réserve bugs Pascal smoke prod] à valider S102. 0 modif côté CRM.)
...
- **S80-S101** (formation-ia, 2026-04-17 / 2026-04-19) : chantier V2 formation-ia (post livraison parcours marketing 12/12 S79). Audit V1 vs best-practices + bench 15 sites + roadmap V2 11 tâches + cadrage 4 maquettes UX/UI (T1-T4 + T6 + T7 `[VALIDÉ]` 6/11, T8 `[LIVRÉ sous réserve]`). 2 specs : `docs/V2_UX_UI_SPEC.md` 738 lignes + `docs/COCKPIT_ADMIN_SPEC.md` 538 lignes. Hotfix prod S95 RLS allowlist. **S98** T6 Pack 1 MVP V2. **S99** T7 Pack 2 V2 (DashboardResume + AnchorNav + 5 sections + 13 covers SVG + 19 logos + endpoint PDF pdfkit + 7 composants + 2 migrations DB, commits `9f1a8e8` + `bea7d12` + `50b5fd9`). **S100** Groupe dette 1D (`rm -rf wizard/v2-uxui/` + dette a11y 9→0 violations axe-core + tokens --deep + scripts/audit-a11y/ + anti-hype intros J3+J8, commits `0541686` + `0e618a1` + `14d2d38`). **S101** T8 Cockpit vidéos v1+v2 livré prod bout en bout ~4h xhigh autonome (pages `/admin/videos{,/chaines,/sourcer,/alertes}` + endpoint POST source + cron mensuel CRON_SECRET + ConfirmModal + VideoPreviewModal + Toast + BackLink + 3 migrations DB additives + RPC atomique `add_video_to_exercice` + refonte v2 logique cours-dirigé `exercices.video_slots` + 7 fixes UX post-smoke Pascal, subagents 0 High/Critical sécu + 3 High bug-hunter + 5 Medium tous fixés session, 7 commits `6af3311` → `09280b3`, validation Chrome MCP 1280px OK, bugs Pascal smoke prod à corriger S102). Dette héritée T1-T7 à zéro. Règle « Livrable fermé » ajoutée à `~/.claude/rules/methodology.md` § Garde-fous (S99). 0 modif côté CRM. Détails session par session : `formation-ia/CLAUDE.md`.
