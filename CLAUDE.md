...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + email récap cron veille (gated) + export/import CSV + page /reporting + sous-projet `formation-ia/` (S2 livrée + UI refresh + générateur retiré + dette <title> résolue + GitHub→Vercel lié) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-17 (session 71 : cadrage parcours marketing formation-ia 7 sessions autonomes, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente -2 :** Session 69 (scope formation-ia uniquement, docs-only côté CRM). Deux tâches bouclées. (1) **Dette SEO + a11y `<title>`** : 7 routes titrées. Commit formation-ia `32c74cd` + deploy `dpl_B8g7kuwBu8HUi9hGqLfZvejjsjfF`. (2) **Friction 68quater levée** : repo `onboarding-ia` lié au projet Vercel, test bout-en-bout commit vide `5bf14ba` → deploy auto `dpl_7au1agn0h`. **Quiproquo mi-session** : mauvais repo connecté initialement - leçon captée en mémoire projet. **Migration `generate.ts` Opus 4.6 → 4.7 commitée hors session** : commit `499f137` (Pascal via terminal parallèle). `thinking: {type: 'adaptive'}` + `output_config: {effort: 'high'}` spread cast TS. Cron veille hebdo à observer sur 1er run post-migration.
**Session precedente -1 :** Session 70 (scope CRM FilmPro + méta - batch autonome validé par Pascal en Opus 4.7 xhigh + `--dangerously-skip-permissions`). **4 commits CRM pushés** (`1c15e4a..12d8bc5`), 411/411 tests verts (77 nouveaux), zéro régression typecheck. (1) **Fix latent cron veille post-migration Opus 4.7** - commit `e1353ba` : retire `temperature: TEMP_PHASE{1,2}` rejeté 400 sur 4.7 (bug découvert en pré-check avant upgrade xhigh, cron jeudi aurait planté) + bump `effort: 'high'` → `'xhigh'` sur Phase 1 + Phase 2 (défaut Claude Code 4.7, coding+agentic). (2) **Email récap veille post-cron** - commit `e0f0b32` : modules `cost-tracker.ts` (15 tests, tarifs Opus 4.7 $5/$25, Sonnet 4.6 $3/$15, Flux $0.06/img, multiplicateurs cache read 0.1x + creation 1.25x, EUR @ 0.92) + `email-recap.ts` (12 tests, template HTML + plain-text, Resend via fetch direct sans dep npm) + hook `run-generation.ts` best-effort succès+échec. Instrumentation Phase 1/2 + Brief LLM + Vision audit + fal.ai counter. Gate par `EMAIL_RECAP_ENABLED` désactivé par défaut. (3) **Export CSV + Import CLI + Reporting** - commit `12d8bc5` : 3 endpoints GET `/api/export/[entity]` (RFC 4180 + BOM UTF-8, 16 tests) ; script `scripts/import-csv.ts` CLI (parser state-machine RFC 4180 + Zod validation + `--dry-run`, 20 tests) ; page `/reporting` (4 KPIs + 2 bar charts SVG inline + section export, helpers purs 14 tests). Route `/reporting` accessible par URL directe (pas dans nav : `config.navigation` auto-généré depuis project.yaml). **Audit 1j (rapport seul, pas de commit)** : `.claude/commands/cadrage.md` ref `/start` obsolète à actualiser + `deploy.md` caduc (deploy auto GitHub→Vercel doit remplacer `vercel --prod` manuel) + `generate.md` OK tel quel - tracé en tâche `[EXÉCUTABLE - ~30 min]` dans Prochaine session. Côté formation-ia : aucune modif. `scripts/image-rework/` toujours untracked.
**Session precedente :** Session 71 (scope formation-ia uniquement, docs-only côté CRM). **Cadrage parcours marketing validé** : pivot paradigme acté - le parcours marketing devient une **bibliothèque de 12 modules indépendants** (pas un parcours linéaire 5 jours). Navigation à la carte, progression locale par module, Claude = ancre principale des recommandations, ChatGPT alternative fréquente, outils tiers pour tâches spécifiques. **Livrables session (commit formation-ia `65c40d5` docs-only)** : (1) `docs/PEDAGOGIE.md` mis à jour - règle source nuancée (standard URL primaire / toléré avec disclaimer si confiance ≥ 90% / interdit), autorisation Claude+ChatGPT, 2 nouveaux gates bloquants (Source URL vérifiée + Vidéos embed). (2) `docs/PLAN_PARCOURS_MARKETING.md` créé - payload exhaustif 7 sessions autonomes (S1 infra additive, S2 matière pédago WebFetch, S3-S7 ingestion par famille fondations/production/analyse/cadre) avec niveau effort + durée + confiance + risques par session. (3) `docs/SOURCES_MARKETING.md` créé - squelette référentiel sources vérifiées. (4) `formation-ia/CLAUDE.md` réécrit - 7 tâches séquentielles S1-S7. **Garde-fous actés** : zéro régression (migrations additives only, charte verrouillée, screenshots before/after) + zéro hallucination nuancée (source primaire cible + secondaire + disclaimer tolérée si confiance ≥ 90%, invention interdite). 2 deep researches Pascal (Claude chat + Google research iCloud) = pistes à valider, pas sources primaires. Payload référencé en mémoire : `memory/project_formation_ia_plan_parcours_marketing.md`. **Note hors session** : `scripts/image-rework/` toujours untracked à la racine AppFactory (hérité session 68quater, à statuer).
