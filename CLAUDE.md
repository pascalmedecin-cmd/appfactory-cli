...
**Derniere mise a jour :** 2026-04-14 (session 62 : push prod Blocs 3+4, validation Chrome MCP avortée extension déconnectée — reportée)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente -3 :** Bloc 2bis qualité sources LLM livré (fenêtre 14j + prompt durci). Régen W16 : 2/5 date_ok=true.
**Session precedente -2 :** Bloc 2 pipeline 2 phases Veille livré (commit `be08674`). Régen W16 : 2/2 date_ok=true, 0 speculatif auto. Détail `archive/handoff-session-60.md` (à archiver).
**Session precedente -1 :** Bloc 4 pont Veille→Prospection auto-exécutable (commits `7682ec4` + `877c3a2`) + Bloc 3 scoring v2 signaux Veille (`3aa1ad6`). **Bloc 4A** : `SearchChipSchema{kind,canton,query,label}` via union Zod avec transform string→chip (rétro-compat pré-Bloc 4), JSON schema `emit_report` structuré, prompt Phase 2 réécrit avec exemples SIMAP/Zefix. **Bloc 4B** : endpoint `POST /api/prospection/from-intelligence` (valide chip + route vers SIMAP/Zefix via `event.fetch`, propage `from_intelligence`/`from_term`/`from_item_rank`), chips UI deviennent `<button>` loading state + redirect auto vers `/prospection?source=X&canton=Y&from_intelligence=Z`, `normalizeStoredChips` au load pages veille (parse mix legacy/structuré). **Bloc 3** : `scoring.ts` param optionnel `intelligenceSignal{maturity,complianceTag,weeksSince}`, barème etabli+OK_FilmPro=+2/etabli=+1/emergent=+1/speculatif=0, décroissance >4 semaines → bonus perdu. `signal-lookup.ts` (défensif). Propagation 3 endpoints import (zefix/simap/regbl) : `from_item_rank` accepté, lookup unique par batch. Tests 285/285 verts (+15), typecheck 3 erreurs pré-existantes inchangées. **Validation Chrome MCP NON effectuée** (requiert dev server + session Supabase) — à faire prochaine session. Détail : `notes/handoff.md`.
**Session precedente :** Session 62 très courte. Push Blocs 3+4 sur prod (commits `20f7b1f..af31911`, main sync GitHub). Tentative validation Chrome MCP avortée : localhost:5173 exige login OTP (session ≠ prod) → bascule prod, puis extension Chrome Claude déconnectée avant le test. Aucun code modifié. Validation parcours live + régen W17 reportées prochaine session (objectif inchangé).
...
- [x] ~~Bloc 4 — Auto-exécution prospection depuis Veille~~ — Fait 2026-04-14 session 61 (commits `7682ec4` + `877c3a2`) : `SearchChipSchema{kind,canton,query,label}` via union Zod rétro-compat, prompt Phase 2 réécrit, endpoint `/api/prospection/from-intelligence`, chips UI cliquables avec loading + redirect auto, `normalizeStoredChips` au load. Pré-requis adressé via /dig (verdict Option B) : REGBL exclu du pont (pas de filtre texte), SIMAP+Zefix supportés. **À valider parcours live Chrome MCP prochaine session**.
...
- [ ] **[EXÉCUTABLE — Bug veille URL doublée — autonome ~30min]** Bug Phase 2 réécrit URL candidat avec segment de chemin doublé (observé session 60 : `.../pompe-a-chaleur.../pompe-a-chaleur-radiateurs-...`). HEAD répond 200 mais URL sémantiquement fausse. Diagnostiquer sur régen W17 : logger l'URL candidat vs URL finale émise par Phase 2. Si reproduit → fixer prompt Phase 2 ou ajouter garde-fou `rejectIfUrlMutated` dans validation schema.
...
1. **Validation parcours live Chrome MCP** (Bloc 3 + 4, 15 min) : chip legacy W16 → redirect prospection, vérifier count imported + score bonus. Pré-requis avant régen W17.
2. **Régen W17** pour valider chips structurés LLM (lecture `intelligence_reports.items[].search_terms` après génération).
