...
**Prochaine attaque** : Bloc 1 Refonte densité /prospection mobile (post-V1) - momentum CRM frais, payload complet créé S125, suite logique de la clôture V1 mobile. OU Bloc 2 Harmonisation PDF FilmPro si tu préfères changer de contexte.

### 1. Refonte densité /prospection mobile (post-V1) [MIXTE • high • ~3h]

**Pourquoi** : identifié S125 lors validation visuelle screenshot golden v5 mobile. Page très dense, ~600px de scroll avant la 1re ligne table (4 cartes raccourcis + bouton import + 4 filtres + bouton alerte + recherche redondants). Pascal a explicitement demandé une refonte. Hors scope V1 (clos), traité en bloc dédié.
**Prérequis** : aucun. Cadrage 4 questions à trancher en début de session.

- [ ] **[EXÉCUTABLE]** Cadrage + implémentation refonte densité /prospection mobile : trancher Q1 cartes raccourcis (afficher mobile ou non, et sous quelle forme), Q2 filtres (visibles vs collapsés derrière bouton avec badge compteur), Q3 bouton "Créer une alerte" (primary/secondary/kebab), Q4 recherche (sticky scroll ou fixe). Implémenter responsive variants. Test:mobile étendu pour mesurer hauteur scroll avant 1re ligne table (cible <400px). Re-screenshot golden v6 mobile. → voir `memory/project_prospection_mobile_density_redesign.md`

### 2. Harmonisation PDF FilmPro [MIXTE • high • ~2h]

**Pourquoi** : aligner les deux stacks PDF (`playbook-pdf` WeasyPrint et `filmpro-pdf-lite` reportlab) sur un standard commun, combler gaps G1-G3-G5. Indépendant des décisions FilmPro shared-assets (~/.claude/CLAUDE.md Bloc 4) qui ne touchent que les assets de marque, pas la stack PDF.
**Prérequis** : aucun.

- [ ] **[EXÉCUTABLE]** Harmoniser production PDF FilmPro : aligner `playbook-pdf` (WeasyPrint) et `filmpro-pdf-lite` (reportlab). Reco option [3] coexistence + combler gaps G1-G3-G5. → voir `memory/project_filmpro_pdf_harmonization.md`
