...
**Statut :** Phase C, Skills et templates HTML + module Veille sectorielle refondu LEAN (S112, prod) + email récap cron veille (gated, actif) + export/import CSV + page /reporting + Golden Standards CRM v3 livré + Phase 4 application `/prospection` en prod (S114) + Migration Material Symbols → Lucide complète (S115) + Cascade golden v3 Bloc 1a Phase 3+4 LIVRÉE INTÉGRALEMENT 6/6 pages CRM (S116-S117, 8 commits push main) + Sweep palette opacités primary + Badge variant info + Select.svelte partagé + golden v4 doc-only (S118, 3 commits + 1 doc) + Alignement `--color-info` ardoise FilmPro + golden v5 (S119, 1 commit) + Intégration Veille→Prospection complète (S120, 1 commit) + **Cadrage CRM mobile V1 validé : spec MD + HTML, 8 décisions structurelles tranchées, 4 sessions A/B/C/D séquencées (S121, doc-only)**. Formation IA est un sous-projet autonome dans `formation-ia/`, accessible directement via `cc` option 2.
**Derniere mise a jour :** 2026-04-27 (session S121 CRM xhigh : cadrage CRM mobile V1 livré en 2 fichiers doc — `docs/SPECS_CRM_MOBILE.md` + `docs/SPECS_CRM_MOBILE.html`. Aucune modification code, aucun test impacté.)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** Session 121 (CRM, 2026-04-27, `/effort xhigh`). 0 commit (cadrage doc-only, à committer en clôture). **Bloc 1 CRM terrain mobile** : cadrage validé après 3 itérations Pascal sur le périmètre. Principe directeur verrouillé : « pas d'UX mobile-spécifique » (référence Linear/Notion/Stripe). 8 décisions structurelles : pas de bottom nav, modales restent modales, OTP suffit (pas de biométrie), PWA légère (manifest + SW assets), tables priorisation colonnes (pas card-style), pas d'offline queue, pas de capture native (photo/geo/voice), 4 sessions séquencées A/B/C/D ~6h cumulées. 14 features hors scope V1 verrouillées. **Bilan** : cadrage clos, V1 implémentation Session A `[EXÉCUTABLE]` pour S122. Bloc 2 (validation cron veille W18) reste bloqué date ≥ 01/05.
**Session -1 :** Session 120 (CRM, 2026-04-27, `/effort xhigh`). 1 commit push main `9df286e` + push S119. Intégration Veille→Prospection 7 phases (migration `prospect_lead_signals`, scoring agrégation plafond=4, 3 modules `recompute-score`/`apply-signals`/`link-import-signal`, cron `/api/cron/lead-rescore`, pont Veille étendu RegBL). Vitest 365/365.
**Session -2 :** Session 119 (CRM, 2026-04-26, `/effort low`). 2 commits push main. Bloc 1a Alignement `--color-info` ardoise FilmPro + golden v5.
**Session -3 :** Session 118 (CRM, 2026-04-26, `/effort xhigh`). 3 commits push main + 1 livrable doc. Bloc 1a sweep opacités primary + Bloc 1e Badge accent → info + Bloc 1b Select.svelte composant partagé + Bloc 1c golden v4 doc-only.
...
**Prochaine attaque** : Bloc 1 V1 mobile Session A - audit responsive 8 pages × 2 viewports (iPhone 14 + Pixel 7) + PWA infra légère (manifest + service worker cache assets). Cadrage validé S121, principe directeur verrouillé : pas d'UX mobile-spécifique (référence Linear/Notion/Stripe). 4 sessions séquencées A/B/C/D (~6h cumulées).

### 1. CRM mobile V1 — Sessions séquencées A/B/C/D [MIXTE • xhigh • cascade 4 sessions]

**Pourquoi** : cadrage validé S121 (`docs/SPECS_CRM_MOBILE.md` + `.html`). Toutes les pages CRM doivent être lisibles sur mobile, pas d'UX différente. Décisions structurelles verrouillées (8) : pas de bottom nav, modales restent modales, OTP suffit, PWA légère, tables = priorisation colonnes.
**Prérequis** : aucun (Session A démarrable). Sessions B/C/D bloquées par session précédente.

- [ ] **[EXÉCUTABLE]** Session A V1 mobile : audit chrome MCP 8 pages × 2 viewports (iPhone 14 + Pixel 7) screenshots + findings.md, audit chrome global (sidebar 260px iPhone SE, tap targets, scroll lock, z-index), manifest PWA + service worker cache assets, test Add to Home Screen iOS + Android. Critères : findings priorisé + PWA installable + 0 régression desktop. ~1.5h MIXTE. → voir `memory/project_crm_mobile_v1_cadrage.md` + `docs/SPECS_CRM_MOBILE.md` section 7
- [ ] **[BLOQUÉ - Session A livrée]** Session B V1 mobile : sweep pages denses (prospection, pipeline, entreprises, contacts). Définir matrice priorisation colonnes par page, masquage Tailwind `hidden md:table-cell`/`hidden lg:table-cell`, vérif filtres reflow + modales full-width, tests Playwright mobile 4 pages. Critères : 0 scroll horizontal table, modales lisibles, filtres exploitables. ~2h MIXTE. → voir `memory/project_crm_mobile_v1_cadrage.md`
