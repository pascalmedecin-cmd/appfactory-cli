...
**Statut :** Phase C, refonte /prospection phase 2 LIVRÉE (S157 2026-05-01 xhigh : 4 onglets par nature de signal + colonnes resizables + tooltip header + sélecteur entrées par page + tri stack bidirectionnel + fix typo + charte v8). V2 mobile terrain CLOS (S127 α + S129 β + S130 γ). V1 MOBILE CLOS (S125). Formation IA = sous-projet autonome dans `formation-ia/`, `cc` option 2.
**Derniere mise a jour :** 2026-05-01 (S157 CRM xhigh : refonte /prospection phase 2. 3 commits `8ea58e2` + `7fa7829` + `a48a9c1`. Nouveaux composants Tooltip + ProspectionTabs. DataTable étendu opt-in rétrocompat. 4 nouveaux tokens onglets, source unique TabKey/TAB_SOURCE_MAP/SORT_FIELDS dans prospection-utils. yaml aligné avec lead_express + veille. 4 audits subagents tous traités no-debt. vitest 406/406, build prod 11s, Vercel deploy READY 40s, smoke test prod HTTP 303 OK).
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session courante :** Session 157 (CRM, 2026-05-01, `/effort xhigh`).
...
**Prochaine attaque** : Bloc #1 - Tests E2E Playwright Phase 2 (3 tests : sélecteur entrées/page, drag colonnes, switch onglet vide). Dette honnête signalée à Pascal en clôture S157 — Phase 2 livrée prod sans validation E2E des nouveaux livrables. ~30 min, AUTO. Ferme proprement la Phase 2 avant Phase 3.


<!-- BEGIN CONSOLIDATION (auto-géré par cockpit, ne pas éditer) -->

### Consolidation cockpit (maj 2026-05-01T09:33:32)

**Blocs actionnables** (ordre d'attaque) :

- **Bloc #1** - Reconciliation yaml ↔ config CRM (0.75h, confiance Élevé)
  - Objectif : Aligner template/project.yaml sur config.ts (vérité Pascal-validée) pour éviter écrasement régression
  - Aligner template/project.yaml sur template/src/lib/config.ts (vérité Pascal-validée) : (1) retirer lindas + activer zefix + ajouter regbl dans prospection.sources, (2) restaurer accents fr labels pipe

- **Bloc #2** - Phase 2 prospection - onglets par nature (5.0h, confiance Moyen)
  - Objectif : Cadrage + mockup HTML onglets SIMAP/RegBL/Zefix/Terrain pour pain marchés publics noyés
  - Cadrage Phase 2 onglets par nature : (1) trancher granularité (4 vs 3 onglets si volumes maigres - regrouper Entreprises+Terrain ?). (2) Définir onglets sticky vs dropdown selon viewport. (3) Colonne
...
- **Bloc B2** [BLOQUÉ] - Phase 3 prospection - généalogie + dashboard coûts (5.5h)
  - Objectif : Phase 3 généalogie cross-pages + dashboard coûts CRM, livrables bloqués par prérequis distincts
  - Blocage : Prérequis bloquants : Phase 2 livrée pour généalogie + session dashboard dédiée pour coûts
  - Débloque si : Phase 2 livrée (bloc #2) + créneau session dashboard dédiée planifié
  ...
  - Dashboard coûts CRM /dashboard/couts : table cost_audit_runs + graphique 12 sem + split cron/catégorie + seuils.

<!-- END CONSOLIDATION -->

### 1. Tests E2E Playwright Phase 2 prospection [AUTO • medium • ~30 min]

**Pourquoi** : Phase 2 livrée S157 prod (4 onglets + colonnes resizables + sélecteur entrées/page + tooltip header + tri stack). 4 audits subagents passés mais zéro test E2E ajouté. Pascal a découvert un bug sélecteur entrées/page que la suite vitest ne couvrait pas (fixé commit `a48a9c1` sans test régression). Dette à fermer avant Phase 3 sinon prochain refacto risque régression silencieuse.
**Prérequis** : Phase 2 prod stable (commits `8ea58e2` + `7fa7829` + `a48a9c1` Vercel READY).

- [ ] **[EXÉCUTABLE]** Ajouter `template/tests/prospection-phase2.spec.ts` avec 3 tests Playwright contre prod (preset desktop + iPhone 14 Pro Max) : (a) cliquer sélecteur entrées/page → URL contient `?perPage=50` + 50 lignes max affichées + sélecteur reflète la valeur après reload, (b) drag handle entre colonne A et B → largeur de A change visuellement + persistée localStorage `datatable.col-widths.prospection-simap` + restaurée après reload, (c) switch vers onglet Terrain (count=0) → onglets toujours visibles + cliquables + retour SIMAP fonctionne. Vérifier aussi mobile : tabs deviennent dropdown <768px. → voir `notes/refonte-prospection-2026-05-01-phase2/mockup-prospection-phase2.html` pour patterns visuels attendus.
...
### 4. Dette dedup createExpress multi-site [SUPERVISÉ • medium • ~45 min]

**Pourquoi** : risque ouvert préexistant S130, signalé H4 par bug-hunter Phase 2 S157 (hors scope strict Phase 2 = préexistant). `createExpress` action peut faire un faux match sur dedup quand plusieurs entrées partagent la même raison sociale (chaînes, multi-sites) et que `telNorm.length < 6`. L'utilisateur croit créer un nouveau lead pour un site différent (Vevey vs Lausanne) et il est silencieusement redirigé vers le premier doublon. Pas de cassure technique mais corruption métier latente.
**Prérequis** : aucun. Validation Pascal sur la stratégie de désambiguation requise (modale ? flag duplicate weak ?).

- [ ] **[EXÉCUTABLE]** Refactor `createExpress` ligne 466-476 de `template/src/routes/(app)/prospection/+page.server.ts` : si `telNorm.length < 6` ET `candidates.length > 1`, retourner `{success: false, ambiguous: true, candidates: [{id, raison_sociale, localite}]}` pour que la modale LeadExpress affiche les options. Côté UI, ajouter un step de désambiguation. Tests : 3 cas (telNorm long + match unique = OK, telNorm court + 1 candidate = silent redirect OK, telNorm court + 2+ candidates = modale options).
