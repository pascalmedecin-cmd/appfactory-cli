...
**Statut :** Phase C — Skills et templates HTML + module Veille sectorielle en production + pipeline images 4 niveaux (Flux 1.1 Pro Ultra + audits Vision) + sous-projet `formation-ia/` (S2 livrée + UI refresh + générateur retiré 68quater) + `/start` AppFactory en dispatcher 2 branches (FilmPro / Formation IA)
**Derniere mise a jour :** 2026-04-16 (session 68quater : audit santé prod formation-ia + retrait générateur de prompts, docs-only côté CRM)
**Derniere revue /optimize :** 2026-04-05
**Prochain bug :** #001
**Session precedente -2 :** Session 67 (Bloc 6ter/quater livré, **QA visuel images rejeté en fin de session**). Pipeline images refondu en cascade 4 niveaux : og:image filtrée → fal.ai **Flux 1.1 Pro Ultra** ($0.06/img, 2K natif) avec brief LLM + audit Vision Sonnet ≥6/10 → media_library picker → gradient. **Retrait complet Pexels + Unsplash** (code, cron, env vars Vercel). Lib purgée 156 → 44 images. Bascule Veille text Sonnet → **Opus 4.6**. Vision audit Sonnet (contrainte 300s Vercel Hobby). 334/334 tests verts. **Pascal a signalé : "image veille test non conformes, continuer cadrage QA prochaine session"** — voir `memory/project_qa_images_veille.md`. (Sessions 65-66 archivées : bascule cron Veille vendredi → jeudi + livraison Bloc 6bis bibliothèque media_library 41 img + dedup SHA256 + cron enrich, contexte récupérable via `git log` ou condensé thématique plus bas.)
**Session precedente -1 :** Session 68ter (docs-only côté CRM ; grosse livraison côté formation-ia). Refonte `/start` AppFactory en **dispatcher 2 branches** : menu racine affiche `[1] CRM FilmPro` / `[2] Formation IA`, chaque branche bascule vers le `/start` complet scopé au sous-projet (tâches, idées, options projet). Règle : tâches linéaires une fois dans un sous-menu, plus de groupement cross-projet. Côté formation-ia, workflow conversationnel d'ingestion de contenu pédagogique créé : `docs/PEDAGOGIE.md` (8 règles + anti-patterns + gates bloquants, adapté Enseignement) + `docs/INGESTION.md` (protocole 6 étapes Opus 4.6, validation par jour, seed idempotent) + `.claude/start-menu.md` Formation IA avec option `[3] Intégrer un parcours`. Multi-thèmes prévu (marketing aujourd'hui, opération/commercial demain, même pipeline). **Règle modèle** : production contenu pédago = Opus 4.6 obligatoire, jamais Sonnet, jamais API in-app (cf. `memory/feedback_opus_for_pedago_content.md`). **En attente** : deep research marketing Claude chat (Pascal la produit côté Claude chat, fichier iCloud initial reçu vide suite à crash Claude AI). Commits : AppFactory `820cb44` + formation-ia `6980086` pushés.
**Session precedente :** Session 68quater (scope formation-ia uniquement, docs-only côté CRM). Audit santé prod formation-ia (5 pages auditables : accueil, parcours/marketing, générateur, historique, admin/allowlist) en attente deep research → zéro régression, console clean, dette mineure `<title>` vide identifiée (jamais settée en S2/68bis). Décision Pascal : **retrait complet du générateur de prompts** (« inutile, on ajoutera un module de formation sur best prompting »). Commit formation-ia `a384c75` : suppression routes `/generateur-prompts/*` + nav items + section Outils accueil + 3 refs `prompt_templates` dans INGESTION.md. Tables `user_prompts` + `prompt_templates` conservées inertes (pattern badges). Deploy prod manuel `dpl_CMFdiiFsMTs6ZdLSKGuPjTQjxfvN` après confirmation que push GitHub seul ne déclenche aucun deploy (repo non lié à Vercel). Audit post-deploy : sidebar épurée à 3 items, accueil sans section Outils, `/generateur-prompts` → 404. Côté CRM FilmPro : aucune modif code. **Note hors session** : `scripts/image-rework/` reste untracked à la racine (hérité d'une session précédente d'expérimentation pipeline images, scripts datés 2026-04-16 13h, à statuer avec Pascal — commit, gitignore, ou suppression).
