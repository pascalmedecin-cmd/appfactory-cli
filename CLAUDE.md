...
- [ ] **[EXÉCUTABLE — ~10 min]** Activer email récap veille (prod). (a) Ajouter env Vercel prod : `EMAIL_RECAP_ENABLED=true` + `RESEND_API_KEY=re_...`. (b) Premier test live = cron jeudi 23 avril 7h UTC (ou trigger manuel via `/api/intelligence/trigger` après activation). (c) Vérifier email reçu `pascal@filmpro.ch` : subject W{week} + split coûts cohérent vs logs Vercel. Spec impl : `memory/project_email_veille_recap.md` + commit `e0f0b32`.
